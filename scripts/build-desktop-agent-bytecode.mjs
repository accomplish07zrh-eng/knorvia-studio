...
const defaultEntryPath = join(repoRoot, "apps/cli/packages/cli/dist/knorvia.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.KNORVIA_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "knorvia.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `knorvia.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `knorvia.bytecode-runtime-${runtimeHash}.cjs`;
