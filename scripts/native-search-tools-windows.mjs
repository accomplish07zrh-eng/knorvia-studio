...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "knorvia-native-search-build-"));
  ...
  console.log("==> Knorvia Studio native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DKNORVIA_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DKNORVIA_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DKNORVIA_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DKNORVIA_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DKNORVIA_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DKNORVIA_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DKNORVIA_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DKNORVIA_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(
