...
const KNORVIA_AGENT_RUNTIME = {
  knorvia: {
    version: readKnorviaAgentRuntimeVersion(),
  },
};
...
/**
 * Node dist 下载源。默认走国内镜像，`KNORVIA_NODE_DIST_MIRROR` 可覆盖（与
...
export function nodeDistBase(env = process.env) {
  const mirror = env.KNORVIA_NODE_DIST_MIRROR?.trim();
  return (mirror || DEFAULT_NODE_DIST_BASE).replace(/\/+$/u, "");
}
const BROWSER_USE_PLUGIN_PACKAGE_NAME = "@knorvia/browser-use-plugin";
// node_repl 宿主抽成独立包 @knorvia/node-repl-host 之后，browser-use
// 不再产出 dist/mcp/server.js，CUA 资产也已归 @knorvia/cua-plugin。这是**第三份**平行清单
...
const remoteOfficialPluginPackages = [
  ...
  // 清单、packages/server/src/remote/agentOfficialPluginAssets.ts 的远端合同保持一致。
  {
    // 远端 shared-host 必须部署 node_repl runtime，否则只剩 skill 而没有 mcp__node_repl__js ——
    // 该 runtime 现由 @knorvia/node-repl-host 提供（见下一个条目），browser-use 只带自己的
...
const remoteOfficialPluginRequiredPaths = [
  "packages/browser-use-plugin/.knorvia-plugin/plugin.json",
  "packages/node-repl-host/.knorvia-plugin/plugin.json",
];

function readKnorviaAgentRuntimeVersion() {
  const runtimeSourcePath = join(rootDir, "packages/shared/src/agent-runtime.ts");
  const runtimeSource = readFileSync(runtimeSourcePath, "utf8");
  const match = runtimeSource.match(/version:\s*["']([^"']+)["']/);
  if (!match?.[1]) {
    throw new Error("Unable to parse Knorvia Studio Agent runtime version");
  }
  return match[1];
}
...
async function extractArchiveMember(url, destinationDir, archiveMember) {
  const tempDir = mkdtempSync(join(tmpdir(), "knorvia-node-dist-"));
...
async function prepareNodeBinaries() {
  for (const platformKey of remotePlatforms) {
    ...
    try {
      ...
      console.error(
        `  [error] 请检查 CI runner 的外网访问、tar/xz 依赖，或用 KNORVIA_NODE_DIST_MIRROR 覆盖下载源（当前 ${nodeDistBase()}）`,
      );
      throw error;
    }
  }
...
function runBootstrapServerRemoteBuild() {
  ...
  // 这里不能复用已有 knorvia-server.cjs：开发时 package version 常不变，旧 bundle 会把缺少新 RPC 的
...
function copyServerBundle() {
  ...
  copyFileSync(
    join(rootDir, "packages/server/dist/remote/knorvia-server.cjs"),
    join(serverDir, "knorvia-server.cjs"),
  );
  console.log("  [ok] mock-cdn server/knorvia-server.cjs");
}
...
function buildRemoteOfficialPluginRuntimes() {
  for (const plugin of remoteOfficialPluginPackages) {
    ...
    runCommand(
      pnpmCommand,
      ["--dir", join(rootDir, "apps/cli"), "--filter", plugin.packageName, "build"],
      {
        cwd: rootDir,
        env: process.env,
      },
...
function stageRemoteOfficialPlugins(agentDir) {
  for (const plugin of remoteOfficialPluginPackages) {
    const sourceRoot = join(rootDir, plugin.relativePath);
    const manifestPath = join(sourceRoot, ".knorvia-plugin", "plugin.json");
    ...
    const targetRoot = join(agentDir, ...plugin.stagedPath.split("/"));
    ...
    for (const relativePath of remoteOfficialPluginRequiredPaths) {
      ...
      const stagedAssetPath = join(agentDir, ...relativePath.split("/"));
    ...
    console.log(`  [ok] mock-cdn knorvia official plugin ${plugin.stagedPath}`);
  }
}

// 远端 agent 现在跑编译出来的 knorvia.cjs（而不是各平台独立的原生二进制）：
// 远端部署时已经有一份独立 node（跑 knorvia-server.cjs），agent 复用它执行 knorvia.cjs 即可，
// 不必再为每个平台准备一份内嵌 node 的 SEA 二进制。knorvia.cjs 跨平台同一份，逐平台只是放进各自的
// knorvia/<platform> 组件目录，保持现有 manifest 组件结构不变。
function stageRemoteAgentBundles() {
  console.log("==> Building cli bundle for remote agents");
  // 复用桌面同款构建脚本（turbo build:desktop-agent --filter=@knorvia/cli），命中缓存时几乎瞬时。
  runCommand(process.execPath, [join(rootDir, "scripts/build-desktop-agent-cli.mjs")], {
    cwd: rootDir,
    env: process.env,
  });
  // browser-use runtime 的 tsc 依赖 @knorvia/core/dist。远端资产也必须先构建
  // agent CLI 依赖，避免 CI 干净检出时被开发机缓存掩盖的 TS2307。
  buildRemoteOfficialPluginRuntimes();
  const cliBundlePath = join(rootDir, "apps/cli/packages/cli/dist/knorvia.cjs");
  if (!existsSync(cliBundlePath)) {
    throw new Error(`[prepare-prebuilds] expected cli bundle missing: ${cliBundlePath}`);
  }

  for (const platformKey of remotePlatforms) {
    const agentDir = join(releaseDir, "knorvia", platformKey);
    // 干净重建：knorvia 组件现在只含 knorvia.cjs，清掉历史遗留的原生二进制 / 旧 meta，
    // 避免被打进组件 tar 把远端资源撑大。
    rmSync(agentDir, { recursive: true, force: true });
    mkdirSync(agentDir, { recursive: true });
    copyFileSync(cliBundlePath, join(agentDir, "knorvia.cjs"));
    stageRemoteOfficialPlugins(agentDir);
    console.log(`  [ok] mock-cdn knorvia/${platformKey}/knorvia.cjs`);
  }
}
...
// knorvia 承载 cli app-server 协议 schema。即使 runtime 版本未变化，
// knorvia.cjs 也可能随 app 代码变更；跨 release 复用旧 knorvia 会让远端 agent 拒绝新协议字段。
const nonReusableReleaseAssetIds = new Set(["server-bundle", "knorvia"]);
...
function buildReusableComponentRequiredPaths(componentId, platformKey) {
  switch (componentId) {
    ...
    case "knorvia":
      // Knorvia Agent 现在是编译产物 knorvia.cjs（跨平台同一份），远端用已部署的 node 执行它。
      // 复用时还要确认官方插件 seed 资源完整，否则旧 release 会继续产出 0 builtin plugin 的远端资源包。
      return ["knorvia.cjs", ...remoteOfficialPluginRequiredPaths];
    case "bfs":
      return ["bfs"];
    case "ripgrep":
...
export function buildRemoteComponentDefinitions(platformKey) {
  const baseComponents = [
    ...
    {
      id: "knorvia",
      // Knorvia Agent native binary 之前固定成 v1，二进制版本升级后不会触发组件 cache 失效。
      // 这里复用 KNORVIA_AGENT_RUNTIME.knorvia.version，保持 manifest 版本与运行时描述一致。
      semanticPrefix: KNORVIA_AGENT_RUNTIME.knorvia.version,
      mount: joinPosix("knorvia", platformKey),
      sourcePath: join(releaseDir, "knorvia", platformKey),
    },
  ];
...
async function main() {
  ...
  for (const platformKey of remotePlatforms) {
    ...
    await stageThirdPartyNotices(join(releaseDir, "knorvia", platformKey), rootDir);
  }
