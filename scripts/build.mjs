...
import {
  ...
  stageTuiRuntime,
} from "./distribution/assets.mjs";
import { installScriptSource } from "./distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "knorvia");
const defaultBaseUrl = (await loadEndpointEnv()).KNORVIA_DIST_BASE_URL?.trim() || "";
const packageDirName = "knorvia";
const usage = `Usage:
  pnpm build:knorvia
  node scripts/build.mjs --skip-build
  node scripts/build.mjs --version 3.3.3-dev.1
  node scripts/build.mjs --out-dir dist/knorvia
  node scripts/build.mjs --base-url http://host/knorvia/deps/knorvia/
...
Options:
  ...
  --out-dir <path>    Output directory. Defaults to dist/knorvia.
...
function run(command, args, options = {}) {
  console.log(`[knorvia] ${commandText(command, args)}`);
...
async function buildOutputs(skipBuild) {
  if (skipBuild) {
    console.log("[knorvia] skipping build; reusing existing outputs");
    return;
  }

  run("pnpm", ["--filter", "@knorvia/cli...", "build"]);
  await rm(resolve(root, "packages", "server", "dist"), {
    force: true,
    recursive: true,
  });
  run("pnpm", ["--filter", "@knorvia/server", "build"]);
  run("pnpm", ["--filter", "@knorvia/web", "build"]);
}

async function stageKnorviaPackage({ packageRoot, version }) {
  const webDist = resolve(root, "packages", "web", "dist");
  const serverDist = resolve(root, "packages", "server", "dist");
  const agentBundle = resolve(root, "apps", "cli", "packages", "cli", "dist", "knorvia.cjs");
  const agentProvider = resolve(root, "apps/cli/packages/cli/dist/provider");
  ...
  await assertFile(resolve(agentProvider, "builtin.json"), "Agent provider config");
  ...
  await cp(agentBundle, resolve(packageRoot, "agent", "knorvia.cjs"));
  // TUI 入口通过真正的 CLI 路径定位伴随配置；只复制 JS 会在仓库外启动失败。
  await cp(agentProvider, resolve(packageRoot, "agent/provider"), { recursive: true });
  await cp(
    resolve(root, "apps/cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
    resolve(packageRoot, "agent/THIRD-PARTY-NOTICES.md"),
  );
  await chmod(resolve(packageRoot, "agent", "knorvia.cjs"), 0o755);
  ...
  const runner = resolve(packageRoot, "bin", "knorvia.mjs");
  await cp(resolve(root, "scripts/distribution/runner.mjs"), runner);
...
async function main() {
  ...
  if (!options.help && !options.baseUrl)
    throw new Error("Configure KNORVIA_DIST_BASE_URL in .env or pass --base-url");
  if (options.help) {
    console.log(usage);
    return;
  }
  ...
  if (!version || typeof version !== "string") {
    throw new Error("Unable to resolve Knorvia Studio version.");
  }
  ...
  console.log(`[knorvia] release directory: ${outDir}`);
  console.log(`[knorvia] tarball: ${tarball}`);
  console.log(`[knorvia] sha256: ${sha256}`);
}
