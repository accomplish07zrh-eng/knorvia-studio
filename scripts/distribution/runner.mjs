...
const agentEntry = join(root, "agent", "knorvia.cjs");

function usage() {
  return `Usage:
  knorvia --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  knorvia --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      KNORVIA_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      KNORVIA_AGENT_SERVER_COMMAND: process.execPath,
      KNORVIA_SERVER_HOST: options.host,
      KNORVIA_SERVER_WORKSPACE: options.workspace,
      KNORVIA_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      KNORVIA_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("Knorvia Studio Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: knorvia --web [options] (knorvia --web --help for details)\n");
    }
