...
import {
  collectSeaTuiAssets,
  seaTuiAssetPrefix,
} from "../../apps/cli/packages/cli/scripts/sea-tui-assets.mjs";
import { supportedTargets } from "../../apps/cli/packages/cli/scripts/sea-targets.mjs";
...
async function copyRuntimePackageTree({ packageName, packageRoot, requireFrom, seen }) {
  ...
  for (const dependencyName of Object.keys(dependencies)) {
    try {
      ...
      console.warn(`[knorvia] optional package ${dependencyName} is unavailable; skipping`);
    }
  }
}
...
export async function copyRuntimeNodeModules(packageRoot) {
  ...
  await copyRuntimePackageTree({
    ...
    requireFrom: createRequire(resolve(root, "apps/cli/packages/cli/package.json")),
    seen: new Set(),
  });
}
...
export async function patchNodePtyPrebuilds(packageRoot) {
  ...
  for (const packageName of lydellNodePtyPackages) {
    ...
    try {
      ...
      console.warn(`[knorvia] ${packageName} is unavailable; skipping node-pty prebuild patch`);
      continue;
    }
