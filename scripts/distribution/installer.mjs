...
BASE_URL="\${KNORVIA_DIST_BASE_URL:-${baseUrl}}"
INSTALL_DIR="\${KNORVIA_DIST_HOME:-$HOME/.knorvia-studio/runtime}"
BIN_DIR="\${KNORVIA_DIST_BIN_DIR:-$HOME/.local/bin}"

need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "knorvia install requires $1" >&2
    exit 1
  fi
}
...
cat > "$BIN_DIR/knorvia" <<SH
#!/usr/bin/env sh
exec node "$INSTALL_DIR/current/bin/knorvia.mjs" "\\$@"
SH
chmod +x "$BIN_DIR/knorvia"

echo "Knorvia Studio $VERSION installed."
echo "Run: knorvia (TUI) or knorvia --web (Web)"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
