  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    const child = spawn(command, spawnArgs, {
      ...
      env: withPinnedNodePath(
        {
          ...process.env,
          KNORVIA_ENV: requestedEnv,
          KNORVIA_DESKTOP_AGENT_BYTECODE: agentBytecode ? "1" : "0",
        },
        process.execPath,
      ),
      stdio: "inherit",
...
try {
  ...
  await run(pnpmCommand, ["--filter", "@knorvia/desktop", "pre-dev"]);
  ...
  await run(pnpmCommand, ["--filter", "@knorvia/desktop", "dev:runtime"]);
} catch (error) {
