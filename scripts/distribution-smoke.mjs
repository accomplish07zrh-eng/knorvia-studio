...
assert.ok(archive, "Usage: node scripts/distribution-smoke.mjs <archive.tar.gz>");
const directory = await realpath(await mkdtemp(join(tmpdir(), "knorvia-release-smoke-")));
...
try {
  ...
  await writeFile(
    runtimeCheck,
    'import { runTui } from "@knorvia/tui"; if (typeof runTui !== "function") throw new Error("Missing TUI export"); console.log("tui-runtime-ok");',
  );
  ...
  await until(
    () => /Knorvia/.test(screen) && /(?:登录|\/login|输入提示词|Type a prompt)/i.test(screen),
