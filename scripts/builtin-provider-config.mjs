...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.KNORVIA_ENV !== undefined) value = parsed.KNORVIA_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid KNORVIA_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.KNORVIA_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/builtin.json",
  );
  try {
    ...
    const { decodeKnorviaBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/builtin-release.ts")).href,
      import.meta.url,
    );
    decodeKnorviaBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "builtin.json"), config.content, "utf8");
  return config;
}
