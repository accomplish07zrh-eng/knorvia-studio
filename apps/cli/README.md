...
- `npm run build` produces `dist/knorvia.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
knorvia plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.knorvia-studio/cli/plugins`:
...
- `marketplaces/knorvia-plugins-bundled/`: bundled and CDN partitions plus the merged metadata for the single official marketplace.

This repository ships the built-in official plugins as asset packages under `packages/*-plugin`. Default-enabled plugins appear as `browser-use@knorvia-plugins-bundled`, `node-repl-host@knorvia-plugins-bundled`, `documents@knorvia-plugins-bundled`, `pdf@knorvia-plugins-bundled`, `presentations@knorvia-plugins-bundled`, `spreadsheets@knorvia-plugins-bundled`, `plugin-creator@knorvia-plugins-bundled`, and `skill-creator@knorvia-plugins-bundled`. `computer-use@knorvia-plugins-bundled` is discovered but stays disabled until the user enables it. See `specs/knorvia-builtin-plugins.md` for the asset rules and acceptance criteria.

```sh
knorvia plugins list
knorvia plugins enable computer-use
knorvia plugins disable browser-use
knorvia plugins disable computer-use
```
...
MCP config can live directly in `.knorvia-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current knorvia plugin surface:
...
Example `.knorvia-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${KNORVIA_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${KNORVIA_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${KNORVIA_PLUGIN_DATA}",
...
Only environment variables with the `KNORVIA_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

knorvia reads MCP servers from the main JSON config. The default user config path is `~/.knorvia-studio/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits knorvia's environment plus any `env` overrides.
...
knorvia reads hooks from the same main JSON config file as MCP, usually `~/.knorvia-studio/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
