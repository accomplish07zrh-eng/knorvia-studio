# @knorvia/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting cli prompt assembly.
...
```bash
pnpm --filter @knorvia/bootstrap^... build
pnpm --filter @knorvia/bootstrap build

pnpm --filter @knorvia/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/knorvia-prompt-trajectory/basic-live

pnpm --filter @knorvia/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @knorvia/prompt-trajectory derive -- \
  --out /tmp/knorvia-prompt-trajectory/basic-live

pnpm --filter @knorvia/prompt-trajectory model-io -- \
  --input ~/.knorvia-studio/cli/debug/model-io-<session>.jsonl \
  --out /tmp/knorvia-prompt-trajectory/model-io-session
```
...
uses the same knorvia model config resolution as the CLI. The upstream request is
