...
interface KnorviaBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createKnorviaApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface KnorviaContractsModule {
...
interface KnorviaProviderModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadKnorviaModules();
    ...
    const app = await bootstrap.createKnorviaApp({
...
async function loadKnorviaModules(): Promise<{
  bootstrap: KnorviaBootstrapModule;
  contracts: KnorviaContractsModule;
  provider: KnorviaProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<KnorviaBootstrapModule>,
      import(contractsPath) as Promise<KnorviaContractsModule>,
      import(providerConfigPath) as Promise<Omit<KnorviaProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<KnorviaProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built cli packages. Run `pnpm --filter @knorvia/bootstrap^... build && pnpm --filter @knorvia/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
