...
def inspect(path):
    names, roots, external = read_package(path, ".docx", ["[Content_Types].xml", "_rels/.rels", "word/document.xml"])
    root = roots["word/document.xml"]
    if root.tag != "{" + NS["w"] + "}document":
        raise ValueError("The main part is not a Word document")
    paragraphs = ["".join(p.itertext()) for p in root.findall(".//w:p", NS)]
    text = "\n".join(paragraphs)
    return {
        "ok": True, "format": "docx", "paragraphs": len(paragraphs),
        "tables": len(root.findall(".//w:tbl", NS)),
        "revisions": len(root.findall(".//w:ins", NS)) + len(root.findall(".//w:del", NS)),
        "commentsPresent": "word/comments.xml" in names,
        "media": sum(name.startswith("word/media/") for name in names),
        "externalRelationships": external, "text": text[:MAX_TEXT],
        "textTruncated": len(text) > MAX_TEXT, "visualVerification": "not-performed",
    }
