...
import type {
  ...
  ToolTraceStart,
} from "@knorvia/contracts/telemetry";
import {
  activeWriterContext,
  BaseSpanWriter,
  compactAttributes,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "knorvia.agent_turn.turn_number": integer(input.turnNumber),
            "knorvia.agent_turn.input_source": safeEnum(input.inputSource),
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "knorvia.agent_step.step_id": safeId(input.stepId),
            "knorvia.agent_step.step_index": integer(input.stepIndex),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "knorvia.execution.tool_call_id": safeId(input.toolCallId),
            "knorvia.tool_execution.tool_name": toolName,
            ...toolCompatibilityAttributes({
    ...
    return this.safeCreate(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent?.correlation),
            "knorvia.context_compaction.trigger": safeEnum(input.trigger),
            "knorvia.context_compaction.phase": safeEnum(input.phase),
            "knorvia.context_compaction.model_mode": safeEnum(input.modelMode),
            "knorvia.context_compaction.outer_attempt": integer(input.outerAttempt),
            "knorvia.context_compaction.max_attempts": integer(input.maxAttempts),
            "knorvia.context_compaction.triggering_step_index": integer(input.triggeringStepIndex),
            "knorvia.context_compaction.policy_context_window_tokens": finiteNonNegative(
              input.policyContextWindowTokens,
            ),
            "knorvia.context_compaction.threshold_tokens": finiteNonNegative(input.thresholdTokens),
            "knorvia.context_compaction.token_source": safeEnum(input.tokenSource),
            "knorvia.context_compaction.recovered_from_logical_call_id": safeId(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "knorvia.detached_operation.operation": safeEnum(input.operation),
            "knorvia.detached_operation.execution_kind": safeEnum(input.executionKind),
            "knorvia.detached_operation.trigger": safeEnum(input.trigger),
            "knorvia.detached_operation.target_kind": safeEnum(input.targetKind),
            "knorvia.detached_operation.goal_iteration": integer(input.goalIteration),
            "knorvia.detached_operation.chunk_index": integer(input.chunkIndex),
            "knorvia.detached_operation.chunk_count": integer(input.chunkCount),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "knorvia.execution.logical_call_id": safeId(input.logicalCallId),
            "knorvia.model_call.operation": safeEnum(input.operation),
            "knorvia.model_call.streaming": input.streaming,
            ...
            "knorvia.model_call.reasoning_requested_control": safeEnum(
              input.requested.reasoning.requestedControl,
            ),
            "knorvia.model_call.reasoning_requested_level": safeString(
              input.requested.reasoning.requestedLevel,
              128,
            ),
            "knorvia.model_call.reasoning_requested_budget_tokens": integer(
              input.requested.reasoning.requestedBudgetTokens,
            ),
            "knorvia.model_call.call_cause": safeEnum(input.callCause),
            "knorvia.model_call.previous_logical_call_id": safeId(input.previousLogicalCallId),
          }),
          context: parentContext,
          correlation: parent?.correlation,
          parent,
    ...
    return this.safeCreate(
      ...
      () => {
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent.correlation),
            "knorvia.execution.tool_call_id": safeId(parent.toolCallId),
            "knorvia.command_execution.safe_name": safeString(input.safeName, 128),
            "knorvia.command_execution.category": safeEnum(input.category),
            "knorvia.command_execution.command_count": integer(input.commandCount),
            "knorvia.command_execution.shell_kind": safeEnum(input.shellKind),
            "knorvia.command_execution.sandboxed": input.sandboxed,
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "knorvia.execution.tool_call_id": safeId(parent.toolCallId),
            "knorvia.execution.logical_call_id": parentWriter.logicalCallId,
            "knorvia.execution.model_operation": parentWriter.operation,
            "knorvia.execution.model_role": parentWriter.modelRole,
            "knorvia.model_attempt.request_id": safeId(input.requestId),
            "knorvia.model_attempt.attempt_number": integer(input.attemptNumber),
            "knorvia.model_attempt.max_attempts": integer(input.maxAttempts),
            "knorvia.model_attempt.attempt_cause": safeEnum(input.attemptCause),
            "knorvia.model_attempt.previous_request_id": safeId(input.previousRequestId),
            "knorvia.model_attempt.retry_delay_ms": finiteNonNegative(input.retryDelayMs),
            "knorvia.model_attempt.provider_id": safeString(target.providerId, 128),
            "knorvia.model_attempt.provider_kind": safeEnum(target.providerKind),
            "knorvia.model_attempt.provider_origin": safeString(target.providerOrigin),
            "knorvia.model_attempt.provider_route": safeString(target.providerRoute),
            "knorvia.model_attempt.requested_model": safeString(target.requestedModel, 128),
            "knorvia.model_attempt.transport": safeEnum(input.transport),
            "knorvia.model_attempt.api_operation": safeEnum(input.apiOperation),
            "knorvia.model_attempt.reasoning_capability": safeEnum(target.reasoning.capability),
            "knorvia.model_attempt.reasoning_requested_state": safeEnum(
              target.reasoning.requestedState,
            ),
            "knorvia.model_attempt.reasoning_requested_control": safeEnum(
              target.reasoning.requestedControl,
            ),
            "knorvia.model_attempt.reasoning_requested_level": safeString(
              target.reasoning.requestedLevel,
              128,
            ),
            "knorvia.model_attempt.reasoning_requested_budget_tokens": integer(
...
function causationLink(
  ...
  return {
    attributes: {
      "knorvia.link.relation": relation,
    },
...
class StepWriter extends TrackedBaseWriter implements AgentStepSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() =>
      this.setAttribute("knorvia.agent_step.terminal_reason", terminalReason),
    );
  }
...
class ToolWriter extends TrackedBaseWriter implements ToolExecutionSpanWriter {
  ...
  setPermissionDecision(decision: "granted" | "denied" | "not_required"): void {
    this.setAttribute("knorvia.tool_execution.permission_decision", decision);
  ...
  setOutputBytes(bytes: number): void {
    this.setAttribute("knorvia.tool_execution.output_bytes", finiteNonNegative(bytes));
  }

  setOutputTruncated(truncated: boolean): void {
    this.setAttribute("knorvia.tool_execution.output_truncated", truncated);
  }
  ...
  finishDenied(reason: "user_denied" | "policy_denied" | "unavailable" | "unknown"): void {
    this.finishDomainOutcomeIfOpen("denied", () =>
      this.setAttribute("knorvia.tool_execution.permission_denial_reason", reason),
    );
  }
...
class CommandWriter extends TrackedBaseWriter implements CommandExecutionSpanWriter {
  ...
  markFirstOutput(): void {
    ...
    this.setAttribute("knorvia.command_execution.first_output_ms", elapsed);
  ...
  setExitCode(exitCode: number): void {
    ...
    this.setAttribute("knorvia.command_execution.exit_code", normalized);
  ...
  setSignal(signal: string): void {
    ...
    this.setAttribute("knorvia.command_execution.signal", normalized);
    this.setAttributes(commandCompatibilityAttributes({ signal: normalized }));
  }

  setOutputBytes(bytes: number): void {
    this.setAttribute("knorvia.command_execution.output_bytes", finiteNonNegative(bytes));
  }

  setTimedOut(timedOut: boolean): void {
    this.setAttribute("knorvia.command_execution.timed_out", timedOut);
  }
...
class CompactionWriter extends TrackedBaseWriter implements ContextCompactionSpanWriter {
  ...
  setInputTokens(tokens: number): void {
    this.setAttribute("knorvia.context_compaction.input_tokens", finiteNonNegative(tokens));
  }

  setOutputTokens(tokens: number): void {
    this.setAttribute("knorvia.context_compaction.output_tokens", finiteNonNegative(tokens));
  }
...
class DetachedWriter extends TrackedBaseWriter implements DetachedOperationSpanWriter {
  ...
  setResultType(resultType: "text" | "boolean" | "metadata" | "other"): void {
    this.setAttribute("knorvia.detached_operation.result_type", resultType);
  }
...
class ModelAttemptWriter extends TrackedBaseWriter implements ModelAttemptSpanWriter {
  ...
  setProviderRequestId(requestId: string): void {
    this.setAttribute("knorvia.model_attempt.provider_request_id", safeString(requestId, 256));
  }

  setResponseModel(model: ResponseModelTelemetryDescriptor): void {
    const value = safeString(model.model, 128);
    this.setAttribute("knorvia.model_attempt.response_model", value);
  ...
  setEffectiveReasoningState(state: ModelReasoningState): void {
    this.setAttribute("knorvia.model_attempt.reasoning_effective_state", safeEnum(state));
  }

  setEffectiveReasoningControl(control: ModelReasoningControlType): void {
    this.setAttribute("knorvia.model_attempt.reasoning_effective_control", safeEnum(control));
  }

  setEffectiveReasoningLevel(level: string): void {
    this.setAttribute("knorvia.model_attempt.reasoning_effective_level", safeString(level, 128));
  }

  setEffectiveReasoningBudgetTokens(tokens: number): void {
    this.setAttribute(
      "knorvia.model_attempt.reasoning_effective_budget_tokens",
  ...
  setFinishReason(reason: ModelFinishReason): void {
    ...
    this.setAttribute("knorvia.model_attempt.finish_reason", value);
    this.setAttributes(modelResponseCompatibilityAttributes({ finishReason: value }));
  }

  setInputTokens(tokens: number): void {
    this.setUsage("inputTokens", "input", tokens, "knorvia.model_attempt.input_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ inputTokens: tokens }));
  }

  setOutputTokens(tokens: number): void {
    this.setUsage("outputTokens", "output", tokens, "knorvia.model_attempt.output_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ outputTokens: tokens }));
  }

  setReasoningTokens(tokens: number): void {
    this.setUsage("reasoningTokens", "reasoning", tokens, "knorvia.model_attempt.reasoning_tokens");
  }
  ...
  setCacheWriteTokens(tokens: number): void {
    this.setUsage(
      ...
      "cache_write",
      tokens,
      "knorvia.model_attempt.cache_write_tokens",
    );
  }

  setStreamOutputCommitted(committed: boolean): void {
    this.setAttribute("knorvia.model_attempt.stream_output_committed", committed);
  }

  setHttpStatusCode(statusCode: number): void {
    const normalized = integer(statusCode);
    this.setAttribute("knorvia.model_attempt.http_status_code", normalized);
    this.setAttributes(httpResponseCompatibilityAttributes(statusCode));
  }

  setProviderErrorCode(code: string): void {
    this.setAttribute("knorvia.model_attempt.provider_error_code", safeString(code, 128));
  }
  ...
  setRetryAfterMs(delayMs: number): void {
    this.setAttribute("knorvia.model_attempt.retry_after_ms", finiteNonNegative(delayMs));
  }
  ...
  markFirstProviderEvent(): void {
    ...
    this.setAttribute("knorvia.model_attempt.time_to_first_provider_event_ms", elapsed);
  ...
  markFirstContent(): void {
    ...
    this.setAttribute("knorvia.model_attempt.time_to_first_content_ms", elapsed);
  ...
  markFirstText(): void {
    ...
    this.setAttribute("knorvia.model_attempt.time_to_first_text_ms", elapsed);
