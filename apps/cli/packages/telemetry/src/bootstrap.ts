...
import type { ModelStatusSink } from "@knorvia/contracts/model";
...
export async function prepareModelTelemetryEnv(
  ...
  // 默认不采集；仅用户显式配置 Knorvia 遥测开关与自己的 OTLP 接收端后启用。
  if (env.KNORVIA_MODEL_TELEMETRY_ENABLED !== "1" || !resolveOtlpTraceEndpoint(env)) {
    return env;
  }
...
async function createPreparedOwner(
  ...
  const resource: TelemetryResourceContext = {
    buildCommitId: normalizeBuildCommitId(options.buildCommitId ?? env.KNORVIA_BUILD_COMMIT_ID),
    cliVersion: normalizeVersion(options.cliVersion),
    deploymentEnvironment:
      env.KNORVIA_ENV ??
      env.KNORVIA_RUNTIME_ENV ??
      resourceAttributesFromEnv(env)["deployment.environment.name"],
    installationId: normalizeTelemetryDeviceMid(env.KNORVIA_TELEMETRY_DEVICE_MID),
    productVersion: normalizeVersion(options.productVersion ?? env.KNORVIA_APP_VERSION),
    runtimeDistribution:
      options.runtimeDistribution ??
      resolveRuntimeDistribution(env.KNORVIA_TELEMETRY_RUNTIME_DISTRIBUTION),
    runtimeSurface:
      options.runtimeSurface ?? resolveRuntimeSurface(env.KNORVIA_TELEMETRY_RUNTIME_SURFACE),
    serviceInstanceId: randomUUID(),
    serviceName: env.OTEL_SERVICE_NAME?.trim() || "cli-agent",
  };
  try {
...
function resolveTelemetryIdentity(env: EnvRecord): TelemetryIdentitySnapshot {
  const userSubjectId = normalizeTelemetryId(env.KNORVIA_TELEMETRY_USER_SUBJECT_ID);
  const configuredState = env.KNORVIA_TELEMETRY_IDENTITY_STATE?.trim();
  const identityState =
    ...
    configuredState === "unknown"
      ...
      : userSubjectId
        ? "authenticated"
        : normalizeTelemetryDeviceMid(env.KNORVIA_TELEMETRY_DEVICE_MID)
          ? "anonymous"
          : "unknown";
...
async function resolveStandaloneDeviceMid(
  knorviaHome: string | undefined,
): Promise<string | undefined> {
  const stateFile = knorviaHome
    ? join(knorviaHome, "v2", "telemetry-state.json")
    : join(resolveKnorviaDataRoot(), "v2", "telemetry-state.json");
