...
const traceHeaderNames = ["x-knorvia-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          KNORVIA_HTTP_PROXY: url,
          KNORVIA_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.knorviaCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.KNORVIA_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.KNORVIA_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.KNORVIA_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.KNORVIA_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.KNORVIA_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
