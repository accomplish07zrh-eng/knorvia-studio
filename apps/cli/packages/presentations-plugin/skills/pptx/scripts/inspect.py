...
def inspect(path):
    names, roots, external = read_package(path, ".pptx", ["[Content_Types].xml", "_rels/.rels", "ppt/presentation.xml"])
    root = roots["ppt/presentation.xml"]
    if root.tag != "{" + NS["p"] + "}presentation":
        raise ValueError("The main part is not a presentation")
    slides = []
    remaining = MAX_TEXT
    for name, slide in sorted(roots.items()):
        if not name.startswith("ppt/slides/slide") or not name.endswith(".xml"):
            continue
        text = "\n".join(node.text or "" for node in slide.findall(".//a:t", NS))
        slides.append({"part": name, "text": text[:remaining], "textTruncated": len(text) > remaining})
        remaining = max(0, remaining - len(text))
    return {
        "ok": True, "format": "pptx", "slideCount": len(root.findall(".//p:sldId", NS)),
        "slidesByPart": slides, "externalRelationships": external,
        "notes": sum(name.startswith("ppt/notesSlides/notesSlide") and name.endswith(".xml") for name in names),
        "media": sum(name.startswith("ppt/media/") for name in names),
