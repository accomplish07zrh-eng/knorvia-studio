...
export async function setupBrowserRuntime({ globals }: { globals: Record<PropertyKey, unknown> }): Promise<void> {
  const connection = readNodeReplBrowserRuntimeBridge(globals);
  const assertAvailable = () => connection.assertAvailable();
  assertAvailable();
  attachBrowserClient({
    globals, documentationRoot: connection.documentationRoot, assertAvailable,
    transport: {
      list: () => { assertAvailable(); return connection.list(); },
      execute: (id, generation, command) => { assertAvailable(); return connection.execute(id, generation, command); },
    },
  });
}
