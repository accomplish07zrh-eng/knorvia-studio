...
import type { TuiCopy } from "@knorvia/i18n";
...
export function ModelConfigurationRequiredPanel({
  copy = DEFAULT_TUI_COPY,
}: {
  copy?: TuiCopy;
} = {}): React.ReactElement {
  return h(
    "box",
    {
      title: copy.modelConfigurationRequired.title,
      style: {
        backgroundColor: palette.panel,
        border: true,
    ...
    h("text", { style: { fg: palette.warning } }, copy.modelConfigurationRequired.message),
    h("text", { style: { fg: palette.muted } }, copy.modelConfigurationRequired.help),
  );
}
