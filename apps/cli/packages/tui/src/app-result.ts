import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@knorvia/contracts";
...
type TuiApplyResultInput = {
  ...
  setModelConfigurationRequired: (modelConfigurationRequired: boolean) => void;
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getKnorviaCopy(result.locale ?? input.fallback.locale).tui;
      ...
      input.setStatus(
        result.selection
          ...
          : activeModelConfigurationRequired
            ? resultCopy.modelConfigurationRequired.status
