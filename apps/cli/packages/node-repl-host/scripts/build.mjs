...
export async function buildNodeReplHostBundle({
  ...
  await build({ entryPoints: [resolve(import.meta.dirname, "../src/server.ts")], outfile,
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) await buildNodeReplHostBundle();
