import { KNORVIA_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY as SCREENSHOTS, KNORVIA_MCP_NODE_REPL_CUA_APP_META_KEY as APP } from "@knorvia/contracts/mcp";
...
function embedded(text: string | undefined): NodeReplStructuredResult | undefined {
  try {
    ...
    if (Array.isArray(value.content) && value.content.every((block) => block && typeof block.type === "string")) return value;
  } catch { /* 普通返回值保留为文本，不把解析失败当作执行错误。 */ }
}
...
export function toMcpRunResult(run: NodeReplRunResult): CallToolResult {
  ...
  const meta: Record<string, unknown> = Object.assign({}, run.responseMeta, ...results.map((item) => item._meta));
  // 代码可写的元数据不能决定截图来源或目标应用，只有宿主的专用记录通道可信。
  for (const key of [SCREENSHOTS, APP, CUA_APP_ASSOCIATIONS_META_KEY, "knorvia/browserScreenshotContentIndices", "knorvia/nodeReplCuaApp", "knorvia.cua/app-associations-v1"]) delete meta[key];
  ...
  if (!run.error) {
    ...
    for (let i = 0; i + 1 < blocks.length; i++) {
      ...
      if (blocks[i]?.type === "image" && authority?.type === "text" && isOfficialCuaImageRefText(authority.text)) latestFrame = i;
    }
    const kept = blocks.filter((block, index) => {
      ...
      if (index <= latestFrame && block.type === "text" && isOfficialCuaImageRefText(block.text) && blocks[index - 1]?.type === "image") return false;
      return true;
    });
    if (structured.length) content.push(...kept);
    for (const [index, picture] of (run.images ?? []).entries()) {
      const duplicate = structured.length ? content.findIndex((block) => block.type === "image" && block.data === picture.base64 && block.mimeType === picture.mimeType) : -1;
      imageLocations.set(index, duplicate < 0 ? content.length : duplicate);
      if (duplicate < 0) content.push({ type: "image", data: picture.base64, mimeType: picture.mimeType });
    }
    if (fallback) content.push(...kept);
    const text = [run.logs, results.length === 0 && run.result !== undefined ? `=> ${run.result}` : ""].filter(Boolean).join("\n");
    if (text) content.push({ type: "text", text });
  } else content.push({ type: "text", text: run.error.message });
  const indices = [...new Set((run.browserScreenshotImageIndices ?? []).flatMap((index) => {
  ...
  const structuredContent = results.findLast((item) => item.structuredContent !== undefined)?.structuredContent;
  return {
