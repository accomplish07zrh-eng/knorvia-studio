...
import { NodeReplSession, type NodeReplRequestMeta, type NodeReplRunResult } from "@knorvia/core/repl";
...
export interface NodeReplExecuteInput {
  code: string; requestMeta: NodeReplRequestMeta; signal: AbortSignal; syncTimeoutMs: number; cuaBroker?: NodeReplCuaBrokerConnection;
}
...
export function createInProcessNodeReplExecutor(): NodeReplExecutor {
  return async (input) => {
    ...
    let active: ActiveNodeReplCall | undefined = { generation: 1, requestMeta: input.requestMeta, signal: input.signal };
    const base = process.env.KNORVIA_PLUGIN_ROOT ?? process.cwd();
    const session: NodeReplSession = new NodeReplSession({ restrictProcess: true, injectedGlobals: () => {
      ...
      return {
        ...
        ...createComputerUseBridgeGlobals({ ...context, broker: input.cuaBroker, documentationRoot: resolve(process.env.KNORVIA_CUA_PLUGIN_ROOT ?? base, "docs") }),
      };
    } });
    try { return await session.run(input.code, { requestMeta: input.requestMeta, signal: input.signal, syncTimeoutMs: input.syncTimeoutMs }); }
    finally { active = undefined; session.dispose(); }
  };
}

export function executeInWorker(moduleUrl: string, input: NodeReplExecuteInput): Promise<NodeReplRunResult> {
  input.signal.throwIfAborted();
