...
export function createNodeReplCuaBroker(input: {
  ...
  const socketPath = windows
    ? `\\\\.\\pipe\\knorvia-repl-${randomUUID()}`
    : join(tmpdir(), `knorvia-${randomUUID()}.sock`);
  ...
  const server = createServer((socket) => {
    ...
    void (async () => {
      ...
      try {
        ...
        id = frame.id;
        if (typeof id !== "string" || typeof frame.method !== "string")
        ...
        const workspaceKey =
          textField(context, "workspaceKey") ??
          textField(context, "workspaceIdentity") ??
          textField(context, "workspacePath");
        ...
        const result = await input.runtime.execute({
          toolName: frame.method as never,
          arguments: frame.input,
          context: { ...context, workspaceKey } as unknown as ComputerUseRuntimeContext,
          signal: abort.signal,
        });
