...
export interface NodeReplCuaBrokerConnection { socketPath: string; token: string }
...
export function createComputerUseBridgeGlobals(input: {
  broker?: NodeReplCuaBrokerConnection; generation: number; getActiveCall(): ActiveCuaNodeReplCall | undefined;
  session(): NodeReplSession; documentationRoot: string;
  ...
  const bridge: ComputerUseRuntimeBridge = {
    ...
    call: async (method, payload) => {
      ...
      const frame = await brokerCall(broker, { method, input: payload, context: callContext(call.requestMeta, true) }, call.signal);
      activeCall(input, "Computer Use");
      const result = frame.result as CallToolResult | undefined;
      if (!result || !Array.isArray(result.content)) throw new Error("Computer Use broker returned no result");
      if (frame.responseMeta && typeof frame.responseMeta === "object") input.session().mergeResponseMeta(frame.responseMeta as Record<string, unknown>);
      const associations = result._meta?.[CUA_APP_ASSOCIATIONS_META_KEY] as { primary?: Record<string, unknown> } | undefined;
      ...
      if (primary && typeof primary === "object") {
        ...
        if (appKey) input.session().recordCuaAppIdentity({ appKey, ...(displayName ? { displayName } : {}) });
      }
      return result;
    },
  };
