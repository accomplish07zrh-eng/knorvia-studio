...
export function activeCall(
  input: { generation: number; getActiveCall(): ActiveNodeReplCall | undefined },
  domain: string,
): ActiveNodeReplCall {
  const call = input.getActiveCall();
  if (!call || call.generation !== input.generation)
    throw new Error(`${domain} runtime binding is stale after kernel reset`);
  call.signal.throwIfAborted();
  if (call.requestMeta.runtime_scope === "subagent")
    throw new Error(
      domain === "Browser"
        ? BROWSER_UNAVAILABLE_IN_SUBAGENT_MESSAGE
...
export function createBrowserBridgeGlobals(input: {
  ...
  const request = async (payload: Record<string, unknown>) => {
    ...
    const response = await brokerCall(
      { socketPath, token },
      { ...payload, ...callContext(current.requestMeta) },
      current.signal,
    );
...
function recordBrowserResult(
  ...
  session.mergeResponseMeta({
    ...
    "knorvia/toolSurface": {
      ...
      ...(final || (result.ok && mutation) ? { openTabIds: meta.openTabIds } : {}),
      ...(final ? { sessionEnded: true } : {}),
    },
