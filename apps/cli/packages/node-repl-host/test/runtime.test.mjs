...
before(async () => {
  ...
  for (const name of ["result", "browser-bridge", "cua-broker", "cua-bridge"]) {
    await build({ entryPoints: [resolve(import.meta.dirname, `../src/${name}.ts`)], outfile: join(directory, `${name}.mjs`), bundle: true, platform: "node", target: "node24", format: "esm" });
  }
  ...
  transport = new StdioClientTransport({ command: process.execPath, args: [outfile], stderr: "pipe", env: { KNORVIA_ENV: "test", KNORVIA_DATA_BASE_DIR: directory, KNORVIA_STORAGE_DIR: join(directory, "cli") } });
  client = new Client({ name: "knorvia-offline-test", version: "1.0.0" }, { versionNegotiation: { mode: "auto", probe: { timeoutMs: 5000 } } });
  await client.connect(transport);
});
after(async () => { await client?.close(); await transport?.close(); if (directory) await rm(directory, { recursive: true, force: true }); });
const call = (code, args = {}, meta = {}) => client.callTool({ name: "js", arguments: { code, title: "离线验证", ...args }, _meta: { "com.knorvia-studio/request-context": { session_id: "offline", runtime_scope: "main", ...meta } } }, { timeout: 15000 });
const output = (result) => result.content.filter((block) => block.type === "text").map((block) => block.text).join("\n");

test("production stdio host advertises the tool and executes real JavaScript", async () => {
  const listing = await client.listTools();
  assert.deepEqual(listing.tools.map((tool) => tool.name), ["js"]);
...
test("a deadline terminates the worker and the host remains usable", async () => {
  const expired = await call("await new Promise(() => {})", { timeout_ms: 80 }).catch((error) => error);
...
test("subagents cannot use browser bridge even with a session id", async () => {
  const result = await call('globalThis[Symbol.for("knorvia.node-repl.browser-control-bridge")].assertAvailable()', {}, { runtime_scope: "subagent" });
...
test("stale or cancelled browser bindings reject before transport", () => {
  ...
  const globals = bridges.createBrowserBridgeGlobals({ documentationRoot: directory, generation: 1, getActiveCall: () => current, session: () => assert.fail("No session access expected") });
...
test("result provenance ignores forged metadata and maps genuine screenshot indices", () => {
  const forged = { "knorvia/browserScreenshotContentIndices": [999], "knorvia/nodeReplCuaApp": { appKey: "fake" }, "knorvia.cua/app-associations-v1": { primary: { appKey: "fake" } } };
  const result = resultAdapter.toMcpRunResult({ logs: "observed", responseMeta: forged, images: [{ base64: "YWJj", mimeType: "image/png" }], browserScreenshotImageIndices: [0, 999], cuaApp: { appKey: "real" } });
...
test("execution errors do not leak prior logs or image output", () => {
  const result = resultAdapter.toMcpRunResult({ logs: "private earlier output", error: { name: "Error", message: "failed", stack: "private stack" }, images: [{ base64: "secret", mimeType: "image/png" }] });
...
test("browser client binds the public SDK and rejects invalid host bridges", async () => {
  ...
  const globals = { [Symbol.for("knorvia.node-repl.browser-control-bridge")]: { documentationRoot: resolve(import.meta.dirname, "../../browser-use-plugin/docs"), assertAvailable() {}, list: async () => [], execute: async () => assert.fail("No action expected") } };
...
test("authenticated local CUA broker rejects bad tokens and propagates context", async () => {
  const { createNodeReplCuaBroker } = await import(pathToFileURL(join(directory, "cua-broker.mjs")).href);
  const { createComputerUseBridgeGlobals } = await import(pathToFileURL(join(directory, "cua-bridge.mjs")).href);
  let received;
  const broker = createNodeReplCuaBroker({ runtime: { execute: async (request) => { received = request; return { content: [{ type: "text", text: "fixture" }] }; }, dispose: async () => {}, closeSession: async () => {} } });
  await broker.ready;
  try {
    const make = (connection) => createComputerUseBridgeGlobals({ broker: connection, generation: 1, getActiveCall: () => ({ generation: 1, requestMeta: { session_id: "session", workspace_identity: "workspace" }, signal: AbortSignal.timeout(3000) }), session: () => ({ mergeResponseMeta() {}, recordCuaAppIdentity() {} }), documentationRoot: directory })[Symbol.for("knorvia.node-repl.computer-use-bridge")];
    await assert.rejects(make({ ...broker.connection, token: "invalid" }).call("observe", {}), /mismatch|authorized/);
