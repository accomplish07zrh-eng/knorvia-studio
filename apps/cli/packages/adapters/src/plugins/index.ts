...
function materializeCommandMetadataRoot(
  ...
  for (const [rawName, rawMetadata] of Object.entries(spec)) {
    ...
    // Knorvia 的 custom command loader 只扫描 markdown 根目录，因此把低风险命令内容
...
function scanOfficialCache(
  ...
  const cacheRoot = join(storageRoot, "cache", KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE);
  try {
    const roots: string[] = [];
...
function findManifest(rootPath: string): string | null {
  const knorviaPath = join(rootPath, KNORVIA_MANIFEST_PATH);
  if (fileExists(knorviaPath)) {
    return knorviaPath;
  }
