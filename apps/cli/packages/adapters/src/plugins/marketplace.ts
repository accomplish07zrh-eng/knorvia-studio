...
import type { PluginDiagnostic, PluginManifest, PluginStoreListing } from "@knorvia/contracts";
import { isOfficialMarketplaceId, KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE } from "@knorvia/contracts";
import { DEFAULT_PLUGIN_MARKETPLACES, sanitizeKnorviaRuntimeEnv } from "@knorvia/shared";
...
const KNORVIA_MANIFEST_PATH = join(".knorvia-plugin", "plugin.json");
...
function buildMarketplaceGitEnv(
  ...
  const env = sanitizeKnorviaRuntimeEnv(sourceEnv);
  // marketplace 安装会启动 Git 子进程，不能只依赖父进程继承的 shell 代理。
  // 这里统一从 Knorvia 显式网络环境恢复 HTTP(S)/NO_PROXY/CA，避免安装按钮卡到协议超时。
...
export async function addMarketplace(input: {
  ...
  try {
    ...
    if (
      input.trustedId === KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE &&
      loaded.manifest.name !== KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE
    ) {
      throw new Error(
        `Official marketplace source must provide ${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}, received ${loaded.manifest.name}`,
      );
    }
    ...
    } else if (loaded.manifest.name !== KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE) {
...
async function clonePluginSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "knorvia-plugin-src-"));
  const args = ["clone"];
...
async function cloneMarketplaceSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "knorvia-marketplace-src-"));
...
async function execGitCommand(args: string[], signal?: AbortSignal): Promise<void> {
  ...
  try {
    ...
    const gitBinary = process.env.KNORVIA_GIT_BINARY?.trim() || "git";
...
function findPluginManifestPath(rootPath: string): string | null {
  for (const candidate of [KNORVIA_MANIFEST_PATH, CLAUDE_MANIFEST_PATH, CODEX_MANIFEST_PATH]) {
  ...
  for (const key of UNSUPPORTED_MANIFEST_FIELDS) {
    if (key in input.manifest) {
      input.diagnostics.push({
        ...
        message: `Plugin component is diagnostic-only in this Knorvia Studio runtime: ${key}`,
