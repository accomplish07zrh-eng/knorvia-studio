...
// 内存 event store 实现已下沉到 @knorvia/contracts，
// 这里保持 `@knorvia/adapters/storage` 的导出路径不变，避免调用方改 import。
export {
...
export class NodeToolArtifactStore implements ToolArtifactStorePort {
  ...
  async writeToolResultArtifact(
    ...
    return {
      id: artifactId,
      uri: createArtifactUri(request.sessionId, artifactId),
      path,
...
function parseArtifactUri(uri: string): { artifactId: string; sessionId: string } {
  ...
  if (parsed.protocol !== "knorvia-artifact:" && parsed.protocol !== "knorvia-artifact:") {
