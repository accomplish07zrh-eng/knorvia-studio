...
export async function resolveWorkspaceHookTrustStorePath(
  ...
  const userConfigPath = resolve(options.userConfigPath ?? join(privateRoot, "cli", "config.json"));
  const config = await readUserConfig(userConfigPath);
  const storage = isRecord(config.storage) ? config.storage : {};
  const configured = process.env.KNORVIA_PORTABLE_DIR
    ? ""
    : typeof storage.dir === "string"
      ? storage.dir.trim()
      : "";
  const storageRoot = configured ? resolveTrustedUserPath(configured, home) : privateRoot;
