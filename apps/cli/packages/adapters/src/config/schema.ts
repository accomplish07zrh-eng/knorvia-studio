/* eslint-disable max-lines -- cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
...
export const LEGACY_CUA_PLUGIN_ID = "cua@knorvia-plugins-bundled";
export const CANONICAL_CUA_PLUGIN_ID = "computer-use@knorvia-plugins-bundled";
...
export const KnorviaConfigFileSchema = z
  .object({
...
export type KnorviaConfigFile = z.infer<typeof KnorviaConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. Knorvia does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；Knorvia 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = KnorviaConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: KnorviaConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: KnorviaConfigFile["plugins"],
...
function parseSkillsRuntimeConfig(
  skills: KnorviaConfigFile["skills"],
