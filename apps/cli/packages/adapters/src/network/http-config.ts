...
function resolveProxyForRequestInternal(
  ...
  const candidates = [[`env:${KNORVIA_HTTP_PROXY_ENV_KEY}`, env[KNORVIA_HTTP_PROXY_ENV_KEY]]];
  ...
  if (capturedProxy) {
    return {
      noProxyMatched: false,
      proxySource: `env:${KNORVIA_TOOL_ENV_PASSTHROUGH_ENV_KEY}.${capturedProxy.key}`,
...
export function resolveTlsCaCertFile(options: NetworkTlsOptions): string | undefined {
  const candidates = [options.caCertFile, options.env?.[KNORVIA_AGENT_CA_CERT_ENV_KEY]];
...
function readExplicitNoProxyValue(options: NetworkProxyOptions): string | undefined {
  return (
    normalizePathLike(options.noProxy) ?? normalizePathLike(options.env?.[KNORVIA_NO_PROXY_ENV_KEY])
  );
}
