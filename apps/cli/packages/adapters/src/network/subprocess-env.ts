...
const EXPLICIT_CA_SOURCE_KEYS = [KNORVIA_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [KNORVIA_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, KNORVIA_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readKnorviaToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const knorviaProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [KNORVIA_HTTP_PROXY_ENV_KEY], platform),
  );
  if (knorviaProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, knorviaProxy, platform);
    }
    return;
  }
}
