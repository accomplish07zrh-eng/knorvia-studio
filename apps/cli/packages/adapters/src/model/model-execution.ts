...
import { withOpenRouterAttributionHeaders } from "@knorvia/shared";
...
export function createProviderBusinessErrorFetch(
  ...
  return async (input, init) => {
    ...
    // knorvia-plan 安全校验拒绝（3007）等场景返回 HTTP 403 + JSON，但未必带
...
/** 从 HTTP JSON body 解析 knorvia-plan 等业务错误（供 failure-classifier 在 APICallError 路径复用）。 */
export function readProviderBusinessFailureFromBody(body: unknown):
  ...
  const providerCode =
    ...
    // 二次包装后的外层 code 是 Knorvia 自己的 PROVIDER_BUSINESS_ERROR，
