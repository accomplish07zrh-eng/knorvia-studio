...
interface ResolveKnorviaBuiltinPromptCommandOptions {
  workingDirectory?: string;
}

export function resolveKnorviaBuiltinPromptCommand(
  input: string,
  options: ResolveKnorviaBuiltinPromptCommandOptions = {},
): string | undefined {
...
function buildInitAgentsPrompt(params: {
  ...
  return [
    "You are running Knorvia Studio's built-in /init command.",
    "",
    "Your task is to create or update a concise workspace instruction file for future Knorvia Studio agents.",
    "",
    "Target:",
    `- Workspace directory: ${params.workingDirectory}`,
    `- Instruction file: ${params.targetPath}`,
    `- Existing hidden instruction candidates: ${join(params.workingDirectory, ".knorvia-studio", "AGENTS.md")} and ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.knorvia-studio/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .knorvia-studio/AGENTS.md or .agents/AGENTS.md exists in the workspace. If either exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
    ...
    "6. Include only project-specific facts future Knorvia Studio agents would otherwise miss.",
