...
import { createNodeCustomCommandAdapter } from "@knorvia/adapters/commands";
import type {
...
import { resolveKnorviaPlugins } from "./plugins.js";
import { collectDisabledPaths } from "./skill-command-overrides.js";

export interface ListKnorviaCustomCommandsOptions {
...
export interface InspectKnorviaCustomCommandOptions extends ListKnorviaCustomCommandsOptions {
  name: string;
}

export interface KnorviaCustomCommandInspection {
  command: CustomCommandContent;
  diagnostics: CustomCommandDiagnostic[];
}

export async function listKnorviaCustomCommands(
...
export async function inspectKnorviaCustomCommand(
  options: InspectKnorviaCustomCommandOptions,
): Promise<KnorviaCustomCommandInspection> {
  const discovery = createCustomCommandDiscovery(options);
...
export async function loadKnorviaCustomCommand(
  options: InspectKnorviaCustomCommandOptions,
...
function createCustomCommandDiscovery(options: ListKnorviaCustomCommandsOptions) {
  ...
  const pluginOutcome = resolveKnorviaPlugins({
    configResult,
    env: options.env,
