...
import {
  ...
  type KnorviaActiveToolCall,
  type KnorviaContextUsageBreakdownItem,
  type KnorviaDeliveryKind,
  type KnorviaPendingPermission,
  type KnorviaSessionContextUsage,
  type KnorviaSessionEvent,
  type KnorviaSessionGoal,
  type KnorviaSessionGoalVerification,
  type KnorviaSessionGoalVerificationTimeline,
  type KnorviaSessionGoalStats,
  type KnorviaSessionInfo,
  type KnorviaSessionKind,
  type KnorviaSessionProjection,
  type KnorviaSessionRuntimeState,
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: KNORVIA_PROTOCOL_NAME,
      version: KNORVIA_PROTOCOL_VERSION,
    },
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 artifact URI 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return isArtifactUri(artifactUri) ? artifactUri : undefined;
}
...
): Promise<KnorviaSessionSettingsState> {
...
export function mapSessionInfo(input: {
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as KnorviaSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is KnorviaSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = knorviaApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const knorviaMeta = asRecord(meta.knorvia);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = knorviaApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): KnorviaSessionProjection {
  return {
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): KnorviaSessionContextUsage | undefined {
...
): KnorviaSessionGoalVerification[] {
...
): KnorviaSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: KnorviaSessionContextUsage["cache"] | undefined,
): KnorviaSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): KnorviaSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): KnorviaPendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 knorviaPendingPermissionSchema（shared/src/protocol/index.ts:1139）和
  // knorviaPermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 knorviaPermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): KnorviaActiveToolCall {
  return {
...
function ensureTodoGroup(
  groups: Map<string, KnorviaSessionTodoGroup>,
  input: {
  ...
  const group: KnorviaSessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): KnorviaSessionEvent["type"] {
  switch (type) {
