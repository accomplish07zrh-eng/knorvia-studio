...
import type { KnorviaSessionEndedSubagent, KnorviaSessionRunningSubagent } from "@knorvia/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: KnorviaSessionRunningSubagent[];
  ended: KnorviaSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): KnorviaSessionRunningSubagent["status"] | undefined {
...
): KnorviaSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): KnorviaSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: KnorviaSessionRunningSubagent[] = [];
  const ended: KnorviaSessionEndedSubagent[] = [];
...
function encodeCursor(item: KnorviaSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly KnorviaSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: KnorviaSessionEndedSubagent[]; nextCursor?: string } {
