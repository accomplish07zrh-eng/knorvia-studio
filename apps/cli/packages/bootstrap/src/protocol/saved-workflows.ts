...
// 解析器与序列化器只从 @knorvia/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.knorvia-studio/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@knorvia/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  KNORVIA_WORKFLOWS_RUNS_MAX_LIMIT,
  knorviaWorkflowsDeleteParamsSchema,
  knorviaWorkflowsGetParamsSchema,
  knorviaWorkflowsListParamsSchema,
  knorviaWorkflowsMoveParamsSchema,
  knorviaWorkflowsRunsParamsSchema,
  knorviaWorkflowsUpdateMetaParamsSchema,
  type KnorviaSavedWorkflowRun,
  type KnorviaSavedWorkflowScope,
  type KnorviaWorkflowsDeleteResult,
  type KnorviaWorkflowsGetResult,
  type KnorviaWorkflowsListResult,
  type KnorviaWorkflowsMoveResult,
  type KnorviaWorkflowsRunsResult,
  type KnorviaWorkflowsUpdateMetaResult,
} from "@knorvia/shared";
...
function scopeOf(params: { scope?: KnorviaSavedWorkflowScope }): KnorviaSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  const params = parseParams(knorviaWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: KnorviaProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KnorviaWorkflowsGetResult> {
  const params = parseParams(knorviaWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(knorviaWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(knorviaWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(knorviaWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(KNORVIA_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: KnorviaSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, KnorviaSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, KnorviaSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  ...
  const params = parseParams(knorviaWorkflowsMoveParamsSchema, rawParams);
