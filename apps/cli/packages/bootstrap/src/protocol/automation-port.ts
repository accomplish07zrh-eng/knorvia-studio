...
import {
  ...
  knorviaAutomationCheckTaskBindingResultSchema,
  knorviaAutomationCreateResultSchema,
  knorviaAutomationDeleteResultSchema,
  knorviaAutomationListResultSchema,
  knorviaAutomationUpdateResultSchema,
  knorviaProtocolMethods,
  type KnorviaAutomationProtocol,
} from "@knorvia/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => KnorviaProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              knorviaProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              knorviaProtocolMethods.automationList,
              {},
              knorviaAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          knorviaProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        knorviaProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        knorviaProtocolMethods.automationDelete,
        { automationId: input.id },
        knorviaAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: KnorviaAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: KnorviaAutomationProtocol): CronAutomation {
  return {
