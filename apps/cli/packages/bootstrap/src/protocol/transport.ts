...
import { knorviaProtocolMessageSchema, knorviaProtocolMethods } from "@knorvia/shared";
import type { Logger } from "@knorvia/contracts";

type KnorviaProtocolOutgoingMessage =
  | KnorviaProtocolError
  | KnorviaProtocolNotification
  | KnorviaProtocolRequest
  | KnorviaProtocolResponse;

type KnorviaProtocolMessageHandler = (
  message: KnorviaProtocolMessage,
) => Promise<KnorviaProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface KnorviaProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: KnorviaProtocolMessageHandler;
  ...
  ) => readonly KnorviaProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: KnorviaProtocolRequestId) => {
    readonly messages: readonly KnorviaProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class KnorviaProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: KnorviaProtocolNdjsonConnectionOptions) {
  ...
  send(message: KnorviaProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("Knorvia Studio Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("Knorvia Studio Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): KnorviaProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("Knorvia Studio Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "knorvia_protocol.parse.failed",
    ...
    const parsed = knorviaProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid Knorvia Studio Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: KnorviaProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === knorviaProtocolMethods.sessionStop ||
        message.method === knorviaProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: KnorviaProtocolMessage): Promise<void> {
    if (this.terminal) return;
