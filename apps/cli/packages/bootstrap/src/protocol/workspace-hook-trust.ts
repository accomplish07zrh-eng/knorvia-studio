...
import { workspaceHookPolicySchema } from "@knorvia/contracts";
import type { WorkspaceHookPolicyProvider } from "@knorvia/core";
import {
  knorviaWorkspaceHookTrustGrantParamsSchema,
  knorviaWorkspaceHookTrustGrantReasonCodeSchema,
  knorviaWorkspaceHookTrustGrantResultSchema,
  type KnorviaWorkspaceHookTrustGrantReasonCode,
  type KnorviaWorkspaceHookTrustGrantResult,
} from "@knorvia/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = knorviaWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return knorviaWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): KnorviaWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): KnorviaWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = knorviaWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
