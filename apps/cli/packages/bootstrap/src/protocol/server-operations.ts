/* eslint-disable max-lines -- Knorvia Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@knorvia/adapters/exec";
import { inputIntentMetadata } from "../protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@knorvia/core";
import {
  CoreErrorType,
...
import {
  ...
  sessionSetThoughtLevelParamsSchema,
  sessionStopParamsSchema,
  sessionSubscribeParamsSchema,
  sessionSubagentsParamsSchema,
  knorviaTaskTokenUsageParamsSchema,
  knorviaUsageStatsParamsSchema,
  knorviaWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type KnorviaSessionCreateParams,
  type KnorviaDeliveryKind,
  type IntegratedTerminalShellSelection,
  type KnorviaSessionRuntimePreferencesScope,
  type KnorviaSessionRuntimePreferencesResult,
  type KnorviaModelContextBudgetStrategy,
  type KnorviaProtocolTrace,
  type KnorviaSessionEvent,
  type KnorviaSessionHistoryTarget,
  type KnorviaSessionResumeParams,
  type KnorviaSessionPersistence,
...
import { runSessionModelConfigMutation } from "../protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<KnorviaProtocolSessionRecord["app"]["readTarget"]>>>
>;

type KnorviaSessionRecordParams = (
  | KnorviaSessionCreateParams
  | (KnorviaSessionResumeParams & {
      mode?: KnorviaSessionCreateParams["mode"];
      model?: KnorviaSessionCreateParams["model"];
      parentSessionId?: KnorviaSessionCreateParams["parentSessionId"];
      thoughtLevel?: KnorviaSessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: KnorviaSessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: KnorviaModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: KnorviaProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<KnorviaProtocolSessionRecord["app"], "listThoughtLevels">,
...
const liveProtocolStreamingDeltaBatches = new WeakMap<
  KnorviaProtocolSessionRecord,
  Map<string, LiveProtocolStreamingDeltaBatch>
>();

function protocolEventSequenceKey(deliveryKind?: KnorviaDeliveryKind): string {
  return deliveryKind ?? DEFAULT_PROTOCOL_EVENT_SEQUENCE_KEY;
}

function getProtocolEventSequenceState(
  record: KnorviaProtocolSessionRecord,
...
function getProtocolToolInputTransmissionState(
  record: KnorviaProtocolSessionRecord,
  deliveryKind?: KnorviaDeliveryKind,
): KnorviaProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: KnorviaProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const knorvia = asRecord(meta.knorvia);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(knorvia.parentToolUseId) ??
    stringValue(knorvia.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: KnorviaSessionEvent,
  toolInputTransmissions: KnorviaProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
  ...
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("Knorvia Studio Protocol session event filtered", {
      event: "knorvia_protocol.session_event.filtered",
  ...
  // tool_input_delta 等高频模型中间态。Knorvia Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<KnorviaSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: KnorviaSessionEvent[] = [];
  const toolInputTransmissions: KnorviaProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("Knorvia Studio Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "knorvia_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<KnorviaSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<KnorviaProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<KnorviaSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: KnorviaProtocolSessionRecord;
  sessionId: SessionId;
  createParams: KnorviaSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("Knorvia Studio Protocol imported history persisted", {
    event: "knorvia_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: KnorviaProtocolAgentServerContext,
  rawParams: unknown,
  trace: KnorviaProtocolTrace | undefined,
  project: (
    record: KnorviaProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(sessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("Knorvia Studio Protocol session/create started", {
    event: "knorvia_protocol.session_create.started",
  ...
  try {
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 Knorvia session。
    ...
    context.logger?.info("Knorvia Studio Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "knorvia_protocol.session_create.completed",
    ...
    context.logger?.warn("Knorvia Studio Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "knorvia_protocol.session_create.failed",
      module: "bootstrap.knorvia_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: KnorviaProtocolAgentServerContext,
  params: KnorviaSessionResumeParams,
  ...
  context.logger?.debug("Knorvia Studio Protocol session resume started", {
    activeBeforeWait,
    event: "knorvia_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("Knorvia Studio Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "knorvia_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.knorvia_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      knorviaProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("Knorvia Studio Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "knorvia_protocol.session.resume_completed",
    module: "bootstrap.knorvia_protocol",
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "knorvia_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "knorvia_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.knorvia_protocol",
      sessionId: session.id,
...
export async function listSessions(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(sessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("Knorvia Studio Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "knorvia_protocol.session.persisted_missing",
      module: "bootstrap.knorvia_protocol",
...
export async function readSession(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionReadParamsSchema, rawParams);
...
export async function readMessages(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(sessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("Knorvia Studio Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("Knorvia Studio Protocol session/send accepted", {
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("Knorvia Studio Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("Knorvia Studio Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("Knorvia Studio Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("Knorvia Studio Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("Knorvia Studio Protocol goal paused"));
    }
...
export async function forkSession(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: KnorviaProtocolAgentServerContext,
  record: KnorviaProtocolSessionRecord,
  fork: Awaited<ReturnType<KnorviaProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: KnorviaProtocolAgentServerContext,
  parentRecord: KnorviaProtocolSessionRecord,
  forkRecord: KnorviaProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("Knorvia Studio Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("Knorvia Studio Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("Knorvia Studio Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("Knorvia Studio Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("Knorvia Studio Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("Knorvia Studio Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("Knorvia Studio Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("Knorvia Studio Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(sessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("Knorvia Studio Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(sessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: KnorviaProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(sessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: KnorviaSessionPersistence,
  expectedPersistence?: KnorviaSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(knorviaWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceKnorviaApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: KnorviaProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: KnorviaProtocolAgentServerContext,
  _record: KnorviaProtocolSessionRecord,
...
async function resolveForkTarget(
  context: KnorviaProtocolAgentServerContext,
  record: KnorviaProtocolSessionRecord,
  target: KnorviaSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(knorviaTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: KnorviaProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: knorviaProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "knorvia_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "knorvia_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // KnorviaProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("Knorvia Studio Protocol runtime preferences request started", {
    event: "knorvia_protocol.runtime_preferences.request_started",
    module: "bootstrap.knorvia_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      knorviaProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      sessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("Knorvia Studio Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "knorvia_protocol.runtime_preferences.response_received",
      module: "bootstrap.knorvia_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "knorvia_protocol.runtime_preferences.request_failed",
      module: "bootstrap.knorvia_protocol",
      scope,
      sessionId,
      traceId,
...
async function materializeSessionRecord(
  context: KnorviaProtocolAgentServerContext,
  params: KnorviaSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: KnorviaProtocolTrace,
): Promise<KnorviaProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("Knorvia Studio Protocol createRecord MCP config", {
    event: "knorvia_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: KnorviaProtocolSessionRecord | undefined;
  const app = await createWorkspaceKnorviaApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.knorvia-studio/.agents 的 enabled MCP，
      ...
      // Knorvia Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // Knorvia Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: KnorviaProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): KnorviaSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isKnorviaSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isKnorviaSessionMode(
  value: unknown,
): value is NonNullable<KnorviaSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("Knorvia Studio Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "knorvia_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "knorvia_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "knorvia_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "knorvia_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "knorvia_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: KnorviaStateUpdatedNotification = {
    patch,
    reason,
