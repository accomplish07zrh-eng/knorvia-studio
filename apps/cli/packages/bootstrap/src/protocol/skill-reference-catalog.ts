...
import {
  knorviaSkillsReferenceCatalogParamsSchema,
  type KnorviaSkillReferenceCatalogEntry,
  type KnorviaSkillsReferenceCatalogResult,
} from "@knorvia/shared";
import type { SkillLoadOutcome, SkillMetadata } from "@knorvia/contracts";
import { listKnorviaSkills } from "../skills.js";
import {
  parseParams,
  requireSession,
...
export async function getSkillReferenceCatalog(
  ...
  const params = parseParams(knorviaSkillsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
  ...
  const outcome = await listKnorviaSkills({
...
function toResult(
  authority: KnorviaSkillsReferenceCatalogResult["authority"],
...
function toReferenceCatalogEntry(skill: SkillMetadata): KnorviaSkillReferenceCatalogEntry {
  const scope =
    skill.source === "plugin" ? "plugin" : skill.scope === "project" ? "workspace" : "user";
  return {
    // `knorvia:` 是现有 UI provider 过滤契约；路径使同名不同来源仍有稳定行身份。
    id: `knorvia:${scope}:${skill.path}`,
    name: skill.name,
