...
import type { KnorviaProtocolMcpServer } from "@knorvia/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: KnorviaProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // Knorvia Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
