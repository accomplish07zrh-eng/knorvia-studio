...
import {
  createKnorviaProcessResourceSampler,
  type KnorviaProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: KnorviaProtocolAgentServer,
  send: (message: KnorviaProtocolNotification) => void,
  logger: Logger,
): KnorviaProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createKnorviaProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: knorviaProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "knorvia_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
