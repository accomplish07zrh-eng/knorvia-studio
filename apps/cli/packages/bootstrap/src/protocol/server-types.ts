...
import type { McpTelemetryTracker } from "@knorvia/adapters";
import type { WorkspaceHookPolicyProvider } from "@knorvia/core";
...
import {
  ...
  type KnorviaProtocolMessage,
  type KnorviaProtocolMethod,
  type KnorviaProtocolNotification,
  type KnorviaProtocolRequest,
  type KnorviaProtocolRequestId,
  type KnorviaProtocolTrace,
  type KnorviaSessionMode,
  type KnorviaSessionPersistence,
  type KnorviaWorkspaceRef,
} from "@knorvia/shared";
import type { KnorviaApp, KnorviaAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../protocol-v4/v4-gateway.js";
...
export type KnorviaProtocolAgentResolvedDependencies = KnorviaProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface KnorviaProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface KnorviaProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface KnorviaProtocolSessionRecord {
  app: KnorviaApp;
  ...
  protocolEventSequences: Map<string, KnorviaProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, KnorviaProtocolToolInputTransmissionState>;
...
export interface KnorviaProtocolClientRequestOptions {
  signal?: AbortSignal;
  timeoutMs?: number;
...
export interface KnorviaProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: KnorviaProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: KnorviaProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: KnorviaProtocolMessage): message is KnorviaProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: KnorviaProtocolMessage,
): message is KnorviaProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: KnorviaProtocolMessage,
): message is { id: KnorviaProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: KnorviaProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): KnorviaProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: KnorviaDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("Knorvia Studio Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "knorvia_protocol.session.require_missing",
...
export function createProtocolLogger(deps: KnorviaProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("knorvia").child({
    module: "bootstrap.knorvia_protocol",
  });
}
