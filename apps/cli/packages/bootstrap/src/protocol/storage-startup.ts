...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@knorvia/adapters/storage";
import {
...
export async function openProtocolStartupStorage(options: {
  ...
  onProgress?: (progress: KnorviaStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = knorviaStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: knorviaProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { knorviaStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@knorvia/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = knorviaStoragePathReadySchema.parse(JSON.parse(line));
