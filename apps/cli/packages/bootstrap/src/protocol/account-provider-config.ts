import {
  knorviaProviderUpdateAccountConfigParamsSchema,
  type KnorviaProviderUpdateAccountConfigResult,
} from "@knorvia/shared";
...
export async function updateAccountProviderConfig(
  ...
  params: unknown,
): Promise<KnorviaProviderUpdateAccountConfigResult> {
  const envelope = parseParams(knorviaProviderUpdateAccountConfigParamsSchema, params);
