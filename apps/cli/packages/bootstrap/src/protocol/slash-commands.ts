...
import {
  ...
  isReservedKnorviaSlashCommandName,
} from "../slash-command-surface.js";

/**
 * `workflow` 是 knorvia-guide 内置插件的自定义命令，随 CLI 打包，不受用户 commandOverrides
 * 影响；灰度关闭时只能在装配目录时按名剔除。
 */
const DYNAMIC_WORKFLOW_SLASH_COMMAND_NAME = "workflow";

export interface ListProtocolSlashCommandsOptions extends ListKnorviaCustomCommandsOptions {
  /**
   * 动态工作流灰度门。**只有显式 false
...
export async function listProtocolSlashCommands(
...
): Promise<KnorviaSlashCommand[]> {
  const builtins = listAppProtocolBuiltinSlashCommands();
  let customCommands: Awaited<ReturnType<typeof listKnorviaCustomCommands>>["commands"] = [];
  try {
    const outcome = await listKnorviaCustomCommands(options);
  ...
  return pinWorkflowAfterGoal([
    ...
    ...customCommands
      ...
      .filter((command) => !isReservedKnorviaSlashCommandName(command.name))
 ...
 * `workflow` 是 knorvia-guide 内置插件的自定义命令，
 * 按发现顺序会沉在 custom 段末尾；产品要求它与 `goal` 一样作为「开启一段工作」的入口，
 * 紧随 goal 之后。
 * 只调顺序：来源、去重与 reserved 规则不变；任一方缺席时保持原序。
 */
function pinWorkflowAfterGoal(commands: KnorviaSlashCommand[]): KnorviaSlashCommand[] {
...
function listAppProtocolBuiltinSlashCommands(): KnorviaSlashCommand[] {
  const sharedBuiltins = APP_PROTOCOL_VISIBLE_BUILTIN_SLASH_COMMAND_NAMES.flatMap((name) => {
    const command = BUILTIN_KNORVIA_SLASH_COMMAND_HELP_ENTRIES.find((entry) => entry.name === name);
    if (!command) return [];
    return [
      {
