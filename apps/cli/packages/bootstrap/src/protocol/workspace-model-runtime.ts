...
import { createInMemorySessionEventStore } from "@knorvia/adapters/storage";
import type { ModelSelection } from "@knorvia/contracts";
import {
  knorviaProviderTestModelConnectivityParamsSchema,
  knorviaWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(knorviaWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(knorviaProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function createWorkspaceKnorviaApp(
  context: KnorviaProtocolAgentServerContext,
  ...
  return context.deps.createKnorviaApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createKnorviaApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<KnorviaProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: KnorviaProtocolAgentServerContext,
  record: Pick<KnorviaProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {
