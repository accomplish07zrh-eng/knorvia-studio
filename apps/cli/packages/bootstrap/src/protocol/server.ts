...
import {
  knorviaPluginsCancelOperationParamsSchema,
  knorviaProtocolMethods,
  knorviaWorkspaceCancelGenerateTextParamsSchema,
  knorviaWorkspaceHookTrustGrantParamsSchema,
} from "@knorvia/shared";
import type { BrowserControlPort } from "@knorvia/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@knorvia/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import {
  ...
  parseParams,
  toProtocolError,
  type KnorviaProtocolClientRequestOptions,
  type KnorviaProtocolAgentDependencies,
  type KnorviaProtocolAgentServerContext,
  type KnorviaProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@knorvia/contracts";

export type { KnorviaProtocolAgentDependencies, KnorviaProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type KnorviaProtocolOutboundMessage = KnorviaProtocolNotification | KnorviaProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, KnorviaProtocolSessionRecord>;
}): Promise<void> {
...
interface KnorviaProtocolPostResponseBatch {
  readonly messages: readonly KnorviaProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class KnorviaProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<KnorviaProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: KnorviaProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: KnorviaProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    KnorviaProtocolRequestId,
    KnorviaProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: KnorviaProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createKnorviaApp);
    const resolvedDeps = {
      ...deps,
      createKnorviaApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, KnorviaProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: KnorviaProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("Knorvia Studio Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "knorvia_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: KnorviaProtocolRequestId): KnorviaProtocolOutboundMessage[] {
  ...
  async handleMessage(
    message: KnorviaProtocolMessage,
  ): Promise<KnorviaProtocolError | KnorviaProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("Knorvia Studio Protocol notification ignored", {
        event: "knorvia_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: KnorviaProtocolRequest,
  ): Promise<KnorviaProtocolError | KnorviaProtocolResponse> {
  ...
  private async dispatchRequest(request: KnorviaProtocolRequest) {
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(knorviaPluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(knorviaWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: KnorviaProtocolRequestId, result: unknown): KnorviaProtocolResponse {
    return { id, result };
  }
  ...
  private resolveClientRequest(id: KnorviaProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: KnorviaProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
