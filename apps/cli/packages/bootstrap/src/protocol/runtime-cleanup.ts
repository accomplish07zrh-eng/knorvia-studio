import type { Logger, McpPort } from "@knorvia/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@knorvia/adapters/mcp";
import type { SqliteSessionStore } from "@knorvia/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@knorvia/telemetry";
...
import type { KnorviaProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<KnorviaProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<KnorviaProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`Knorvia Studio Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `knorvia_protocol.${resource}.shutdown.failed`,
