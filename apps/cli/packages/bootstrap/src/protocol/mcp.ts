...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@knorvia/contracts";
import {
  knorviaMcpListParamsSchema,
  knorviaMcpListResultSchema,
  type KnorviaMcpListResult,
} from "@knorvia/shared";
import {
...
export async function listMcpServers(
  ...
  const params = parseParams(knorviaMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.knorvia_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.knorvia-studio` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return knorviaMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return knorviaMcpListResultSchema.parse({
