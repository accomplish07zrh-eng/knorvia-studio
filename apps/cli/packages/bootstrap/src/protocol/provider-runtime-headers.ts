...
import {
  ...
  knorviaProviderRuntimeHeadersResponseSchema,
  type KnorviaWorkspaceRef,
} from "@knorvia/shared";
import type { KnorviaAppOptions } from "../app/types.js";
import {
...
): NonNullable<KnorviaAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          knorviaProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: knorviaProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
      ...
      // knorvia-plan 的账号鉴权材料是每次请求刷新的运行时配置，
