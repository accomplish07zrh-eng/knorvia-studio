import {
  knorviaPluginsConfigureParamsSchema,
  knorviaPluginsResetConfigParamsSchema,
  knorviaPluginsInstallParamsSchema,
  knorviaPluginsMarketplaceAddParamsSchema,
  knorviaPluginsMarketplaceRemoveParamsSchema,
  knorviaPluginsMarketplaceUpdateParamsSchema,
  knorviaPluginsOverviewParamsSchema,
  knorviaPluginsListParamsSchema,
  knorviaPluginsSetEnabledParamsSchema,
  knorviaPluginsUninstallParamsSchema,
  knorviaPluginsUpdateParamsSchema,
  knorviaPluginsValidateParamsSchema,
  knorviaPluginsDescribeParamsSchema,
  knorviaPluginsRestoreBuiltinParamsSchema,
  type KnorviaAvailablePluginSummary,
  type KnorviaInstalledPluginSummary,
  type KnorviaPluginComponentGroup,
  type KnorviaPluginDiagnostic,
  type KnorviaPluginInfo,
  type KnorviaPluginMarketplaceSummary,
  type KnorviaPluginsConfigureResult,
  type KnorviaPluginsDescribeResult,
  type KnorviaPluginsInstallResult,
  type KnorviaPluginsListResult,
  type KnorviaPluginsMarketplaceMutationResult,
  type KnorviaPluginsOverviewResult,
  type KnorviaPluginsRestoreBuiltinResult,
  type KnorviaPluginsSetEnabledResult,
  type KnorviaPluginsUninstallResult,
  type KnorviaPluginsValidateResult,
} from "@knorvia/shared";
import type { PluginDiagnostic, PluginMetadata } from "@knorvia/contracts";
import {
  addKnorviaPluginMarketplace,
  configureKnorviaPlugin,
  describeKnorviaPlugin,
  getKnorviaPluginsOverview,
  installKnorviaMarketplacePlugin,
  removeKnorviaPluginMarketplace,
  resolveKnorviaPlugins,
  resetKnorviaPluginConfig,
  ...
  validateKnorviaPlugin,
} from "../plugins.js";
import { listInstalledPluginRecords } from "@knorvia/adapters/plugins";
...
import { createConfig, resolvePath, type ConfigResult } from "@knorvia/adapters/config";
import { parseParams, type KnorviaProtocolAgentServerContext } from "./server-types.js";

// 把 CLI 的 PluginMetadata 投影成协议可序列化的 KnorviaPluginInfo (只保留 UI 需要的字段)。
function toPluginInfo(plugin: PluginMetadata, configResult?: ConfigResult): KnorviaPluginInfo {
...
function toPluginDiagnostic(diagnostic: PluginDiagnostic): KnorviaPluginDiagnostic {
  return {
...
export async function listPlugins(
  context: KnorviaProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KnorviaPluginsListResult> {
  const params = parseParams(knorviaPluginsListParamsSchema, rawParams);
...
export async function setPluginEnabled(
  ...
  const params = parseParams(knorviaPluginsSetEnabledParamsSchema, rawParams);
...
export async function getPluginsOverview(
  context: KnorviaProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KnorviaPluginsOverviewResult> {
  const params = parseParams(knorviaPluginsOverviewParamsSchema, rawParams);
  const overview = getKnorviaPluginsOverview({
...
export async function addPluginMarketplace(
  ...
  const params = parseParams(knorviaPluginsMarketplaceAddParamsSchema, rawParams);
...
export async function removePluginMarketplace(
  ...
  const params = parseParams(knorviaPluginsMarketplaceRemoveParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function updatePluginMarketplace(
  ...
  const params = parseParams(knorviaPluginsMarketplaceUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function installPlugin(
  ...
  rawParams: unknown,
  abortSignal?: AbortSignal,
): Promise<KnorviaPluginsInstallResult> {
  const params = parseParams(knorviaPluginsInstallParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    installKnorviaMarketplacePlugin({
      abortSignal,
...
export async function uninstallPlugin(
  ...
  const params = parseParams(knorviaPluginsUninstallParamsSchema, rawParams);
  const removed = await uninstallKnorviaMarketplacePlugin({
    logger: context.logger,
    marketplace: params.marketplace,
...
export async function updatePlugin(
  ...
  const params = parseParams(knorviaPluginsUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  ...
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const installedPlugins: KnorviaInstalledPluginSummary[] = [];
    const dependencyClosure: string[] = [];
    // 聚合每条记录重装产生的诊断：installKnorviaMarketplacePlugin 失败时不抛错，而是返回
    // CLI 形态的 PluginDiagnostic（见其错误分支的 toMarketplaceInstallDiagnostic），
    // 这里逐条经协议侧 toPluginDiagnostic 投影成 KnorviaPluginDiagnostic 回传，
    // 让失败的重装显式暴露，而不是静默"成功"。
    const diagnostics: KnorviaPluginDiagnostic[] = [];
    for (const record of installed) {
      const result = await installKnorviaMarketplacePlugin({
        logger: context.logger,
...
export async function restoreBuiltinPlugin(
  ...
  const params = parseParams(knorviaPluginsRestoreBuiltinParamsSchema, rawParams);
...
export async function configurePlugin(
  ...
  const params = parseParams(knorviaPluginsConfigureParamsSchema, rawParams);
  await configureKnorviaPlugin({
...
export async function resetPluginConfig(
  ...
  const params = parseParams(knorviaPluginsResetConfigParamsSchema, rawParams);
  await resetKnorviaPluginConfig({
    logger: context.logger,
    pluginId: params.pluginId,
    scope: params.scope,
    workingDirectory: params.workspace.workspacePath,
...
export async function validatePlugin(
  context: KnorviaProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KnorviaPluginsValidateResult> {
  const params = parseParams(knorviaPluginsValidateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function describePlugin(
  context: KnorviaProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KnorviaPluginsDescribeResult> {
  const params = parseParams(knorviaPluginsDescribeParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    describeKnorviaPlugin({
      logger: context.logger,
  ...
  const components: KnorviaPluginComponentGroup[] = result.components.map((group) => ({
    kind: group.kind,
    items: group.items.map((item) => ({
      name: item.name,
      ...(item.description ? { description: item.description } : {}),
...
function toMarketplaceSummary(input: {
  ...
  isOfficial?: boolean;
  featured?: string[];
  refreshFailure?: KnorviaPluginMarketplaceSummary["refreshFailure"];
}): KnorviaPluginMarketplaceSummary {
  return {
    id: input.id,
    name: input.name,
    source: input.source,
...
function toAvailablePluginSummary(input: {
  ...
  listing?: KnorviaAvailablePluginSummary["listing"];
}): KnorviaAvailablePluginSummary {
  return {
    id: input.id,
    name: input.name,
...
function toInstalledPluginSummary(input: {
  ...
  installedAt?: string;
  componentTypes?: string[];
  hookDetails?: KnorviaInstalledPluginSummary["hookDetails"];
  updateStatus?: "none" | "update-available" | "version-changed";
  latestVersion?: string;
  listing?: KnorviaInstalledPluginSummary["listing"];
