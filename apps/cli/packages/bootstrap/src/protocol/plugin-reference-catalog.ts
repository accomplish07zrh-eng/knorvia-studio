...
import {
  ...
  knorviaPluginsReferenceCatalogParamsSchema,
  knorviaPluginsResolveSuggestedReferenceParamsSchema,
  type KnorviaPluginReferenceCatalogEntry,
  type KnorviaPluginDiagnostic as SharedPluginDiagnostic,
  type KnorviaPluginsReferenceCatalogResult,
  type KnorviaPluginsResolveSuggestedReferenceResult,
} from "@knorvia/shared";
import type { PluginReferenceCatalogEntry } from "@knorvia/contracts";
import { buildPluginReferenceCatalog } from "@knorvia/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(knorviaPluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<KnorviaPluginsResolveSuggestedReferenceResult> {
  const params = parseParams(knorviaPluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  if (
    ...
    return unavailable(
      ...
      "推荐插件不是受信任的官方 knorvia-plugins-bundled 来源",
    );
  }

  const workingDirectory = params.workspace.workspacePath;
  const readState = () => {
    const outcome = resolveKnorviaPlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: knorviaProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateKnorviaPluginMarketplace({
    ...
    const refreshTimeoutRequest = new Promise<never>((_, reject) => {
      ...
      refreshTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("刷新 knorvia-plugins-bundled 超时（10000 ms）");
  ...
  const overview = getKnorviaPluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getKnorviaPluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): KnorviaPluginReferenceCatalogEntry {
