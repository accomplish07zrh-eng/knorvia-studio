...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@knorvia/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@knorvia/core";
import type { Logger, PluginMetadata } from "@knorvia/contracts";
import {
  createAgentStateId,
...
interface LoadKnorviaAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadKnorviaAgentProfilesResult {
...
export async function loadKnorviaAgentProfiles(
  input: LoadKnorviaAgentProfilesInput,
): Promise<LoadKnorviaAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".knorvia-studio", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .knorvia-studio/agents/*.md 是仓库内容，不能通过 frontmatter
...
): LoadKnorviaAgentProfilesResult {
  const diagnostics: AgentProfileParseDiagnostic[] = [];
