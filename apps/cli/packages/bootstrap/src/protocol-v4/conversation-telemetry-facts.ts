...
import { getModelUsageTotalTokens, SessionEventType } from "@knorvia/contracts";
import { parseAutomationRunId } from "@knorvia/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const knorvia = recordValue(meta.knorvia);
  return (
    ...
    optionalString(knorvia.parentToolCallId) ??
    optionalString(knorvia.parentToolUseId)
  );
}
