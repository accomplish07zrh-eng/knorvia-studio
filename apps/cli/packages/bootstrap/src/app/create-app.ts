...
} from "@knorvia/adapters/storage";
import { createNodeLoggerFactory } from "@knorvia/adapters/logging";
import { createConfig, resolvePath } from "@knorvia/adapters/config";
import {
  createNodeExecutionAdapter,
  resolveEffectiveBashShellSelection,
} from "@knorvia/adapters/exec";
import { createNodeFileSystemAdapter } from "@knorvia/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@knorvia/adapters/http";
import { createJimpImageProcessorAdapter } from "@knorvia/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@knorvia/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@knorvia/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@knorvia/adapters/context";
import { createNodeSkillAdapter } from "@knorvia/adapters/skills";
import { createMcpAdapter } from "@knorvia/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@knorvia/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveKnorviaRuntimeEnv } from "@knorvia/shared";
import {
...
import { resolveKnorviaCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveKnorviaBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadKnorviaAgentProfiles } from "../subagents.js";
...
export async function createKnorviaApp(options: KnorviaAppOptions): Promise<KnorviaApp> {
  if (!options?.providerRegistry) {
    throw new Error("createKnorviaApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("knorvia").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveKnorviaRuntimeEnv(options.env ?? process.env) === "development",
    );
    const knorviaSubagentProfileOutcome = await loadKnorviaAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const knorviaSubagentProfiles = knorviaSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: knorviaSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: knorviaSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...knorviaSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          knorviaSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("Knorvia Studio runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("Knorvia Studio runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).KNORVIA_MAILBOX_ROOT ?? "~/.knorvia-studio/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveKnorviaBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveKnorviaCustomCommandPrompt(text, {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new KnorviaAttachmentFaultError(KNORVIA_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new KnorviaAttachmentFaultError(KNORVIA_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("Knorvia Studio app startup failed", error, {
