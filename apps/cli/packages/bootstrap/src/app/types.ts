import type { KnorviaToolExecResource, BackgroundBashOutputResult } from "@knorvia/shared";
import type { AiSdkModelAdapter } from "@knorvia/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { KnorviaModelOption } from "@knorvia/shared";
import type { EffectiveModelSelectionResult } from "@knorvia/shared/model-selection";
export type { KnorviaModelOption } from "@knorvia/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { KnorviaInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@knorvia/contracts";
...
export type KnorviaAppRuntimeConfigInput = AgentRuntimeConfig;

export interface KnorviaAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: KnorviaAppRuntimeConfigInput;
  /**
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@knorvia/contracts").TurnBackgroundAttribution;
...
export interface KnorviaPluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface KnorviaPluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: KnorviaInstalledPluginData | null;
}
...
export interface KnorviaApp {
  ...
  getCurrentModelOption?(): KnorviaModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): KnorviaModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@knorvia/shared").KnorviaSessionSubagentsResult>;
  ...
  listModels(): KnorviaModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<KnorviaPluginSetResult>;
  uninstallPlugin(plugin: string): Promise<KnorviaPluginUninstallResult>;
  /**
...
export interface RunKnorviaProtocolAgentOptions {
...
export interface ListKnorviaSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
