...
export const OFFICIAL_BROWSER_USE_PLUGIN_ID = `${OFFICIAL_BROWSER_USE_PLUGIN_NAME}@${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}`;
/**
...
export const OFFICIAL_NODE_REPL_HOST_PLUGIN_ID = `${OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME}@${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}`;
const OFFICIAL_CUA_PLUGIN_NAME = "computer-use";
export const OFFICIAL_CUA_PLUGIN_ID = `${OFFICIAL_CUA_PLUGIN_NAME}@${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}`;

export interface OfficialPluginDefinition {
  // 内容型 plugin (无 MCP server / 无系统依赖) 可以设为 true,
  // 这样用户首次 `/skill <name>` 就能用,不必先 `knorvia plugins enable`。
...
// knorvia-guide 原本没有 requiredSeedPaths，seed 丢文件时会静默装出一个
// 没有 /workflow 命令的插件——症状是命令不存在，没有任何诊断。commands/ 与技能正文都钉住。
const OFFICIAL_KNORVIA_GUIDE_REQUIRED_SEED_PATHS = [
...
export const OFFICIAL_PLUGIN_DEFINITIONS: readonly OfficialPluginDefinition[] = [
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: {
        "zh-CN": "操作 Knorvia Studio 内置浏览器，检查网页并验证交互。",
      },
    },
  ...
  },
  {
    listing: {
      ...
      description_i18n: {
        "zh-CN": "将旧版会话恢复为 Knorvia Studio 任务与会话记录。",
      },
    },
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: {
        "zh-CN": "开发、校验 Knorvia Studio 插件，完成本地 dev 市场安装、试用与更新。",
      },
    },
    rootCandidates: [
  ...
  },
  {
    ...
    listing: {
      ...
      description_i18n: { "zh-CN": "创建、编辑和验证可复用的 Knorvia Studio 技能。" },
    },
    name: "skill-creator",
  ...
  },
  {
    ...
    // 让用户/agent 开箱即用地拿到 Knorvia 配置指南、自诊断技能与 dynamic workflow 编写指南。
    defaultEnabled: true,
    listing: {
      author: KNORVIA_AUTHOR,
      category: "utilities",
      displayName: "Knorvia Studio Guide",
      displayName_i18n: { "zh-CN": "Knorvia Studio 使用指南" },
      description_i18n: {
        "zh-CN": "提供 Knorvia Studio 配置指南与插件、技能、MCP、命令和钩子诊断。",
      },
      examplePrompts: [
        "How do I configure MCP servers in Knorvia Studio?",
        "Diagnose my current Knorvia Studio setup",
      ],
      examplePrompts_i18n: {
        "zh-CN": ["Knorvia Studio 里怎么配置 MCP 服务器？", "帮我诊断当前的 Knorvia Studio 配置"],
      },
    },
    name: "knorvia-guide",
    requiredSeedPaths: OFFICIAL_KNORVIA_GUIDE_REQUIRED_SEED_PATHS,
    rootCandidates: [
      "packages/knorvia-guide-plugin",
      "../knorvia-guide-plugin",
      "../../knorvia-guide-plugin",
      "../../../knorvia-guide-plugin",
    ],
    version: "0.2.0",
  },
  {
    ...
    // isKnorviaCuaInternalFeatureEnabled（打包层默认 true）与输入框入口 hidden 默认值的联动语义。
    name: "computer-use",
    hostMcpServerNames: ["node_repl"],
    // 用户露出名统一为「Computer Use / 电脑控制」。包名与 producer 仓库仍保持 cua，
    ...
    listing: {
      ...
      // 插件更名为 computer-use 后，CDN 图标仍发布在 cua 目录；沿用资源路径避免 404。
    },
    rootCandidates: [
      "packages/cua-plugin",
      "../cua-plugin",
...
// 注意: 任何解析 plugin 的入口 (CLI 子命令 resolveKnorviaPlugins、应用启动 resolveStartupPlugins)
...
export const DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS: ReadonlySet<string> = new Set(
  OFFICIAL_PLUGIN_DEFINITIONS.filter((definition) => definition.defaultEnabled).map(
    (definition) => `${definition.name}@${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}`,
  ),
);
...
export function resolveOfficialPluginHostMcpServerNames(pluginId: string): string[] {
  const definition = OFFICIAL_PLUGIN_DEFINITIONS.find(
    (candidate) => `${candidate.name}@${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}` === pluginId,
  );
