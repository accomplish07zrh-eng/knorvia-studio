...
export function createBundledMcpRuntimeConfig(input: {
  ...
  return {
    ...
    env: {
      ...input.env,
      // 桌面打包态 process.execPath 是 Knorvia Helper；缺少 Node 模式会误进 Electron main。
...
export function writeOfficialPluginRuntimeManifest(input: OfficialRuntimeManifestInput): void {
  const manifestPath = join(input.rootPath, ".knorvia-plugin", "plugin.json");
  ...
  for (const [serverKey, serverRaw] of Object.entries(mcpServers)) {
    ...
    mcpServer.env = {
      ...mcpServerEnv,
      // 桌面打包态的 process.execPath 是 Knorvia Helper。
      ...
      [KNORVIA_PLUGIN_ID_ENV_KEY]: `${input.pluginName}@${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}`,
    };
...
export function officialPluginHostPrefixArgs(): string[] | undefined {
  if (isSeaRuntime()) return [KNORVIA_PLUGIN_HOST_COMMAND];

  const entrypoint = process.argv[1];
  if (!entrypoint) return undefined;

  return [...process.execArgv, resolve(entrypoint), KNORVIA_PLUGIN_HOST_COMMAND];
}
