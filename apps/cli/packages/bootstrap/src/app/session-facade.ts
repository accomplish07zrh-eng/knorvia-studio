import { updateUiLocaleInFileConfig, type ConfigResult } from "@knorvia/adapters/config";
import type { AgentRuntime } from "@knorvia/core";
import { resolveLocale } from "@knorvia/i18n";
import { normalizeModelSelection, type ModelSelection } from "@knorvia/provider";
import {
...
import type { PrepareUserExecutionBoundary, KnorviaApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<KnorviaApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<KnorviaApp["updateTargetStatus"]>
      >,
