...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    ...
    uninstallPlugin: async (plugin: string): Promise<KnorviaPluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE) {
