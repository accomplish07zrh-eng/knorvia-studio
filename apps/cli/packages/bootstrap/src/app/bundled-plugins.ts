...
const OFFICIAL_PLUGIN_MARKETPLACE = KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE;
const SEA_PLUGIN_ASSET_PREFIX = "knorvia-official-plugins/";
const SEA_PLUGIN_MANIFEST_ASSET_KEY = `${SEA_PLUGIN_ASSET_PREFIX}manifest.json`;
const SEED_MARKER_FILE = ".knorvia-plugin-seed.json";
    ...
    if (plugin.missingSeedPaths.length > 0) {
      warnCacheDegraded(input.logger, {
        error: Object.assign(
          ...
          { code: "KNORVIA_PLUGIN_SEED_INCOMPLETE" },
        ),
...
export function resolveOfficialPluginRoots(input: {
  ...
  // cua 内置 plugin 默认不启用，由 feature flag 控制加载。在 seed/discovery 层门控
  // （而非只隐藏某个 UI 面），这样开关关闭时用户无法经 plugin 列表/marketplace/MCP 设置/CLI 命令看到它。
  if (!isKnorviaCuaInternalFeatureEnabled(input.env ?? process.env)) {
    suppressedBuiltins.add(KNORVIA_CUA_OFFICIAL_PLUGIN_ID);
  }
  ...
  const fallbackRoots = failedSeeds.flatMap((definition) => {
    ...
    if (`${definition.name}@${OFFICIAL_PLUGIN_MARKETPLACE}` === KNORVIA_CUA_OFFICIAL_PLUGIN_ID) {
      return [];
    }
...
function resolveFilesystemPluginRoot(definition: OfficialPluginDefinition): string | undefined {
  for (const baseDir of candidateBaseDirs()) {
    for (const relativePath of definition.rootCandidates) {
      ...
      if (existsSync(join(rootPath, ".knorvia-plugin", "plugin.json"))) return rootPath;
    }
  }
  return undefined;
}
...
function readSeedPluginDescription(
  ...
  const manifestFile = plugin.files.find((file) => file.path === ".knorvia-plugin/plugin.json");
...
function isSeedUsable(targetRoot: string, definition: OfficialPluginDefinition): boolean {
  try {
    const manifest = JSON.parse(
      readFileSync(join(targetRoot, ".knorvia-plugin", "plugin.json"), "utf8"),
    ) as { name?: unknown };
...
function candidateBaseDirs(): string[] {
  // Electron app-server 运行在 resources/knorvia/knorvia.cjs，官方插件资源也随桌面包
