...
import { openStartupSqliteSessionStore } from "@knorvia/adapters/storage";
import type { SessionInfo } from "@knorvia/contracts";
import type { ListKnorviaSessionsOptions, ResolveLatestSessionOptions } from "./app/types.js";
...
export async function listKnorviaSessions(
  options: ListKnorviaSessionsOptions = {},
): Promise<SessionInfo[]> {
  const ownsStore = options.sessionStore === undefined;
  let sessionStore = options.sessionStore;
  if (!sessionStore) {
