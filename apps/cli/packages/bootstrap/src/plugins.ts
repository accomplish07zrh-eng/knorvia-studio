...
export interface ResolveKnorviaPluginsOptions {
...
export interface ListKnorviaPluginsOptions extends ResolveKnorviaPluginsOptions {}

export interface SetKnorviaPluginEnabledOptions extends ResolveKnorviaPluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetKnorviaPluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface KnorviaMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface KnorviaAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface KnorviaInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface KnorviaPluginsOverviewData {
  marketplaces: KnorviaMarketplaceSummaryData[];
  availablePlugins: KnorviaAvailablePluginData[];
  installedPlugins: KnorviaInstalledPluginData[];
  restorableBuiltins: KnorviaAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface KnorviaMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: KnorviaMarketplaceSummaryData[];
}

export interface AddKnorviaMarketplaceOptions extends ResolveKnorviaPluginsOptions {
...
export interface RemoveKnorviaMarketplaceOptions extends ResolveKnorviaPluginsOptions {
  marketplace: string;
}

export interface UpdateKnorviaMarketplaceOptions extends ResolveKnorviaPluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallKnorviaMarketplacePluginOptions extends ResolveKnorviaPluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallKnorviaMarketplacePluginOptions extends ResolveKnorviaPluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`knorvia plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateKnorviaMarketplacePluginOptions extends ResolveKnorviaPluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateKnorviaPluginPathOptions extends ResolveKnorviaPluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface KnorviaPluginUpdateData extends KnorviaPluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveKnorviaPluginsOptions {
  pluginId: string;
}

interface ConfigureKnorviaPluginOptions extends ResolveKnorviaPluginsOptions {
...
interface ResetKnorviaPluginConfigOptions extends ResolveKnorviaPluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateKnorviaPluginOptions extends ResolveKnorviaPluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeKnorviaPluginOptions extends ResolveKnorviaPluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface KnorviaPluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
...
export function getKnorviaPluginsOverview(
  options: ResolveKnorviaPluginsOptions = {},
): KnorviaPluginsOverviewData {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  const outcome = resolveKnorviaPlugins({
    ...options,
    configResult,
    pluginStorageRoot,
  });
  ...
  // knorvia-plugins-bundled 的内置与 CDN 分片已在 adapter 层合并为唯一 canonical manifest。
  const catalogs: Array<{
    summary: KnorviaMarketplaceSummaryData;
  ...
  const restorableBuiltins: KnorviaAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
    ...
    const listing = def.listing
      ...
      : undefined;
    return {
      id: `${def.name}@${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
...
export function listKnorviaPlugins(options: ListKnorviaPluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveKnorviaPlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setKnorviaPluginEnabled(
  options: SetKnorviaPluginEnabledOptions,
): Promise<SetKnorviaPluginEnabledResult> {
...
export async function addKnorviaPluginMarketplace(
  options: AddKnorviaMarketplaceOptions,
): Promise<KnorviaMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeKnorviaPluginMarketplace(
  options: RemoveKnorviaMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateKnorviaPluginMarketplace(
  options: UpdateKnorviaMarketplaceOptions,
): Promise<KnorviaMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installKnorviaMarketplacePlugin(
  options: InstallKnorviaMarketplacePluginOptions,
): Promise<KnorviaPluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallKnorviaMarketplacePlugin(
  options: UninstallKnorviaMarketplacePluginOptions,
): Promise<KnorviaInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const pluginId = resolvePluginIdForMutation(options);

    // 官方 CDN marketplace 与内置插件共享 knorvia-plugins-bundled id 空间，且其缓存
...
/**
 * `knorvia plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateKnorviaMarketplacePlugin(
  options: UpdateKnorviaMarketplacePluginOptions,
): Promise<KnorviaPluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateKnorviaPluginMarketplace({
    ...options,
  ...
  const installed = await installKnorviaMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `knorvia plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateKnorviaPluginPath(
  options: ValidateKnorviaPluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const knorviaCuaPluginId = KNORVIA_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === knorviaCuaPluginId &&
    !isKnorviaCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
...
export async function resetKnorviaPluginConfig(
  options: ResetKnorviaPluginConfigOptions,
...
export async function validateKnorviaPlugin(
  options: ValidateKnorviaPluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeKnorviaPlugin(
  options: DescribeKnorviaPluginOptions,
...
function resolvePluginContext(options: ResolveKnorviaPluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallKnorviaMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveKnorviaPluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.knorvia-studio/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".knorvia-studio", "config.json");
