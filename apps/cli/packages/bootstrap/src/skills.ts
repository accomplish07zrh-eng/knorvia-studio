...
import type { Logger, SkillContent, SkillDiagnostic, SkillLoadOutcome } from "@knorvia/contracts";
import { resolveKnorviaPlugins } from "./plugins.js";
import { collectDisabledPaths } from "./skill-command-overrides.js";

export interface ListKnorviaSkillsOptions {
...
export interface InspectKnorviaSkillOptions extends ListKnorviaSkillsOptions {
  name: string;
}

export interface KnorviaSkillInspection {
  diagnostics: SkillDiagnostic[];
  skill: SkillContent;
}

export async function listKnorviaSkills(
  options: ListKnorviaSkillsOptions = {},
...
export async function inspectKnorviaSkill(
  options: InspectKnorviaSkillOptions,
): Promise<KnorviaSkillInspection> {
  const discovery = createSkillDiscovery(options);
  if (!discovery.enabled) {
...
function createSkillDiscovery(options: ListKnorviaSkillsOptions):
  | {
      enabled: false;
      workingDirectory: string;
    }
