...
import {
  KNORVIA_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type KnorviaProcessResourceSample,
} from "@knorvia/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const KNORVIA_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = KNORVIA_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateKnorviaProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: KnorviaProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: KnorviaProcessResourceSample["platform"];
  arch?: KnorviaProcessResourceSample["arch"];
...
export interface KnorviaProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createKnorviaProcessResourceSampler(
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: KnorviaProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, KNORVIA_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
