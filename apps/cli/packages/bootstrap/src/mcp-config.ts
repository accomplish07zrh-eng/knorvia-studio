...
import {
  ...
  KNORVIA_CUA_NODE_REPL_HOST_ENV_KEY,
  KNORVIA_CUA_OFFICIAL_PLUGIN_ID,
  KNORVIA_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  KNORVIA_PLUGIN_ID_ENV_KEY,
} from "@knorvia/shared";

export { KNORVIA_CUA_BROKER_SOCKET_ENV_KEY as KNORVIA_CUA_BROKER_SOCKET_ENV } from "@knorvia/shared";
// CLI 入口会先清理 broker 凭据；shared node_repl 的可信配置随后从进程内捕获快照恢复它们。
function resolveKnorviaCuaBrokerSocket(): string | undefined {
  // captured 优先；运行时残留的 stale socket 不能覆盖可信快照。
  return (
    getCapturedKnorviaCuaBrokerCredentials().socket ||
    process.env[KNORVIA_CUA_BROKER_SOCKET_ENV_KEY]?.trim()
  );
}

function resolveKnorviaCuaBrokerToken(): string | undefined {
  return undefined;
}

const NODE_REPL_SERVER_NAME = "node_repl";
const REFRESH_MARKER_ENV = "KNORVIA_CUA_PERMISSION_BROKER_REFRESH_MARKER";
    ...
    Object.entries(pluginServers)
      .filter(
        ([name, config]) =>
          ...
          config.type === "stdio" &&
          config.env?.[KNORVIA_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() ===
            KNORVIA_CUA_OFFICIAL_PLUGIN_ID,
      )
      .map(([name]) => name),
  );
...
export async function listMcpServerStatuses(
  ...
  for (const [name, config] of Object.entries(servers)) {
    // 迁移后旧的 cua MCP 只是历史配置，不再进入状态投影；CUA
...
export function omitMcpServers(
  ...
  return injectKnorviaCuaBrokerMcpServers(
    kept,
    resolveKnorviaCuaBrokerSocket(),
    resolveKnorviaCuaBrokerToken(),
    trustedOfficialCuaServerNames,
  );
}

function injectKnorviaCuaBrokerMcpServers(
  servers: Record<string, McpServerConfig>,
...
function injectCuaCredentialsIntoNodeRepl(
  ...
  const captured = getCapturedKnorviaCuaBrokerCredentials();
  ...
  return {
    ...
    env: {
      ...config.env,
      [KNORVIA_CUA_BROKER_SOCKET_ENV_KEY]: socketPath,
      ...(refreshMarker ? { [REFRESH_MARKER_ENV]: refreshMarker } : {}),
      ...(pluginAuthority ? { [KNORVIA_CUA_PLUGIN_AUTHORITY_ENV_KEY]: pluginAuthority } : {}),
      [KNORVIA_CUA_NODE_REPL_HOST_ENV_KEY]: "1",
      [KNORVIA_PLUGIN_ID_ENV_KEY]: KNORVIA_CUA_OFFICIAL_PLUGIN_ID,
    },
  };
}

function isKnorviaCuaStdioServer(
  name: string,
  config: McpServerConfig,
): config is McpStdioServerConfig {
  if (config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 内置 official cua plugin 的 MCP server 走 __knorvia-plugin-host，command 是 Helper
  // (非 cua)、args 是 [knorvia.cjs, __knorvia-plugin-host, server.js]（非 cua package arg），
  // 上面的 name/command/args 三条都匹配不到。_plugin id 由 adapters resolver 权威写入 env
  // （manifest/user env 不可覆盖），用它识别 official plugin server。
  if (
    config.env?.[KNORVIA_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() === KNORVIA_CUA_OFFICIAL_PLUGIN_ID
  ) {
    return true;
  }
  // 判定与 desktop/services 共用 @knorvia/shared 的单一事实源，避免两条注入入口漂移。
  if (isKnorviaCuaMcpCommand(config.command)) return true;
  return (config.args ?? []).some(isKnorviaCuaMcpPackageArg);
}
...
function isRetiredCuaMcpServer(name: string, config: McpServerConfig | undefined): boolean {
  ...
  return (
    name !== NODE_REPL_SERVER_NAME && config !== undefined && isKnorviaCuaStdioServer(name, config)
  );
}
