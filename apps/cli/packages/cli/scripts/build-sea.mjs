...
const cliBundle = resolve(dist, "knorvia.cjs");
const seaBlobForTarget = (target) => resolve(dist, `knorvia-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "knorvia-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/knorvia.cjs",
        output: `dist/knorvia-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "knorvia-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        KNORVIA_STORAGE_DIR: storageRoot,
      };
      delete env.KNORVIA_BFS_BINARY;
      delete env.KNORVIA_RG_BINARY;
      delete env.KNORVIA_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/knorvia.cjs. Run `pnpm build` first.");
  }
