...
export const resolveBuildExternal = () => ["@knorvia/tui", "playwright-core", "koffi"];
  ...
  return {
    ...
    setup(builder) {
      ...
      builder.onResolve({ filter: /^zod(?:\/|$)/ }, async (args) => {
        if (args.pluginData?.knorviaZodResolving) return;
        const resolved = await builder.resolve(args.path, {
          importer: args.importer,
          resolveDir: args.resolveDir,
          kind: args.kind,
          pluginData: { knorviaZodResolving: true },
        });
...
export const resolveBuildOptions = (args = [], env = process.env) => {
  ...
  const e2eCoverage = env.KNORVIA_E2E_COVERAGE === "1";
...
export const resolveBuildAliases = ({
  ...
  "@knorvia/shared-types": resolve(cliDirectory, "../shared-types/dist/index.js"),
  ...
  "@knorvia/shared/protocol-v4": resolve(
    rootDirectory,
    "../../packages/shared/src/protocol-v4/index.ts",
  ),
  ...
  "@knorvia/shared/model-selection": resolve(
    rootDirectory,
  ...
  "@knorvia/shared/config-schema": resolve(
    rootDirectory,
    "../../packages/shared/src/config-schema.ts",
  ),
  "@knorvia/shared/workspace-hook-discovery": resolve(
    rootDirectory,
  ...
  "@knorvia/shared/workspace-hook-mutation": resolve(
    rootDirectory,
    "../../packages/shared/src/workspace-hook-mutation.ts",
  ),
  // verdict 直连 import 需要本精确 alias；漏声明会被通用
  // "@knorvia/shared" 前缀改写成 `src/index.ts/workspace-hook-review-monotonicity`，
  // Desktop agent/SEA 打包直接失败。
  "@knorvia/shared/workspace-hook-review-monotonicity": resolve(
    rootDirectory,
    "../../packages/shared/src/workspace-hook-review-monotonicity.ts",
  ),
  // trust store 文件 schema 单源下沉后的新 subpath；漏声明会被通用
  // "@knorvia/shared" 前缀改写成 `src/index.ts/workspace-hook-trust-store-file`，
  // Desktop agent/SEA 打包失败（同上两类既有规则）。
  "@knorvia/shared/workspace-hook-trust-store-file": resolve(
    rootDirectory,
...
export const buildCli = async ({
  ...
  const outfile = resolve(cliDirectory, "dist/knorvia.cjs");
  ...
  await build({
    banner: {
      ...
      js: `#!/usr/bin/env node\n"use strict";\nif (process.argv.length === 3 && process.argv[2] === "--licenses") { const sea = require("node:sea"); const nodeNotice = sea.isSea() ? "\\n\\n## Bundled Node.js runtime\\n\\n" + sea.getAsset("knorvia-node-license", "utf8") : ""; process.stdout.write(${JSON.stringify(notices.toString("utf8"))} + nodeNotice, () => process.exit(0)); } else {`,
    },
    footer: { js: "}" },
    bundle: true,
    define: {
    ...
    format: "cjs",
    // 桌面 app 集成只内置 knorvia.cjs，旧 desktop-agent 构建复用 CLI 调试产物，
    ...
    // 远端 SSH 复用已部署的独立 Node v22.16 跑同一份 knorvia.cjs。降到 node22 保证这份产物
