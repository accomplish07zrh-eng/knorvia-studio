...
import { createDenyPermissionBroker } from "@knorvia/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  KnorviaAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `sessionEventTypeSchema`（`shared/src/protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `sessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createKnorviaApp` 注入点边界）。
 *
