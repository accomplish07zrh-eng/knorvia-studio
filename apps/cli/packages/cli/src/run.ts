...
import { createNodeLoggerFactory } from "@knorvia/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@knorvia/core";
import { color, formatJson, supportsColor } from "@knorvia/core";
import { getKnorviaCopy, isUiLocale, type UiLocale } from "@knorvia/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@knorvia/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getKnorviaCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "knorvia_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("knorvia doctor", colors)}\n`);
...
const runKnorviaProtocolCommand = async (
  ctx: RunContext,
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("knorvia").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runKnorviaProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
