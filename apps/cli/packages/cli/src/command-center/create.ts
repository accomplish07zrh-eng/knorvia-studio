import type { TuiSubmitPrompt } from "@knorvia/tui";
import {
...
import { modelConfigurationRequiredResponse } from "../tui-login-state.js";
...
export function createCommandCenter(deps: CommandCenterDeps): TuiSubmitPrompt {
  return async (input, options) => {
    ...
    if (!command) {
      if (await isModelConfigurationRequired(deps)) {
        return {
          modelConfigurationRequired: true,
          mode: deps.getMode?.(),
          response: modelConfigurationRequiredResponse(deps.getLocale?.()),
        };
      }
...
async function isModelConfigurationRequired(deps: CommandCenterDeps): Promise<boolean> {
