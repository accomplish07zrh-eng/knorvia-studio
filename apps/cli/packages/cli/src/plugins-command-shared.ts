...
import type { listKnorviaPlugins } from "@knorvia/bootstrap";
import { createInterface } from "node:readline";
import type { GlobalOptions, RunContext } from "@knorvia/shared-types";
import type { CliEnv } from "./env.js";

export type BootstrapModule = typeof import("@knorvia/bootstrap");
export type PluginListOutcome = ReturnType<typeof listKnorviaPlugins>;
 ...
 * 避免 `knorvia plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addKnorviaPluginMarketplace",
  getPluginsOverview: "getKnorviaPluginsOverview",
  installPlugin: "installKnorviaMarketplacePlugin",
  listPlugins: "listKnorviaPlugins",
  removeMarketplace: "removeKnorviaPluginMarketplace",
  setPluginEnabled: "setKnorviaPluginEnabled",
  uninstallPlugin: "uninstallKnorviaMarketplacePlugin",
  updateMarketplace: "updateKnorviaPluginMarketplace",
  updatePlugin: "updateKnorviaMarketplacePlugin",
  validatePluginPath: "validateKnorviaPluginPath",
...
/** `knorvia plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@knorvia/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by knorvia. Use: user, project");
  }
