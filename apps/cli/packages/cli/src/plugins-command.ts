...
import type { SetKnorviaPluginEnabledResult, KnorviaPluginInstallData } from "@knorvia/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: knorvia plugins <command> [options]
...
Scopes: user (default), project. \`knorvia plugin\` is an alias of \`knorvia plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart knorvia to apply.\n`,
  );
...
function reportInstallOutcome(
  ctx: RunContext,
  options: GlobalOptions,
  result: KnorviaPluginInstallData,
  pluginId: string,
): number {
  const failed = hasErrors(result.diagnostics);
  if (options.json) {
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetKnorviaPluginEnabledResult; error?: string }> = [];
