...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@knorvia/tui";
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "clipboard-"));
