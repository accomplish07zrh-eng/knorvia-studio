...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@knorvia/shared/runtime-tool-runtime";
...
const assetPrefix = "knorvia-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".knorvia-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.KNORVIA_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || resolveKnorviaDataRoot();
