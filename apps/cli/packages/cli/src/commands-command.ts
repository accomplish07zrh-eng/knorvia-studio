...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listKnorviaCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: knorvia commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listKnorviaCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@knorvia/bootstrap"));
  return (await bootstrap()).inspectKnorviaCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: KnorviaCustomCommandInspection,
  options: GlobalOptions,
): string {
