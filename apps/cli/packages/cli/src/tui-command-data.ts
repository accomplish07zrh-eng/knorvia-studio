import type { TuiSessionMetadata } from "@knorvia/tui";
...
export async function listSessionsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listKnorviaSessionsForTui =
    deps.listSessions ?? (await loadBootstrapModule()).listKnorviaSessions;
  const sessions = await listKnorviaSessionsForTui({
