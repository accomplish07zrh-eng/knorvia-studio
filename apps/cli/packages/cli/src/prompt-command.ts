...
import { formatJson, type PresentationSurface } from "@knorvia/core";
import type { RunContext, GlobalOptions } from "@knorvia/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createKnorviaApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createKnorviaApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createKnorviaApp ?? bootstrapModule?.createKnorviaApp;
    if (!createApp) throw new Error("Knorvia Studio app factory is unavailable.");
    ...
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.KNORVIA_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `knorvia -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveKnorviaCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedKnorviaSlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listKnorviaCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadKnorviaCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: knorvia hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
