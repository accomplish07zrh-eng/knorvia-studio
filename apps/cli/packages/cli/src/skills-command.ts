...
type BootstrapModule = typeof import("@knorvia/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listKnorviaSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: knorvia skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listKnorviaSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@knorvia/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectKnorviaSkill;
...
function formatHumanSkillInspection(
  inspection: KnorviaSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: KnorviaSkillInspection, cwd: string): string =>
  formatJson({
    cwd,
