...
// 品牌类型（SessionId）和 createKnorviaApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createKnorviaApp ?? bootstrapModule?.createKnorviaApp;
  if (!createAppFactory) throw new Error("Knorvia Studio app factory is unavailable.");
