...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeKnorviaRuntimeEnv(env),
    ...buildKnorviaToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildKnorviaToolEnvPassthroughEnv(env);
...
function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): KnorviaRuntimeEnv {
  const explicit = normalizeKnorviaRuntimeEnv(env[KNORVIA_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.KNORVIA_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.KNORVIA_BETA === "1" || env.KNORVIA_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])knorvia-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.KNORVIA_STORAGE_DIR = join(homedir(), ".knorvia-studio-beta");
}
