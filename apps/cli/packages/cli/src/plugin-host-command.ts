...
import { KNORVIA_PLUGIN_HOST_COMMAND } from "@knorvia/contracts/plugins";
import {
...
import { KNORVIA_CUA_OFFICIAL_PLUGIN_ID, KNORVIA_PLUGIN_ID_ENV_KEY } from "@knorvia/shared/mcp";
import type { RunContext } from "@knorvia/shared-types";

const HOST_USAGE = `${KNORVIA_PLUGIN_HOST_COMMAND} <server-path> [-- <server-arg>...]`;
...
export function isPluginHostInvocation(argv: readonly string[]): boolean {
  return argv[0] === KNORVIA_PLUGIN_HOST_COMMAND;
}

// __knorvia-plugin-host 在 agent 子进程里运行 official plugin 的 MCP server（server.js）。
...
// Agent 进程曾收到过 Helper 凭据，不能证明当前传入的 server 就是官方 cua：
...
export async function runPluginHostCommand(ctx: RunContext, argv: string[]): Promise<number> {
  ...
  try {
    ...
    const capturedBrokerCredentials = getCapturedKnorviaCuaBrokerCredentials();
    ...
    const originalBrokerSocket = process.env[KNORVIA_CUA_BROKER_SOCKET_ENV_KEY];
    ...
    try {
      ...
      if (originalBrokerSocket === undefined) {
        delete process.env[KNORVIA_CUA_BROKER_SOCKET_ENV_KEY];
      } else {
        process.env[KNORVIA_CUA_BROKER_SOCKET_ENV_KEY] = originalBrokerSocket;
      }
    }
...
type CapturedBrokerCredentials = ReturnType<typeof getCapturedKnorviaCuaBrokerCredentials>;
...
function assertCapturedBrokerLaunchIsAuthorized(credentials: CapturedBrokerCredentials): void {
  ...
  const pluginId = process.env[KNORVIA_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase();
  ...
  // 残留读取方只能靠 CLI 自己的 typecheck 发现（根 `pnpm typecheck` 不含 apps/cli）。
  ...
  if (
    ...
    pluginId !== KNORVIA_CUA_OFFICIAL_PLUGIN_ID ||
    process.env[KNORVIA_CUA_NODE_REPL_HOST_ENV_KEY] !== "1"
  ) {
    throw new Error(
      "Captured Knorvia Studio CUA broker credentials may only launch the trusted shared node_repl host",
    );
  }
}
