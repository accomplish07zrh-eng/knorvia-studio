...
import type { PlaywrightChromiumModule } from "@knorvia/adapters/browser";
...
const ASSET_PREFIX = "knorvia-playwright-runtime/";
...
export async function loadCliPlaywrightChromium(): Promise<PlaywrightChromiumModule> {
  ...
  const require = createRequire(join(runtimeDirectory, "knorvia-playwright-loader.cjs"));
...
function cacheBaseDirectory(): string {
  return join(resolveKnorviaDataRoot(), "cache", "sea-assets");
}
