...
export const KNORVIA_INLINE_PLUGIN_MARKETPLACE = "inline";
export const KNORVIA_PLUGIN_HOST_COMMAND = "__knorvia-plugin-host";
/**
 * 隐藏子命令：dynamic workflow 的沙箱子进程入口（`__knorvia-dwf-child <entry path>`；argv 末位是
 * harness 写好的入口文件路径，payload 不过命令行）。
 *
 * 与 {@link KNORVIA_PLUGIN_HOST_COMMAND} 同族、同机制：SEA 单文件二进制不解释 Node CLI 旗标，
 ...
 */
export const KNORVIA_DWF_CHILD_COMMAND = "__knorvia-dwf-child";

export function isOfficialMarketplaceId(id: string): boolean {
  return id === KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE;
}
...
export interface PluginDiscoverRequest {
  ...
  // 让用户不必先 `knorvia plugins enable` 就能用 (例如纯内容型的 skill-creator)。
