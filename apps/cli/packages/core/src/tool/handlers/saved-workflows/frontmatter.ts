...
//
// body 用 YAML 而不是 JSON：`yaml` 已经是 @knorvia/core 的直接依赖（不新增依赖），而手改
...
export function isSavedWorkflowSentinel(value: string): boolean {
  ...
  return normalized === SAVED_WORKFLOW_SENTINEL || normalized === LEGACY_SAVED_WORKFLOW_SENTINEL;
}
...
export function parseSavedWorkflow(source: string): SavedWorkflowParseResult {
  ...
  if (start >= lines.length || !isSavedWorkflowSentinel(lines[start]!.trim())) {
    return {
      ok: false,
