...
import { parseOfficialMcpToolError, type OfficialMcpToolErrorCode } from "@knorvia/shared";
import {
 ...
 * 响应来自已校验 origin 的 Knorvia 后端。stdio 官方 MCP 与第三方 MCP 塞同样的 payload 一律忽略：
...
function readCuaToolName(toolName: string): string | undefined {
  ...
  // 命名空间形式 `mcp__plugin_cua_computer-use__<action>`。
 ...
 * 读取宿主写入的 CUA 目标应用身份。
 *
 * 只认 `knorvia/nodeReplCuaApp`：producer 自己的 `knorvia.cua/app-associations-v1` 也可能出现在
...
function readNodeReplCuaApp(output: Record<string, unknown>): NodeReplCuaAppDisplay | undefined {
  ...
  const parsed = nodeReplCuaAppDisplaySchema.safeParse(
    meta?.[KNORVIA_MCP_NODE_REPL_CUA_APP_META_KEY],
  );
