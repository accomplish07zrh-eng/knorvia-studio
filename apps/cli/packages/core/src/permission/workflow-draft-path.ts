...
import { WORKFLOW_DRAFTS_DIR } from "@knorvia/contracts";
...
/** `filePath` 是否落在 `<workingDirectory>/.knorvia-studio/workflow-drafts/` 之内。 */
function isWorkflowDraftPath(input: WorkflowDraftPathInput): boolean {
  ...
  // 只看路径字符串，不看文件系统：`.knorvia-studio/workflow-drafts-other/x.ts` 因此不会因为前缀
