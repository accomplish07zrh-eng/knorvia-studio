...
export function serviceRegistrationEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.KNORVIA_SERVER_SKIP_SERVICE_REGISTRATION !== "1";
}
...
async function runServe(
  ...
  if (shouldRegisterService(daemonRequested, supervisorProcess)) {
    ...
    if (existing && (existing.state === "ready" || existing.state === "starting")) {
      if ((existing.serviceRegistered && !legacyIdentityMigration) || startupMode === "fallback") {
        if (json) stdout(io, existing);
          ...
          stdout(
            io,
            `Knorvia Studio Server ${existing.state} at ${existing.host ?? ""}:${existing.port ?? ""}`,
          );
        return 0;
      }
    ...
    if (!serviceStarted) {
      const child = fork(
        ...
        [...SERVER_SERVICE_ARGS, "--server-root", layout.serverRoot],
        {
          ...
          env: {
            ...process.env,
            KNORVIA_DATA_BASE_DIR: layout.dataBaseDir,
            KNORVIA_SERVER_ROOT: layout.serverRoot,
          },
        },
      );
      child.unref();
    ...
    const started = await waitForPersistedStatus(
      ...
      () => {
        if (childEarlyExit) {
          throw new Error(
            `Knorvia Studio Server daemon exited before ready (code=${childEarlyExit.code ?? "null"} signal=${childEarlyExit.signal ?? "none"}); check ${layout.statusFile} for details`,
          );
        }
      },
      serviceStarted,
    ...
    if (json) stdout(io, started);
      ...
      stdout(
        io,
        `Knorvia Studio Server ${started.state} at ${started.host ?? ""}:${started.port ?? ""}`,
      );
    process.stdin.pause();
      ...
      launch: (generation, release) => {
        ...
        return fork(corePath, [String(generation)], {
          ...
          env: {
            ...inheritedEnv,
            ...releaseWiring,
            ...(runtimeRoot ? { KNORVIA_SERVER_RUNTIME_ROOT: runtimeRoot } : {}),
          },
  ...
  if (json) stdout(io, status);
    ...
    stdout(
      io,
      `Knorvia Studio Server ${status.state} at ${status.host ?? ""}:${status.port ?? ""}`,
    );
...
async function runControl(
  ...
  try {
    ...
    if (command === "status") {
      ...
      result = persisted.status ?? createStoppedServerStatus(KNORVIA_VERSION);
...
async function runUninstall(
  ...
  const first = await (io.confirm?.("Type DELETE to uninstall Knorvia Studio Server: ") ??
    Promise.resolve(""));
...
async function finishUninstall(
  ...
  try {
    ...
    // Knorvia allowlist，并把保留项写入结果供用户审计。先落盘 marker，再释放 lock，
    // 让并发 Supervisor 在删除 run 目录的最后窗口也会 fail-closed。
    await writeFile(
      layout.uninstalledFile,
      `${JSON.stringify({ uninstalled: true, uninstalledAt: Date.now(), version: KNORVIA_VERSION, preservedPaths }, null, 2)}\n`,
...
async function delegateLegacyCli(argv: readonly string[], io: CliIO): Promise<number> {
  const candidate =
    process.env.KNORVIA_LEGACY_CLI_ENTRY?.trim() ||
    join(dirname(fileURLToPath(import.meta.url)), "knorvia.cjs");
  try {
    await access(candidate);
  } catch {
    stdout(io, argv.length ? `Unknown command: ${argv[0]}` : "Knorvia Studio TUI");
