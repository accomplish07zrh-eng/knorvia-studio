...
function getDefaultServerDataRoot(): string {
  const configured = process.env.KNORVIA_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".knorvia-studio", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\knorvia-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".knorvia-studio") {
