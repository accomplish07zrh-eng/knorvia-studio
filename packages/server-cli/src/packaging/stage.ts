...
interface StageOptions {
  ...
  distDir: string;
  /** 既有 CLI/Agent bundle（knorvia.cjs，自包含 CJS） */
  ...
  /** 未被 pnpm 链接到 node_modules 的 workspace 包（例如 @knorvia/tui）。 */
...
# 由 knorvia-server staging 生成：定位发行根后用随包 Node 启动 Server CLI。
...
/** 组装 `knorvia-server-<os>-<arch>/` 发行目录；只做本地组装，不上传、不发布。 */
export async function stageRelease(options: StageOptions): Promise<StagedRelease> {
  ...
  const releaseName = `knorvia-server-${options.target}`;
  ...
  await cp(options.agentBundlePath, join(runtimeDir, "knorvia.cjs"), { dereference: true });
  ...
  const launcherPath = join(
    releaseDir,
    "bin",
    options.target.startsWith("win32-") ? "knorvia.cmd" : "knorvia",
  );
  ...
  const componentSpecs = [
    ...
    { id: "agent-runtime", paths: ["runtime/knorvia.cjs", "runtime/licenses/agent"] },
    ...(plugins.length > 0
      ? [
          {
            id: "official-plugins",
