...
import { createServiceLogger } from "@knorvia/services/node";
import {
...
function exposeWebSocket(
  ...
  services.exposeOnChannelServer(
    server,
    scope ? new Map([[IKnorviaAgentService.channelName, scope.service]]) : new Map(),
  );
  socket.onClose(() => {
    void scope?.dispose();
    rawServer.dispose();
...
export async function createCoreHttpServer(
  ...
  const info: ServerRemoteInfo = {
    serverId: options.serverId ?? hostname() ?? "knorvia-server",
  ...
  app.use("/ws/host", async (context, next) => {
    const capability = context.req.header(KNORVIA_RPC_HOST_CAPABILITY_HEADER);
