...
import { IKnorviaAgentService } from "@knorvia/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitKnorviaBuiltinProviderConfigFilePath =
    process.env[KNORVIA_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const knorviaBuiltinProviderConfigFilePath = explicitKnorviaBuiltinProviderConfigFilePath
    ? explicitKnorviaBuiltinProviderConfigFilePath
    : typeof __KNORVIA_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeKnorviaBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __KNORVIA_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!knorviaBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 Knorvia Studio Built-in Provider Config，且未设置 ${KNORVIA_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IKnorviaAgentService));
