...
import { invalidateDeferredDraftSessionForSkillChange } from "@/lib/draftSkillInvalidation.js";
...
const EMPTY_PINNED_TASKS: KnorviaTaskMeta[] = [];
...
export const WorkspaceSidebarItem = memo(function WorkspaceSidebarItem({
  ...
  const workspaceKnorviaState = useKnorviaSessionStore((state) =>
    selectWorkspaceKnorviaState(state, tab.workspacePath, tab.workspaceIdentity),
  );
  const activeTaskId = workspaceKnorviaState.activeTaskId;
  ...
  const removeOptimisticTaskListItem = useKnorviaSessionStore(
  ...
  const workspaceKnorviaStateRef = useRef(workspaceKnorviaState);
  workspaceKnorviaStateRef.current = workspaceKnorviaState;
  ...
  const handleRemoveWorkspace = useCallback(async () => {
    ...
    if (
      hasRunningWorkspaceChat({
        workspaceState: workspaceKnorviaStateRef.current,
  ...
  const handleRenameTask = useCallback(
    async (taskId: string, title: string) => {
      ...
      try {
        meta = await taskService.renameTask({
          taskId,
          workspacePath: tab.workspacePath,
          title,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleSetTaskPinned = useCallback(
    async (taskId: string, pinned: boolean) => {
      ...
      try {
        const meta = await taskService.setTaskPinned({
          taskId,
          workspacePath: tab.workspacePath,
          pinned,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleArchiveTask = useCallback(
    async (taskId: string) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await taskService.archiveTask({
        taskId,
        workspacePath: tab.workspacePath,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
      });
  ...
  return (
    <li ref={itemRef} style={itemStyle} className="space-y-2">
      ...
      <RemoteSyncDialogs
        ...
        localKnorviaAgentService={baseServices.agentService}
        remoteKnorviaAgentService={services.agentService}
        remoteTarget={tab.remoteTarget}
