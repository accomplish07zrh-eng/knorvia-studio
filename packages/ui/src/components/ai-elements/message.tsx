...
import type { EditorInfo, FileStat, OpenInEditorOptions } from "@knorvia/shared";
...
import { createKnorviaFileCitationRemarkPlugin } from "@/lib/fileCitationRemarkPlugin.js";
import { windowsFileLinkEscapeRemarkPlugin } from "@/lib/windowsFileLinkEscapeRemarkPlugin.js";
import { projectKnorviaFileCitations } from "@/lib/fileCitation.js";
import { rewriteMarkdownArtifactImageSources } from "@knorvia/shared";
...
export type MessageResponseProps = {
  ...
  /** 仅 Assistant 正文开启：把完整 knorvia-file-citation 投影为现有文件链接。 */
  renderKnorviaFileCitations?: boolean;
};
...
function resolveMessageCodeTheme(
  ...
  return theme === "dark" || theme === "knorvia-dark"
    ? codePreviewSettings.darkTheme
    : codePreviewSettings.lightTheme;
}
...
export function buildMessageStreamdownRenderKey(params: {
  ...
  return [
    ...
    params.renderKnorviaFileCitations ? "citations" : "plain",
...
export const messageResponsePropsAreEqual = (
  ...
  nextProps.renderKnorviaFileCitations === prevProps.renderKnorviaFileCitations &&
...
export const MessageResponse = memo(
  ({
    ...
    onOpenFileLink,
    onOpenExternalUrl,
    renderKnorviaFileCitations = false,
    workspacePath,
    workspaceHomePath,
    workspaceIdentity,
    ...
    const projectedCitationMarkdown = useMemo(
      () =>
        renderKnorviaFileCitations
          ? projectKnorviaFileCitations(rawMarkdown, { streaming: renderStreaming }).visibleText
          : rawMarkdown,
      [rawMarkdown, renderStreaming, renderKnorviaFileCitations],
    );
    ...
    const messageRemarkPlugins = useMemo<PluggableList>(
      () => [
        ...
        ...(renderKnorviaFileCitations && workspacePath
          ? [createKnorviaFileCitationRemarkPlugin(workspacePath, workspaceHomePath)]
          : []),
      ],
      [renderKnorviaFileCitations, workspaceHomePath, workspacePath],
    );
    ...
    const boundaryResetKey = useMemo(
      () =>
        renderStreaming
          ...
          : `${streamdownMode}:${renderKnorviaFileCitations ? "citations" : "plain"}:${hashMarkdownCacheKey(targetMarkdown)}`,
      [renderStreaming, renderKnorviaFileCitations, streamdownMode, targetMarkdown],
    );
