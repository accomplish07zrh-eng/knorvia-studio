 ...
 * 3. mention / slash 目录按内核能力开启；未连接内核只复用编辑器，不挂载原生目录。
 */
...
import type { KnorviaSlashCommand } from "@knorvia/shared";
...
function E2ELexicalInputBridgePlugin({ inputTestId }: { inputTestId?: string }) {
  ...
  useEffect(() => {
    ...
    const detachBridge = (input: HTMLElement | null) => {
      if (
        input &&
        (input as { __knorviaLexicalInputE2E?: typeof bridge }).__knorviaLexicalInputE2E === bridge
      ) {
        delete (input as { __knorviaLexicalInputE2E?: typeof bridge }).__knorviaLexicalInputE2E;
      }
    ...
    const attachBridge = (input: HTMLElement | null) => {
      ...
      Object.defineProperty(input, "__knorviaLexicalInputE2E", {
        configurable: true,
        value: bridge,
      });
...
interface LexicalChatInputProps {
  ...
  nativeSlashCommands?: readonly KnorviaSlashCommand[];
  nativeSlashOnly?: boolean;
    ...
    <div className="relative flex-1">
      <LexicalComposer initialConfig={initialConfig}>
        <div className="relative">
          ...
          <LeadingChineseSlashAliasPlugin disabled={disabled || !enableSlashCommands} />
