// composer parity：`#` 会话候选的数据源从旧 sessionStore/taskQueryCache/remote*
...
function getSessionLabel(task: KnorviaTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: KnorviaTaskMeta): string {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, KnorviaTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.agentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.agentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.agentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.agentService,
        },
      ];
    }
