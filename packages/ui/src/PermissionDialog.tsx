...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@knorvia/shared/protocol-v4";
...
function readPermissionRuleScopes(option: KnorviaPermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: KnorviaPermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: KnorviaPermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<KnorviaProvider, Record<string, string>>
> = {
  knorvia: {
    // 内置 Agent 发来的项目级记忆授权文案是英文原文。
...
function getPermissionDisplayReason(request: KnorviaPermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: KnorviaPermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: KnorviaPermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  const respondWithOption = useCallback(
    (option: KnorviaPermissionOption) => {
  ...
  // 当前 Knorvia Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // Knorvia Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。
