...
export function getTaskChangeSummary(
  task: Pick<KnorviaTaskMeta, "changeSummary"> | null | undefined,
): KnorviaTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: KnorviaTaskChangeSummary,
...
export function buildTurnChangeSummary(
  turn: KnorviaPersistedFileChange | null | undefined,
): KnorviaTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: KnorviaTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
