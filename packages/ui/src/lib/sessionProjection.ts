...
export function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    ...
    // Knorvia Protocol 必须收到严格的 providerId/modelId 结构。
    return {
  ...
  return [
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableKnorviaMode(mode),
      options: getKnorviaAgentModeSelectOptions(),
    },
  ];
}
...
function resolveSettingsThoughtLevelCurrentValue(
  thoughtLevel: KnorviaSessionSettingsState["thoughtLevel"],
): string | undefined {
  ...
  const defaultThoughtLevel =
    thoughtLevel.defaultLevel && thoughtLevelValues.has(thoughtLevel.defaultLevel)
      ...
      : undefined;
  // Knorvia Protocol 的 defaultLevel 是模型事实，current 为空时表示用户尚未显式修改。
...
function normalizeAvailableKnorviaMode(mode: KnorviaSessionMode): string {
  return KNORVIA_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

function getKnorviaAgentModeSelectOptions(): NonNullable<KnorviaConfigOption["options"]> {
