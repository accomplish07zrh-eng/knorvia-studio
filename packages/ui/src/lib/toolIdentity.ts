import {
  getKnorviaToolFamilyForName,
  isTodoPlanToolName,
  isKnorviaFileContentWriteToolName,
  normalizeKnorviaToolName,
  type KnorviaKnownToolName,
  type KnorviaToolFamily,
} from "@knorvia/shared";
...
export interface ToolCallIdentity {
  toolName: KnorviaKnownToolName | string | null;
...
function identityFromKnownToolName(
  ...
  const family = getKnorviaToolFamilyForName(toolName);
  if (!family) {
    return null;
  }
  return {
...
function readRawToolNameCandidates(raw: unknown) {
  return {
    ...
    knorvia: readNestedString(raw, ["_meta", "knorvia", "toolName"]),
...
function isLegacyAgentTool(
  ...
  const rawKnorvia = normalizeLegacyToken(rawNames.knorvia);
...
export function resolveToolCallIdentity(toolCall: ToolIdentityLike): ToolCallIdentity {
  ...
  for (const candidate of [
    ...
    { value: rawNames.knorvia, source: "raw-knorvia-meta" as const },
  ]) {
  ...
  if (isLegacyAgentTool(toolCall, rawNames)) {
    return identityFromLegacyFamily(
      rawNames.direct ?? rawNames.knorvia ?? "Agent",
      "agent",
      "legacy-payload",
    );
  }

  // `Task` 现在是现役 Claude 兼容工具名，但历史 Knorvia Agent 投影会用
  ...
  if (
    // 当前 Knorvia Agent 传给 app 的 plan mode 退出工具是 ExitPlanMode，
...
export function isFileContentWriteToolCall(
  ...
  if (isKnorviaFileContentWriteToolName(identity.toolName)) {
    return true;
  }
