import type { KnorviaWorkspaceTaskListChanged } from "@knorvia/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<KnorviaWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<KnorviaWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<KnorviaWorkspaceTaskListChanged, "reason">,
): boolean {
