...
// sessions-index 只补实时 activity/detail，最终拼出旧 KnorviaGroupedTaskView 同形结果。
...
function taskKeyOf(task: KnorviaTaskMeta): string {
...
function taskOrderMapKey(task: KnorviaTaskMeta): string {
...
function nodeMapKey(node: KnorviaGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, KnorviaGroupedTaskViewStructureMember>,
): KnorviaTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: KnorviaTaskListItem; addedAt: number; key: string }> = [];
...
interface BuildGroupedTaskViewParams {
  ...
  taskIndexItems: KnorviaTaskMeta[];
  /** sessions-index 派生的会话 activity/detail，只 enrich 命中的持久行。 */
  sessions: KnorviaTaskMeta[];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, KnorviaTaskListItem>();
  ...
  const membersByGroupId = new Map<string, KnorviaGroupedTaskViewStructureMember[]>();
  ...
  const nodes: KnorviaGroupedTaskViewNode[] = structure.groups.map((group) => {
