// re-home 产物（为删除 knorviaChatMessages 铺路，模式同 task-types-core）。
// 本文件承载旧消息转换层（knorviaChatMessages/knorviaChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（knorviaChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  KnorviaAssistantMessagePart,
  KnorviaAssistantMessageFeedback,
  KnorviaPromptAttachment,
  KnorviaTimelineMeta,
  KnorviaTaskSnapshotBodyRef,
  KnorviaTaskSnapshotToolFieldRef,
  KnorviaTaskSnapshotToolSlice,
} from "@knorvia/shared";
...
export interface TaskChatToolCall {
  ...
  /** Knorvia 固定工具名；kind 仍保留给旧 Knorvia Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = KnorviaAssistantMessagePart;
