...
> &
  Partial<Pick<IKnorviaTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<KnorviaTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: KnorviaTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: KnorviaTaskMeta[][]): KnorviaTaskMeta[] {
  const itemByEntityKey = new Map<string, KnorviaTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, KnorviaTaskMeta>();
