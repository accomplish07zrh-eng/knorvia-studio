...
import { extractConversationPreviewFileReferences } from "@knorvia/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@knorvia/shared";
import {
  extractKnorviaFileCitationDirectives,
  resolveKnorviaFileCitationPreviewKind,
} from "@/lib/fileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractKnorviaFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 knorvia-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
