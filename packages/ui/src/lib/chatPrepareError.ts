...
import { normalizeKnorviaUiError } from "@/lib/uiError.js";

export const MODEL_CONFIG_MISSING_UI_ERROR_CODE = "model_config_missing";

export type ModelConfigMissingUiError = KnorviaError & {
...
export function isProviderNotReadyError(error: unknown): boolean {
  return normalizeUnknownError(error).code === KNORVIA_AGENT_PROVIDER_NOT_READY_CODE;
}
...
): KnorviaError & { detail?: string } {
  const normalizedError = normalizeKnorviaUiError(
