import type { KnorviaConfigOption, KnorviaTaskMeta } from "@knorvia/shared";
import { getKnorviaAgentModeSelectOptions } from "@knorvia/shared";
...
function resolveKnorviaRecoveredTaskModelValue(taskModel: string | undefined): string | null {
...
function resolveRecoveredTaskModelValue(
  taskMeta: Pick<KnorviaTaskMeta, "provider" | "model">,
): string | null {
  ...
  if (taskMeta.provider === "knorvia") {
    return resolveKnorviaRecoveredTaskModelValue(normalizedTaskModel);
  }
...
function createRecoveredModelOption(modelValue: string): KnorviaConfigOption {
  return {
    category: "model",
    currentValue: modelValue,
    id: "model",
...
function createRecoveredModeOption(modeValue: string): KnorviaConfigOption {
  return {
    category: "mode",
...
function createRecoveredThoughtLevelOption(thoughtLevel: string): KnorviaConfigOption {
  return {
...
function mergeRecoveredTaskModelConfigOptions({
  ...
  taskMeta: Pick<KnorviaTaskMeta, "provider" | "model">;
  configOptions: readonly KnorviaConfigOption[];
...
export function resolveTaskRestorePreloadConfigOptions({
  ...
  taskMeta: Pick<KnorviaTaskMeta, "provider" | "model"> &
    Partial<Pick<KnorviaTaskMeta, "mode" | "thoughtLevel">>;
  cachedTaskConfigOptions?: readonly KnorviaConfigOption[];
}): KnorviaConfigOption[] {
  ...
  if (taskMeta.provider === "knorvia" && mode) {
  ...
  if (taskMeta.provider === "knorvia" && thoughtLevel) {
