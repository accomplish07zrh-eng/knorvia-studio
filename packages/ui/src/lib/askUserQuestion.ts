type KnorviaUserQuestionAnswers = Record<string, unknown>;
...
interface AskUserQuestionData {
  ...
  answers?: KnorviaUserQuestionAnswers;
}
...
function readQuestions(input: unknown): unknown[] {
  ...
  // Knorvia Agent 的单题输入与交互请求的多题输入共用展示管线。
  return typeof input.question === "string" && Array.isArray(input.options) ? [input] : [];
}

function readAnswers(input: unknown): KnorviaUserQuestionAnswers | undefined {
  if (!isPlainRecord(input)) {
    return undefined;
  }
...
function readNestedAskUserQuestionAnswers(input: unknown): KnorviaUserQuestionAnswers | undefined {
...
function parseKnorviaAskUserQuestionOutput(
  output: unknown,
  input: unknown,
): KnorviaUserQuestionAnswers | undefined {
...
export function readAskUserQuestionAnswers(value: {
  ...
  const parsedKnorviaOutputAnswers = parseKnorviaAskUserQuestionOutput(
    value.output,
    readAskUserQuestionInput(value),
  );
  if (parsedKnorviaOutputAnswers) {
    return parsedKnorviaOutputAnswers;
  }
  ...
  if (isPlainRecord(value.raw)) {
    ...
    const parsedRawKnorviaOutputAnswers = parseKnorviaAskUserQuestionOutput(
      value.raw.rawOutput,
      readAskUserQuestionInput(value),
    );
    if (parsedRawKnorviaOutputAnswers) {
      return parsedRawKnorviaOutputAnswers;
    }
    const nestedRawAnswers = readNestedAskUserQuestionAnswers(value.raw.output);
    if (nestedRawAnswers) {
      return nestedRawAnswers;
    }
    const parsedRawKnorviaAnswers = parseKnorviaAskUserQuestionOutput(
      value.raw.output,
      readAskUserQuestionInput(value),
    );
    if (parsedRawKnorviaAnswers) {
      return parsedRawKnorviaAnswers;
    }
...
export function getAskUserQuestionAnswerText(
  ...
  answers: KnorviaUserQuestionAnswers | undefined,
  noAnswerText: string,
) {
