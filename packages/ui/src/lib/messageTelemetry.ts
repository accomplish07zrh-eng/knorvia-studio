...
interface FinalizePromptTelemetryInput {
  ...
  status: string;
  finishedAt: number;
  fileChanges?: readonly KnorviaPersistedFileChange[];
  usage?: KnorviaUsage;
  errorType?: string;
  errorMsg?: string;
...
function resolvePromptTelemetryModelProvider(params: {
  ...
  // ghost supplier 代表尚未解析到稳定 custom provider 的临时隔离态，provider 维度统一回退到当前 Knorvia Agent provider。
...
export function buildPromptTelemetryExtraDetail(params: {
  ...
  return {
    ...
    // agent 字段取 Knorvia Agent provider；本仓库没有独立 session.agentId。
...
const COMPACTION_TERMINAL_STATUSES: readonly KnorviaTimelineStatus[] = [
  "completed",
  "failed",
  "interrupted",
];

function isCompactionTerminalStatus(status: KnorviaTimelineStatus): boolean {
...
export function buildCompactionTelemetryExtraDetail(params: {
  timeline: KnorviaContextCompactionTimelineMeta;
  provider?: KnorviaProvider;
...
function buildPromptUsageTelemetryExtraDetail(
  ...
  return {
    ...
    cache_write_input_tokens: String(usage.cachedWriteInputTokens ?? 0),
    // Knorvia Agent 链路未透出 tool use prompt token，成功态显式补 0 保持 extraDetail 字段集合完整。
...
function mergePromptUsage(left: KnorviaUsage | null, right: KnorviaUsage): KnorviaUsage {
  if (!left) {
    return {
...
function collectFileChangeMetrics(fileChanges: readonly KnorviaPersistedFileChange[] | undefined) {
...
export function recordPromptModelRequestStarted(
  taskId: string,
  event: Extract<KnorviaStreamEvent, { type: "task_network_debug_status" }>,
...
export function recordAgentStepTelemetryEvent(input: {
  ...
  switch (input.event.type) {
    ...
    case "agent_message_chunk":
      if (input.event.knorviaTimeline) {
        return finalized;
      }
