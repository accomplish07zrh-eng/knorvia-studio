import type { FileEntry, KnorviaTaskMeta, KnorviaTaskRuntimeStatus } from "@knorvia/shared";
import type { IFileService } from "@knorvia/services";
import type { WorkspaceKnorviaUIState } from "@/store/sessionStoreTypes.js";
...
export function hasRunningWorkspaceChat(params: {
  workspaceState: Pick<WorkspaceKnorviaUIState, "draftRuntime" | "taskRuntimeByTaskId">;
  taskItems: Pick<KnorviaTaskMeta, "taskId">[];
}): boolean {
...
function isWorkspaceRemovalBlockingRuntimeStatus(status: KnorviaTaskRuntimeStatus): boolean {
