import type { ArmsCustomEventPayload } from "@knorvia/shared";
...
type ArmsCustomEventDebugWindow = Window & {
  __knorviaArmsCustomEventsE2E?: ArmsCustomEventE2EEntry[];
};
...
export function recordArmsCustomEventForE2E(
  ...
  const buffer = (host.__knorviaArmsCustomEventsE2E ??= []);
  buffer.push({
    ...payload,
