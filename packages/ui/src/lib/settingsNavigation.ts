...
const SETTINGS_SECTION_INTENT_KEY = "knorvia-settings-section-intent",
  SETTINGS_USAGE_TAB_INTENT_KEY = "knorvia-settings-usage-tab-intent",
  SETTINGS_PLUGIN_TAB_INTENT_KEY = "knorvia-settings-plugin-tab-intent",
  SETTINGS_PLUGIN_ORIGIN_INTENT_KEY = "knorvia-settings-plugin-origin-intent",
  SETTINGS_PLUGIN_SCOPE_KEY_INTENT_KEY = "knorvia-settings-plugin-scope-key-intent";
const SETTINGS_MODEL_PROVIDER_ID_INTENT_KEY = "knorvia-settings-model-provider-id-intent";
const SETTINGS_SECTION_INTENT_EVENT = "knorvia:settings-section-intent",
  SETTINGS_LAST_SECTION_STORAGE_KEY = "knorvia-settings-last-section";
const HIDDEN_SETTINGS_SECTIONS = new Set<SettingsSectionId>([
  // 产品语义：定时任务是 workspace 主视图，不能再作为设置页分区出现。
  // 注意：hooks 已是正式设置页分区，不在此列。
  "automations",
  // 独立插件市场已取消；旧 id 仅用于迁移历史偏好和调用，不能成为可见分区。
  "pluginStore",
  "plugins",
  // 工作区搜索（.knorviaignore）设置入口先隐藏：规则文件仍生效并可手动编辑，
  // 编辑页代码保留，放开时从这里移除即可。
  "workspaceFileSearch",
  "computerUse",
]);
...
export function resolveSettingsSection(
  ...
  if (section === "pluginStore" || section === "plugins") return "plugin";
...
function readLastSettingsSectionPreference(
  ...
  try {
    ...
    if (raw === "pluginStore" || raw === "plugins") {
...
export function setPendingSettingsSectionIntent(
  ...
  // 旧市场入口统一落到已安装管理，不能携带历史 MCP/Skill tab 显示成另一页面。
  const legacyPluginSection = section === "pluginStore" || section === "plugins";
  const targetSection = legacyPluginSection ? "plugin" : section;
  const pluginTab = legacyPluginSection ? "plugins" : options.pluginTab;

  try {
    window.sessionStorage.setItem(SETTINGS_SECTION_INTENT_KEY, targetSection);
    if (pluginTab) {
...
export function shouldFallbackSettingsUsageTabToApp({
  ...
  return (
    activeTab === "app" && !showCodingPlanTab && !loadingModelProviders && !checkingCodingPlanTab
  );
}
...
export function addPendingSettingsSectionListener(
  ...
  const handleIntent = (event: Event) => {
    ...
    if (detail?.section && isSettingsSectionId(detail.section)) {
      ...
      const legacyPluginSection = detail.section === "pluginStore" || detail.section === "plugins";
      const section = legacyPluginSection ? "plugin" : detail.section;
      listener(section, {
        ...detail,
        section,
        pluginTab: legacyPluginSection ? "plugins" : detail.pluginTab,
      });
    }
  };
