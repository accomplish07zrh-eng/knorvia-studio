import type { KnorviaTaskRuntimeStatus } from "@knorvia/shared";
import { getWorkspaceDisplayedTaskState } from "@/store/sessionStore.js";
import type { KnorviaSessionStoreState, WorkspaceKnorviaUIState } from "@/store/sessionStore.js";
...
interface MarkRemoteWorkspaceRunningTasksFailedParams {
  ...
  getWorkspaceState: KnorviaSessionStoreState["getWorkspaceState"];
  setTaskRuntimeState: KnorviaSessionStoreState["setTaskRuntimeState"];
  reason: string;
}

function isRunningRuntimeStatus(status: KnorviaTaskRuntimeStatus): boolean {
...
function collectRemoteWorkspaceRunningTaskIds(workspaceState: WorkspaceKnorviaUIState): string[] {
