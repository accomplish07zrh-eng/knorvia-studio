...
import type { ConversationTelemetryFact } from "@knorvia/shared/protocol-v4";
...
export interface ConversationPromptTelemetrySeed {
  localTtft?: import("@knorvia/shared").LocalTtftContext;
...
function toLegacyNetworkEvent(
  ...
  } as Extract<KnorviaStreamEvent, { type: "task_network_debug_status" }>;
}
...
function toToolLifecycleEvent(input: {
  ...
  if (!input.hasStarted && fact.phase !== "completed" && fact.phase !== "failed") {
    ...
    } as Extract<KnorviaStreamEvent, { type: "tool_call" }>;
  }
  return {
    ...
    raw: {},
    ...(skillMetadata ? { skillMetadata } : {}),
  } as Extract<KnorviaStreamEvent, { type: "tool_call_update" }>;
}

function toUsage(fact: Extract<ConversationTelemetryFact, { kind: "usage.delta" }>): KnorviaUsage {
  return {
...
function mergeUsage(left: KnorviaUsage | null, right: KnorviaUsage): KnorviaUsage {
...
export class ConversationTelemetrySupervisor {
  ...
  private processPromptFact(
    ...
    switch (fact.kind) {
      ...
      case "stream.chunk": {
        ...
        } as Extract<KnorviaStreamEvent, { type: "agent_thought_chunk" | "agent_message_chunk" }>;
  ...
  private handleTerminal(
    ...
    const terminalEvent = // 修复原因：旧 adapter 把所有 TurnComplete（包括 cancelled）先投影成
      ...
      ) as Extract<KnorviaStreamEvent, { type: "task_complete" | "task_error" }>;
    this.reportFinalizedSteps(
      lifecycle,
      recordAgentStepTelemetryEvent({
        taskId: lifecycle.taskKey,
  ...
  private handleCompaction(
    ...
    const timeline: KnorviaContextCompactionTimelineMeta = {
      version: 1,
      kind: "synthetic",
