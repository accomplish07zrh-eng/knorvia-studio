...
import { resolveWorkspaceTelemetryDetail, type IPlatformService } from "@knorvia/shared";
import { createConversationTelemetryService, type IServiceAccessor } from "@knorvia/services";
...
function acquireSupervisor(
  ...
  const key = attachmentScopeKey(scope, services.agentService);
...
function ensureSupervisorSubscription(
  ...
  const telemetryService = createConversationTelemetryService(services.agentService);
...
export function ConversationTelemetryWorkspaceAttachment({
  ...
  const lease = useMemo(() => {
    const agentService = services.agentService as object | null | undefined;
