...
// 复用 treemappingActivity 的既有解析规则，不复活 knorviaChatMessages。
//
...
import type { ConversationRow, ToolCallRow } from "@knorvia/shared/protocol-v4";
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(agentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [agentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
