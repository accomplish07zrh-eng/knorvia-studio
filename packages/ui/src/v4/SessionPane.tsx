...
import type { GroupedDraftTaskState } from "@/store/sessionStoreTypes.js";
...
export function SessionPane({
  ...
  const { modelSelectionService, sessionService, taskService } = useServices();
  ...
  const [sendSubmissionError, setSendSubmissionError] = useState<KnorviaUiError | null>(null);
  ...
  const workspaceConfigOptions = useKnorviaSessionStore(
  ...
  const draftRuntimeInvalidationVersion = useKnorviaSessionStore(
    (store) =>
  ...
  useEffect(() => {
    ...
    useKnorviaSessionStore.getState().invalidateDraftRuntime(workspacePath, workspaceIdentity);
  ...
  const promoteGroupedDraftTask = useKnorviaSessionStore((state) => state.promoteGroupedDraftTask);
  ...
  const theme = useKnorviaStoreWithDefault((state) => state.theme, "system");
  const codePreviewSettings = useKnorviaStoreWithDefault(
    ...
    async (reference?: ConversationSelectionReference, forceNew = false) => {
      ...
      try {
        ...
        if (targetChildSessionId) {
          try {
            await sessionService.readSession({
              workspacePath,
              ...(workspaceIdentity ? { workspaceIdentity } : {}),
  ...
  const handleOpenSelectionSideConversationWithPrompt = useCallback(
    async (text: string, telemetrySeed?: ConversationPromptTelemetrySeed): Promise<boolean> => {
      ...
      const chosen = inherited ? inherited : undefined;
  ...
  const dispatchSendText = useCallback(
    (text: string, options?: ConversationComposerSendOptions) => {
      const createSource = useKnorviaSessionStore
        .getState()
    ...
    async (
      ...
      try {
        ...
        setSendSubmissionError({
          code: runtimeModelUnavailable ? "KNORVIA_RUNTIME_MODEL_UNAVAILABLE" : "SEND_FAILED",
  ...
  const handleRecoverCustomModelSelection = useCallback(
    async (value: string, sourceModel: ModelSelectionSource | null) => {
      ...
      const displayProvider = provider ?? KNORVIA_AGENT_PROVIDER;
      ...
      try {
        await taskService.restartWorkspaceProcess({
          workspacePath,
          workspaceIdentity,
        ...
        const prepareResult = await prepareWorkspaceWithKnorviaSessionService({
          workspacePath,
          workspaceIdentity,
          provider: displayProvider,
          sessionService,
