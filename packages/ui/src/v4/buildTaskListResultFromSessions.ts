...
interface BuildTaskListParams {
  ...
  terminalStatusByTaskId?: ReadonlyMap<
    string,
    Extract<KnorviaTaskMeta["status"], "completed" | "error">
  >;
...
function mergeTaskIndexRowWithSession(
  taskIndexTask: KnorviaTaskMeta,
  sessionTask: KnorviaTaskMeta,
): KnorviaTaskMeta {
...
export function buildTaskListResult(params: BuildTaskListParams): BuildTaskListResult {
  ...
  filtered.sort((a, b) => compareKnorviaTaskListItems(a, b, params.sortBy));
