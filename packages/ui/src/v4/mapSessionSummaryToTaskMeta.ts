// sessions-index 的 SessionSummary → 侧栏实时 detail KnorviaTaskMeta 映射。
...
import type { TraceId, KnorviaProvider, KnorviaTaskMeta } from "@knorvia/shared";
import type { SessionSummary } from "@knorvia/shared/protocol-v4";
import {
...
function phaseToStatus(phase: SessionSummary["phase"]): KnorviaTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
/**
 * SessionSummary → 实时 detail KnorviaTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as KnorviaProvider } : {}),
      ...(status ? { status } : {}),
