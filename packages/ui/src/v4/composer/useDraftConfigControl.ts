...
import { KNORVIA_AGENT_PROVIDER, resolveExecutionState } from "@knorvia/shared";
...
import type { SessionConfigState } from "@knorvia/shared/protocol-v4";
import type { IModelSelectionService } from "@knorvia/services";
import { completeNewModelSelection } from "@knorvia/provider";
import {
  useModelSelectionServiceView,
  type ModelSelectionRead,
} from "@/hooks/useModelSelectionView.js";
import { submissionModeSchema } from "@knorvia/shared/protocol-v4";
...
function shouldHydrateWorkspaceCatalog(params: {
  ...
  slashCommands: readonly KnorviaSlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithKnorviaSessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useKnorviaSessionStore.getState();
