...
function buildV4ElicitationProgressKey(request: KnorviaElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useKnorviaSessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: KnorviaPermissionOption, feedback?: string) => {
