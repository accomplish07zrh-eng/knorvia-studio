...
export function nativeVisibleMessages(rows: readonly ConversationRow[], sessionId: string): StudioMessage[] {
  return rows.flatMap((row): StudioMessage[] => {
    ...
    const text = row.kind === "toolCall"
      ? ""
      : row.kind === "userInput" && row.epilogueStart !== undefined
        ? row.text.slice(0, row.epilogueStart)
        : row.text;
    return [{
      id: String(row.rowId), sequence: row.rowId, targetId: sessionId,
      runId: row.turnId, sender: row.kind === "userInput" ? "user" : "knorvia",
      kind, text,
...
export async function exportNativeConversationMarkdown(
  ...
  for (;;) {
    const page = await rowsRange({ sessionId, beforeRowId, limit: PROTOCOL_V4_LIMITS.rowsRangeMaxLimit });
    if (page.atLogEpoch !== snapshot.logEpoch ||
