 ...
 * 1. `/` 直接展示 Knorvia Agent 广播的真实 slash commands，并补充可用 subagents
...
export function SlashCommandPlugin({
  ...
  nativeCommands,
  nativeOnly = false,
}: SlashCommandPluginProps & { provider: KnorviaProvider }) {
  ...
  // 这里只按 workspacePath 读取会落到 path 桶，表现为 Knorvia Agent 已收到 available_commands_update 但 / 面板为空。
  const knorviaCommands = useSlashCommands(workspacePath, workspaceIdentity);
  const commands = nativeCommands ?? knorviaCommands;
  const {
    agents,
    loading: subagentsLoading,
    error: subagentsError,
  } = useSubagents(nativeOnly ? null : workspacePath, provider, workspaceIdentity);
  const {
    skills,
    ...
    enabled: !nativeOnly && !disabled && activeTrigger?.trigger === "/",
  });
  ...
  const commandSuggestions = useMemo(() => {
    ...
    const appSuggestions = buildAppSlashCommandSuggestions(
      nativeOnly ? [] : (appCommands ?? []),
    ).filter((item) => !cliValues.has(item.value) && !excluded.has(item.value));
    return [...cliSuggestions, ...appSuggestions];
  }, [appCommands, commands, excludedCommandNames, nativeOnly]);
  const subagentSuggestions = useMemo(
    () => (nativeOnly ? [] : buildSubagentSuggestions(agents)),
    [agents, nativeOnly],
  );
  const skillSuggestions = useMemo(
    ...
    [locale, provider, skills, nativeOnly],
  );
  ...
  const panelDescription = hasActiveQuery
    ...
    : nativeOnly
      ? locale.startsWith("zh")
        ? "输入以搜索当前 CLI 提供的命令"
        : "Type to search commands from this CLI"
      : intl.formatMessage({ id: "chat.slash.searchHint" });

  const allPanelSections = useSlashCommandMentionPanelSections(
    intl,
    commands.length,
    filteredCommandSuggestions,
    filteredSkillSuggestions,
    nativeOnly ? false : skillsLoading,
    nativeOnly ? null : (skillsError ?? null),
    filteredSubagentSuggestions,
    nativeOnly ? false : subagentsLoading,
    nativeOnly ? null : (subagentsError ?? null),
  );
  const panelSections = nativeOnly ? allPanelSections.slice(0, 1) : allPanelSections;
