...
import {
  KNORVIA_MODE_OPTION_DESCRIPTION_IDS,
  KNORVIA_MODE_OPTION_LABEL_IDS,
...
function isConfigSelectValueInOptions(option: KnorviaConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: KnorviaConfigOption): string | undefined {
...
function getConfigSelectItemTestId(
  ...
  entry: KnorviaConfigSelectValue,
): string | undefined {
  if (option.category === "thought_level") {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 Knorvia Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<KnorviaConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: KnorviaProvider | undefined,
  entry: Pick<KnorviaConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return KNORVIA_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return KNORVIA_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
