...
import { useWorkspaceShellState } from "@/app-shell/useWorkspaceShellState.js";
import {
...
import appLogoUrl from "@/assets/knorvia-logo.png";
...
import { getVisibleTaskMetas, useKnorviaSessionStore } from "@/store/sessionStore.js";
...
import type { GitChangeSourceId, WorkspacePurpose } from "@knorvia/shared";
...
export function App({
  ...
  const setWorkspaceMainView = useCallback(
    (view: WorkspaceMainView) => {
      if (view === "chat") showStudioTask();
      else navigateStudio({ view });
    },
    [navigateStudio, showStudioTask],
  );
  useEffect(
    () =>
      addStudioNewTaskListener(() => {
        if (!isWorkspaceVisible) return false;
        if (studioNavigation.route.chatMode === "groups") {
          studioNavigation.createGroup();
          return true;
        }
        if (studioNavigation.route.kernelId !== "knorvia") {
          studioNavigation.selectKernel(studioNavigation.route.kernelId);
          return true;
        }
        return false;
      }),
  ...
  const workspaceReadOnlyReason = workspaceReadOnly
    ...
    : undefined;
  const { workspaceShellKnorviaState, reloadSessionDisabled } = useWorkspaceShellState(
    workspaceAbsPath,
    workspaceIdentity,
  );
  const activeTaskId = workspaceShellKnorviaState.activeTaskId;
  ...
  const draftSessionId = useKnorviaSessionStore(
  ...
  const draftFocusVersion = workspaceShellKnorviaState.draftFocusVersion;
  const {
    isTerminalOpen,
    setIsTerminalOpen,
    sidePaneState,
  ...
  const notificationEnabled = useKnorviaStore((s) => s.notificationEnabled);
  ...
  useEffect(() => {
    ...
    // 菜单、设置中的插件提示词及原生快捷键共用草稿版本；新任务必须回到原聊天。
    showStudioTask();
  ...
  const theme = useKnorviaStore((s) => s.theme);
  const setTheme = useKnorviaStore((s) => s.setTheme);
  const {
    isMacFullscreen,
  ...
  const {
    ...
    workspaceRemoteSessionId,
    workspaceIdentity,
    selectedProvider: workspaceShellKnorviaState.selectedProvider,
    intl,
  });
  ...
  const handleCreateTaskIfWritable = useCallback(
    ...
    [onCreateTask, workspaceReadOnlyReason, showStudioTask],
  );
  ...
  const handleStartDraftInWorkspace = useCallback(
    (
      ...
      createSource?: import("@knorvia/shared").SessionCreateSource,
    ) => {
      const store = useKnorviaSessionStore.getState();
      ...
      const targetSelectedProvider = resolveWorkspaceSwitchDraftProvider({
        currentSelectedProvider: workspaceShellKnorviaState.selectedProvider,
        targetWorkspacePath,
    ...
    [
      ...
      workspaceAbsPath,
      workspaceIdentity,
      workspaceShellKnorviaState.selectedProvider,
    ],
  );
  ...
  const testActions = useMemo<TestActions>(
    () => ({
      ...
      getPluginReferenceCatalog: (params) =>
        services.agentService.getPluginReferenceCatalog(params),
    }),
    [locale, services.agentService, setLocale, theme, setTheme, testMessages],
  );
  ...
  > | null>(null);
  const handleNavigateToTaskMain = showStudioTask;
  const handleNavigateToAutomationsMain = useCallback(
    (target: AutomationsNavigationTarget) => {
  ...
  const handleNavigateToPluginStoreMain = useCallback(() => {
    // 独立市场已取消；旧历史和插件事件均进入唯一的插件管理页。
    consumePluginStoreOpenTarget();
    setPendingSettingsPluginIntent("plugins", { scopeKey: "user" });
    openSettingsTab();
  }, [openSettingsTab]);
  ...
  const handleOpenPluginStoreForScope = useCallback(
    (_target: PluginStoreOpenTarget = {}) => {
      handleNavigateToPluginStoreMain();
    },
    [handleNavigateToPluginStoreMain],
  );
  useEffect(
  ...
  const handleSelectAdjacentConversation = useCallback(
    (direction: "previous" | "next") => {
      runVisibleWorkspaceCommand(() => {
        const sessionState = useKnorviaSessionStore.getState();
  ...
  const handlePrimaryNavigationBack = studioNavigation.canGoBack
    ? studioNavigation.back
    : handleTaskNavBack;
  const canPrimaryNavigationBack = studioNavigation.canGoBack || canTaskNavBack;
  const handlePrimaryNavigationForward = studioNavigation.canGoForward
    ? studioNavigation.forward
    : handleTaskNavForward;
  const canPrimaryNavigationForward = studioNavigation.canGoForward || canTaskNavForward;
  ...
  useAppKeyboard({
    ...
    navigateBack: canPrimaryNavigationBack
      ...
      : null,
    navigateForward: canPrimaryNavigationForward
      ? () => runVisibleWorkspaceCommand(handlePrimaryNavigationForward)
      : null,
  });
  ...
  return (
    <>
      ...
      <WorkspaceShellLayout
        ...
        workspaceShellKnorviaState={workspaceShellKnorviaState}
        theme={theme}
        ...
        canTaskNavForward={canPrimaryNavigationForward}
        ...
        handleTaskNavBack={handlePrimaryNavigationBack}
        handleTaskNavForward={handlePrimaryNavigationForward}
