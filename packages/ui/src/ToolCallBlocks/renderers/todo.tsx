...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@knorvia/shared";
import type { KnorviaPlanStep } from "@knorvia/shared";
...
const todoTextClasses: Record<KnorviaPlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): KnorviaPlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: KnorviaPlanStep["status"] }) {
  if (status === "completed") {
