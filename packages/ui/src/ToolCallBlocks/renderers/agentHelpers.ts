...
type AgentIntl = ReturnType<typeof useKnorviaIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "knorvia", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "knorvia", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "knorvia", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "knorvia", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const knorvia = meta && isPlainRecord(meta.knorvia) ? meta.knorvia : null;
  const knorviaBackgroundAgent =
    knorvia && isPlainRecord(knorvia.backgroundAgent) ? knorvia.backgroundAgent : null;
  const taskNotification =
    knorvia && isPlainRecord(knorvia.taskNotification) ? knorvia.taskNotification : null;
  const backgroundAgent = knorviaBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "knorvia", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "knorvia", "taskNotification", "summary"]);
