...
import type { KnorviaPluginComponentKind, KnorviaPluginsDescribeResult } from "@knorvia/shared";
...
const SECTION_ORDER: KnorviaPluginComponentKind[] = ["mcp", "skill", "command", "agent", "hook"];

const SECTION_TITLE_IDS: Record<KnorviaPluginComponentKind, string> = {
...
const SECTION_ICONS: Record<KnorviaPluginComponentKind, typeof Server> = {
  mcp: Server,
  skill: WandSparkles,
  command: Terminal,
  agent: Bot,
...
export function PluginStoreDetailView({
  ...
  return (
    <div className="space-y-4" data-testid="plugin-store-detail" data-plugin-id={item.id}>
      ...
      <div className="space-y-3">
        ...
        >
          <div className="flex min-w-0 items-center gap-2">
            <h1 className="min-w-0 truncate text-ui-lg font-medium text-foreground">
              {displayName}
            </h1>
          </div>
      ...
      {/* 组件分区：已安装走权威枚举（KnorviaPluginInfo.components），候选走 plugins/describe 按需拉取。 */}
      {componentsLoading ? (
        <div
...
function InfoSection({
  ...
  item: StorePluginItem;
  describeMetadata: KnorviaPluginsDescribeResult["metadata"];
}) {
  const { intl } = useKnorviaIntl();
