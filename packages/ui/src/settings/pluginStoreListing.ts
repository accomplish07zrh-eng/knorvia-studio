...
export function resolveUniquePluginListingByName(
  plugins: readonly Pick<KnorviaAvailablePluginSummary, "name" | "listing">[],
  name: string,
): KnorviaPluginStoreListing | undefined {
  const normalizedName = name.trim().toLocaleLowerCase();
  let match: KnorviaPluginStoreListing | undefined;
  let count = 0;
...
export interface StorePluginItem {
  ...
  orphaned: boolean;
  listing?: KnorviaPluginStoreListing;
  summary?: KnorviaAvailablePluginSummary;
  /** 运行时信息（仅已发现的已安装插件有）：启用态、组件、manifest 回退字段。 */
  info?: KnorviaPluginInfo;
  installedMeta?: KnorviaInstalledPluginSummary;
}

export type PluginUpdateStatus = NonNullable<KnorviaInstalledPluginSummary["updateStatus"]>;
...
const OFFICIAL_MARKETPLACE_ORDER: readonly string[] = [KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE_ID];
...
export function sortMarketplaceSources(
  marketplaces: readonly KnorviaPluginMarketplaceSummary[],
  locale: string,
): KnorviaPluginMarketplaceSummary[] {
...
export function buildStoreItems(input: {
  marketplaces: KnorviaPluginMarketplaceSummary[];
  marketplaceAvailabilityKnown: boolean;
  availablePlugins: KnorviaAvailablePluginSummary[];
  installedPlugins: KnorviaInstalledPluginSummary[];
  plugins: KnorviaPluginInfo[];
  restorableBuiltins: KnorviaAvailablePluginSummary[];
}): StorePluginItem[] {
