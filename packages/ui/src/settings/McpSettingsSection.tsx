 ...
 * Supports the unified Knorvia Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToKnorviaAgentMcpServer,
...
const DEFAULT_MCP_SOURCE: ServerScope = "knorviaagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 agentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isKnorviaAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "knorviaagentmcp" || !server.enabled) {
      continue;
    }
    ...
    result.push(agentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: KnorviaAgentMcpServer[]): {
  mode: "status";
  mcpServers: KnorviaAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: KnorviaMcpServer[]): string {
  return servers
    .filter((server) => server.source === "knorviaagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: KnorviaPluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, KnorviaAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<KnorviaMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForKnorvia("knorvia");
  ...
  async function handleSave(form: FormState, prev?: KnorviaMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: KnorviaMcpServer) {
  ...
  function handleEdit(server: KnorviaMcpServer) {
  ...
  function handleOpenAuthorization(server: KnorviaMcpServer) {
