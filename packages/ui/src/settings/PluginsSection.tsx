...
import { PluginSourcesDialog } from "@/settings/PluginSourcesDialog.js";
...
function PluginList({
  ...
  const getPluginOptionValue = (
    ...
    key: string,
    option: KnorviaPluginUserConfigOption,
  ...
  const savePluginOptions = async (plugin: KnorviaPluginInfo) => {
  ...
  if (selectedPlugin && selectedStoreItem && targetServiceResolution.rpcReady) {
    ...
    // 已安装详情继续复用原组件和配置操作；返回唯一的插件管理列表，不恢复市场路由。
    return (
  ...
  return (
    <section className="space-y-6" aria-labelledby="plugin-installed-title">
            ...
            {configScope === "user" ? (
              <>
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  data-testid="plugin-settings-sources"
                  disabled={!targetServiceResolution.rpcReady}
                  onClick={() => setSourcesOpen(true)}
                >
                  <FolderOpen className="size-3.5" aria-hidden="true" />
                  {intl.formatMessage({ id: "settings.plugins.sources.button" })}
                </Button>
                <PluginAddMenu testId="plugin-settings-add" onCreateTask={onCreateTask} />
              </>
            ) : null}
          </div>
        </div>
      ...
      <RemoteSyncDialogs
        ...
        remoteKnorviaAgentService={targetServiceResolution.services.agentService}
