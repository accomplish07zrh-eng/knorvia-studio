import type { KnorviaCommand, KnorviaSlashCommand } from "@knorvia/shared";
...
function commandToSlashCommand(command: KnorviaCommand): KnorviaSlashCommand | null {
  if (!command.enabled) {
    return null;
  }
...
export function mergeSlashCommandsAfterCommandRefresh(
  currentSlashCommands: readonly KnorviaSlashCommand[],
  refreshedCommands: readonly KnorviaCommand[],
): KnorviaSlashCommand[] {
  ...
  const customCommands: KnorviaSlashCommand[] = [];
