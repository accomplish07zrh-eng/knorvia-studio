...
import {
  ...
  type KnorviaAutomationScheduleRule,
} from "@knorvia/shared";
import {
  ArrowLeft,
  ArrowRight,
...
interface AutomationEditViewProps {
  /** null = 新建；否则编辑。 */
  editing: KnorviaAutomation | null;
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: KnorviaAutomation) => Promise<void> | void;
...
function normalizeAutomationScheduleRule(
  rule: KnorviaAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: KNORVIA_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<KnorviaAutomationScheduleRule | undefined>(
    () =>
