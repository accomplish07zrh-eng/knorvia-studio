...
import type { Hook, HookConfig, KnorviaInstalledPluginSummary, KnorviaPluginInfo } from "@knorvia/shared";
import { Button } from "@/components/ui/button.js";
import { toast } from "@/components/ui/toast.js";
import { useConfirmDialog } from "@/hooks/useConfirmDialog.js";
import { useWorkspaceServicesResolution } from "@/hooks/useWorkspaceServices.js";
import { useSessionService } from "@/hooks/useSessionService.js";
import { useKnorviaIntl } from "@/i18n/IntlProvider.js";
import { invalidateDeferredDraftSessionForRuntimeChange } from "@/lib/draftSkillInvalidation.js";
...
function isEditableHook(hook: Hook): boolean {
  ...
  return hook.editable ?? (!hook.location || hook.location.source === "knorvia");
}

// workspace-hook-trust：editable=false 且 source=knorvia 的行是「上游/祖先 knorvia.json
// 里的只读工作区 Hook」。它们不是外部格式兼容导入源，塞进 Legacy 会让 Import 按钮
// 必然失败（importHook 拒绝 source=knorvia），也违反「只读但可逐条 Trust」的约定。
// 这类行应留在 Installed 分组，由信任状态门控 Switch，走行内 Trust 流程。
function isReadOnlyKnorviaHook(hook: Hook): boolean {
  return hook.editable === false && (hook.location?.source ?? "knorvia") === "knorvia";
}

function isInCompatibilitySection(hook: Hook): boolean {
  return !isEditableHook(hook) && !isReadOnlyKnorviaHook(hook);
}
...
function buildPluginHookRows(
  plugins: readonly Pick<KnorviaPluginInfo, "enabled" | "hookDetails" | "id" | "name">[],
  installedPlugins: readonly Pick<KnorviaInstalledPluginSummary, "id" | "scope">[],
...
export function HooksSection({ workspacePath, workspaceIdentity }: HooksSectionProps) {
  ...
  const editableHooks = useMemo(
    () =>
      hooksState.hooks.filter(
        (hook) =>
          (isEditableHook(hook) || isReadOnlyKnorviaHook(hook)) &&
  ...
  const invalidateDraft = useCallback(
    ...
    [targetWorkspaceIdentity, targetWorkspacePath, sessionService],
  );
