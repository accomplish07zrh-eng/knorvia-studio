import type {
  ...
  KnorviaPluginInfo,
  KnorviaPluginComponentKind,
  KnorviaPluginsDescribeResult,
} from "@knorvia/shared";
import { isPluginCommand, isUserCommand, KNORVIA_COMMAND_AGENT_SOURCE } from "@knorvia/shared";
...
const COMPONENT_KIND_ORDER: KnorviaPluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: KnorviaPluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<KnorviaPluginComponentKind, PluginComponentDisplayGroup>();
...
export interface PluginMcpServerItem {
  ...
  error?: string;
  failureKind?: KnorviaMcpServerStatusSnapshot["failureKind"];
  id: string;
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === KNORVIA_COMMAND_AGENT_SOURCE &&
...
function toPluginMcpServerDisplayName(plugin: KnorviaPluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: KnorviaPluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: KnorviaMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
