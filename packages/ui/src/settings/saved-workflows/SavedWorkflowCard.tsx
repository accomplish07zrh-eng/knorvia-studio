...
function SavedWorkflowLastRunBadge({
  ...
  run: KnorviaSavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: KnorviaSavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: KnorviaSavedWorkflowEntry) => void;
  onRun: (entry: KnorviaSavedWorkflowEntry) => void;
  onRevise: (entry: KnorviaSavedWorkflowEntry) => void;
  onCopyPath: (entry: KnorviaSavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: KnorviaSavedWorkflowEntry) => void;
  onDelete: (entry: KnorviaSavedWorkflowEntry) => void;
}
