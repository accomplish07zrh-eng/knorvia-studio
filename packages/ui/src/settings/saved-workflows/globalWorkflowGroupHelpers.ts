...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type KnorviaSavedWorkflowRun } from "@knorvia/shared";
import type { KnorviaAgentSavedWorkflowTarget } from "@knorvia/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: KnorviaAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: KnorviaSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
