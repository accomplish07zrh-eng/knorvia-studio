...
import {
  ...
  type KnorviaSavedWorkflowEntry,
  type KnorviaSavedWorkflowMeta,
  type KnorviaSavedWorkflowRun,
  type KnorviaWorkflowsGetResult,
} from "@knorvia/shared";
import type { IKnorviaAgentService, KnorviaAgentSavedWorkflowTarget } from "@knorvia/services";
...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<KnorviaWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: KnorviaSavedWorkflowMeta): MetaDraft {
  return {
...
interface SavedWorkflowDetailViewProps {
  ...
  target: KnorviaAgentSavedWorkflowTarget;
  agentService: IKnorviaAgentService;
  name: string;
  /** 面包屑「自动化 › <项目名> › <工作流名>」里的项目一级。 */
  projectLabel: string;
  entry: KnorviaSavedWorkflowEntry | undefined;
  runs: readonly KnorviaSavedWorkflowRun[];
  now: number;
  busy: boolean;
  canOpenRun: boolean;
  onBack: () => void;
  ...
  onMove?: () => void;
  onDelete: () => void;
  onOpenRun: (run: KnorviaSavedWorkflowRun) => void;
  /** 产物 chip → `workflow-artifact` tab；缺席即 chips 只读。 */
  onOpenArtifact?: (run: KnorviaSavedWorkflowRun, artifactId: string) => void;
  onMetaSaved: () => void;
  /** 传入即在运行历史每行渲染项目列（全局工作流跨项目历史）。 */
  resolveRunProject?: (run: KnorviaSavedWorkflowRun) => SavedWorkflowRunProject | null;
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: KnorviaSavedWorkflowMeta = {
      description,
