...
import type {
  ...
  RemoteTarget,
  KnorviaPluginOptionValue,
  KnorviaPluginInfo,
  KnorviaPluginMarketplaceSummary,
  KnorviaPluginUserConfigOption,
  KnorviaPluginsOverviewResult,
} from "@knorvia/shared";
import type { IPluginSyncService, IKnorviaAgentService } from "@knorvia/services";
...
interface RemotePluginSyncPluginOptions {
  configuredOptions: Record<string, KnorviaPluginOptionValue>;
  userConfig: Record<string, KnorviaPluginUserConfigOption>;
}
...
interface RemotePluginSyncDialogProps {
  ...
  localKnorviaAgentService?: RemotePluginSyncAgentService | null;
  remoteKnorviaAgentService?: RemotePluginSyncAgentService | null;
...
function buildMarketplaceRemotePluginSyncCandidates(
  overview: KnorviaPluginsOverviewResult,
...
function buildMarketplaceRemotePluginStatuses(
  candidates: readonly RemotePluginSyncCandidate[],
  remoteOverview: KnorviaPluginsOverviewResult,
...
function componentTypesFromPluginInfo(plugin: KnorviaPluginInfo | undefined): string[] {
  return plugin?.components?.map((component) => component.kind) ?? [];
}

function buildRemotePluginSyncPluginOptions(
  plugin: KnorviaPluginInfo | undefined,
...
function serializeMarketplaceSourceInput(
  marketplace: KnorviaPluginMarketplaceSummary,
): string | undefined {
...
async function loadRemotePluginSyncCandidates(params: {
  ...
  if (!params.localKnorviaAgentService || !params.remoteKnorviaAgentService) {
  ...
  const [localOverview, localPluginsResult, remoteOverview] = await Promise.all([
    params.localKnorviaAgentService.getPluginsOverview({
      workspacePath: params.localWorkspacePath,
    }),
    params.localKnorviaAgentService.listPlugins({
      workspacePath: params.localWorkspacePath,
    }),
    params.remoteKnorviaAgentService.getPluginsOverview({
...
function isPathPluginOption(option: KnorviaPluginUserConfigOption | undefined): boolean {
  return option?.type === "file" || option?.type === "directory";
}

function isPortablePluginOptionValue(
  value: KnorviaPluginOptionValue,
  localOption: KnorviaPluginUserConfigOption | undefined,
  remoteOption: KnorviaPluginUserConfigOption | undefined,
): boolean {
...
function buildPortablePluginOptionsSyncPlan(
  ...
  remotePlugin: KnorviaPluginInfo,
): { options: Record<string, KnorviaPluginOptionValue>; skippedCount: number } {
  const options: Record<string, KnorviaPluginOptionValue> = {};
  let skippedCount = 0;
...
function arePluginOptionsEqual(
  left: Record<string, KnorviaPluginOptionValue>,
  right: Record<string, KnorviaPluginOptionValue>,
): boolean {
...
async function syncPortablePluginOptions(
  ...
  const remoteKnorviaAgentService = params.remoteKnorviaAgentService;
  if (!remoteKnorviaAgentService) {
    emitRemotePluginSyncProgress(
      params,
      row,
      "syncing",
  ...
  const remotePlugins = await awaitRemotePluginSyncStep(
    ...
    () => remoteKnorviaAgentService.listPlugins(workspace),
  );
  ...
  await awaitRemotePluginSyncStep(
    ...
    () =>
      remoteKnorviaAgentService.configurePlugin({
        ...workspace,
...
async function syncMarketplaceRemotePlugin(
  ...
  try {
    throwIfRemotePluginSyncStopped(params, row);
    if (!params.remoteKnorviaAgentService || !marketplacePlugin) {
    ...
    const installResult = await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        remoteKnorviaAgentService.installPlugin({
          ...workspace,
    ...
    await awaitRemotePluginSyncStep(
      ...
      () =>
        remoteKnorviaAgentService.setPluginEnabled({
          ...workspace,
          enabled: candidate.enabled,
          pluginId: candidate.pluginId,
          scope: REMOTE_PLUGIN_SYNC_CONFIG_SCOPE,
...
async function prepareRemoteMarketplaceSource(
  params: {
    ...
    onItemProgress?: (event: RemotePluginSyncProgressEvent) => void;
    remotePluginSyncService: IPluginSyncService;
    remoteKnorviaAgentService: RemotePluginSyncAgentService;
    selectedMarketplacePluginNames: ReadonlyMap<string, string[]>;
    signal?: AbortSignal;
    stopControl?: RemotePluginSyncStopControl;
  },
  ...
  if (marketplaceSourceInput) {
    await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        params.remoteKnorviaAgentService.addPluginMarketplace({
          ...workspace,
          operationId,
      ...
      const result = await syncSelectedRemotePlugins({
        ...
        stopControl: {
          cancelOperation: async (operationId) => {
            if (!remoteKnorviaAgentService?.cancelPluginOperation) {
              return;
            }
            await remoteKnorviaAgentService.cancelPluginOperation({ operationId });
          },
