...
function visible(message: StudioMessage): boolean {
  return message.sender !== "system" &&
    (message.kind === "tool" || message.kind === "text");
}
...
export function buildStudioHandoffDraft(messages: readonly StudioMessage[], sourceName: string, zh = true) {
...
export async function performStudioHandoff(
  ...
  if (attempt.targetKernel === "knorvia" || attempt.targetKernel.startsWith("ssh:") ||
...
function messageMarkdown(message: StudioMessage, kernelName: string): string {
  const when = Number.isFinite(message.createdAt) && Math.abs(message.createdAt) <= 8.64e15
...
export function formatHandoffMarkdown(
  ...
  const ordered = messages.filter(visible).sort((a, b) =>
...
export function studioExportFileName(title: string): string {
  const base = title.replace(/[<>:"/\\|?*]/g, "-").replace(/\p{Cc}/gu, "-").replace(/[. ]+$/g, "").slice(0, 80);
