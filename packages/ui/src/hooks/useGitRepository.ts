...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, KnorviaPersistedFileSnapshot>,
  file: KnorviaTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: KnorviaPersistedFileChange | null;
  summary: KnorviaTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, KnorviaPersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 Knorvia Agent 已持久化的单轮文件快照，
