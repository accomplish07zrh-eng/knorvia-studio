...
function areResolvedTaskMetasEqual(left: KnorviaTaskMeta | null, right: KnorviaTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: KnorviaTaskMeta | null) {
  const stableTaskMetaRef = useRef<KnorviaTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useKnorviaSessionStore((state) =>
    selectWorkspaceKnorviaState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 taskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task
