...
interface PluginReferenceCatalogState {
  entries: KnorviaPluginReferenceCatalogEntry[];
...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<KnorviaPluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<KnorviaPluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<KnorviaPluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    const subscription = services.agentService.onAgentRuntimeRestarted((event) => {
  ...
  useEffect(() => {
    ...
    let request: Promise<KnorviaPluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<KnorviaPluginsReferenceCatalogResult>> | undefined;
