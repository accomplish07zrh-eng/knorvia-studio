...
import type { IPlatformService, TaskNotificationPayload } from "@knorvia/shared";
import type { ConversationSnapshot, SessionSummary } from "@knorvia/shared/protocol-v4";
...
export function useWorkspaceTerminalTaskNotifications({
  ...
  useEffect(() => {
    ...
    const store = acquireSessionsIndex(registryScope, agentService);
    const syncState = () => {
      setIndexState({
        signature,
