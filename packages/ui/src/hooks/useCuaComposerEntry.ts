...
import { isRemoteWorkspaceIdentity, KNORVIA_CUA_OFFICIAL_PLUGIN_ID } from "@knorvia/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/sessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 sessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useKnorviaSessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === KNORVIA_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === KNORVIA_CUA_OFFICIAL_PLUGIN_ID,
