 ...
 * （见 sessionSettingsToConfigOptions），那种「名字」在这里就当作没有——屏幕上绝不出现
 * providerId（团队套餐的它是一个 UUID）。
 */
function workflowSubagentProviderNameLookup(
  configOptions: readonly KnorviaConfigOption[] | null | undefined,
...
export function useWorkflowSubagentModelProviderName(
  ...
  const configOptions = useKnorviaSessionStore((state) =>
    workspacePath === undefined
      ? undefined
      : selectWorkspaceKnorviaState(state, workspacePath, workspaceIdentity).configOptions,
  );
