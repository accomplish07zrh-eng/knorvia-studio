...
import type { KnorviaSessionEndedSubagent, KnorviaSessionSubagentsResult } from "@knorvia/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: KnorviaSessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly KnorviaSessionEndedSubagent[],
  incoming: readonly KnorviaSessionEndedSubagent[],
): KnorviaSessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<KnorviaSessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof agentService?.listSessionSubagents !== "function") {
      ...
      return agentService.listSessionSubagents({
