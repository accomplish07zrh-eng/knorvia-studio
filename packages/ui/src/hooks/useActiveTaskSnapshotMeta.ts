...
import { sessionSnapshotToTaskMeta } from "@/lib/sessionProjection.js";

function resolveImmediateActiveTaskSnapshotMeta(
  previousSnapshotMeta: KnorviaTaskMeta | null,
  taskId: string | null,
  taskMetaFromLists?: KnorviaTaskMeta | null,
) {
...
export function useActiveTaskSnapshotMeta(
  ...
  const [snapshotMeta, setSnapshotMeta] = useState<KnorviaTaskMeta | null>(null);
  ...
  useEffect(() => {
    ...
    void sessionService
      // active header 只需要 session meta/标题兜底，走 Knorvia Protocol 的轻量读取，
      ...
      .then((snapshot) => {
        ...
        setSnapshotMeta(sessionSnapshotToTaskMeta(snapshot));
      })
      .catch(() => {
        if (cancelled) {
          return;
  ...
  }, [sessionService, taskId, taskMetaFromLists, workspaceIdentity, workspacePath]);
