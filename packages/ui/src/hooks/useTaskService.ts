...
type GetTaskSnapshotParams = Parameters<IKnorviaTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<KnorviaTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IKnorviaTaskService["getTaskSnapshotWithEtag"]>[0];

const taskServiceProxyCache = new WeakMap<IKnorviaTaskService, IKnorviaTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IKnorviaTaskService,
  Map<string, { etag: string; snapshot: KnorviaTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "knorvia-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: KnorviaTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: KnorviaTaskSnapshot }>();
...
function writePersistedSnapshotEntry(key: string, etag: string, snapshot: KnorviaTaskSnapshot): void {
  ensurePersistedSnapshotCacheLoaded();
...
function createKnorviaTaskServiceProxy(service: IKnorviaTaskService): IKnorviaTaskService {
...
export function useTaskService(
  ...
  const cachedProxy = taskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createKnorviaTaskServiceProxy(rawService);
  taskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
