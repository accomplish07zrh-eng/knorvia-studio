...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly KnorviaTaskMeta[];
  optimisticTasks: readonly KnorviaTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareKnorviaTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useKnorviaSessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useKnorviaSessionStore.getState();
