...
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@knorvia/shared";
...
export function useAppPanels(options: {
  ...
  const { agentService, sessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await agentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,
