...
function supportsTaskNativeSessionLogFile(_provider: KnorviaProvider | null | undefined): boolean {
 ...
 *
 * 路径规则统一通过 taskService 解析，避免 UI 层猜 provider 自己的目录结构。
 */
export function useTaskNativeSessionLogFile(
  workspacePath: string,
  taskId: string | null,
  providerHint?: KnorviaProvider | null,
  workspaceIdentity?: string,
  options: { enabled?: boolean } = {},
) {
  const taskService = useTaskService(workspacePath, undefined, workspaceIdentity);
  ...
  }, [enabled, taskService, providerHint, taskId, workspaceIdentity, workspacePath]);
