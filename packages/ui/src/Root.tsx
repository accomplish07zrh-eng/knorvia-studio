...
import { reportUiLaunchToInput, setUiPerfArmsReporter } from "@/lib/uiPerfArmsTelemetry.js";
...
import { StoreProvider, useKnorviaStore } from "@/store/StoreProvider.js";
...
function RootInner({
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel === APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL) {
        ...
        void services.agentService.syncAppRuntimePreferences(parsed.data).catch((error) => {
  ...
  }, [refreshAppSettings, services.broadcastService, services.agentService]);
  ...
  const totalUnreadTaskCount = useKnorviaSessionStore((state) =>
  ...
  // 这里把 provider 启动检查纳入 workspace 恢复门禁，避免未连接账号前启动 Knorvia session。
  useEffect(() => {
  ...
  const isStartupRenderBlocked = Boolean(
    isDesktop && (isRestoring || isBootstrappingInitialWorkspace || isCreatingFallbackWorkspace),
  );
  ...
  const settingsLayerProps = {
    ...
    onOpenRemoteWorkspace: allowRemoteWorkspace ? handleOpenRemoteConnection : undefined,
    allowOpenWorkspace,
  };
  ...
  if (isStartupRenderBlocked) {
    ...
    return (
      <RootShell>
        ...
        {/* HTML 启动壳已经展示 Knorvia SVG，但 React 接管 root 后旧壳会被整棵替换。
