...
export function useWorkspaceTaskNavigation({
  ...
  const taskNavPushAutomations = useKnorviaSessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useKnorviaSessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useKnorviaSessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useKnorviaSessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useKnorviaSessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useKnorviaSessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.taskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    const currentWorkspaceState = useKnorviaSessionStore
      .getState()
    ...
    while (entry) {
      ...
      const navWorkspaceState = useKnorviaSessionStore
        .getState()
  ...
  const currentWorkspaceState = useKnorviaSessionStore.getState().getWorkspaceState(workspaceAbsPath);
