...
import { TID_APP_HEADER } from "@knorvia/shared";
...
import { StudioConversationSwitch } from "@/studio/StudioConversationSwitch.js";
import { StudioPageFrame } from "@/studio/StudioPageFrame.js";
...
import type { ComposerMentionPrefill } from "@/store/sessionStoreTypes.js";
import {
...
const WORKSPACE_SIDEBAR_WIDTH_STORAGE_KEY = "knorvia:workspace-shell:sidebar-width-px";
...
export const WorkspaceShellLayout = memo(function WorkspaceShellLayoutComponent({
  ...
  usePaneSessionPersistence({
    ...
    draftFocusVersion: workspaceShellKnorviaState.draftFocusVersion,
  ...
  const {
    ...
    open: isSingleChat && isTerminalOpen,
    expandedSize: "30%",
  ...
  const {
    ...
    open: isSingleChat && isSidePaneOpen,
  ...
  const searchShortcutLabel = useShortcutCommandLabel("openCommandCenter");
  ...
  const handleCreateStudioTask = useCallback(() => {
    ...
    else if (studioNavigation.route.kernelId !== "knorvia")
      studioNavigation.selectKernel(studioNavigation.route.kernelId);
    else {
      showChatMainView();
      onCreateTask();
    }
  }, [studioNavigation, onCreateTask, showChatMainView]);
    ...
    (chatMode: "single" | "groups") => {
      studioNavigation.navigate({
        ...
        view:
          chatMode === "groups"
            ? "groups"
            : studioNavigation.route.kernelId === "knorvia"
              ? "chat"
              : "external-chat",
      });
    },
  ...
  const handleStartDraftInWorkspaceInChat = useCallback(
    (
      targetWorkspacePath: string,
      targetWorkspaceIdentity?: string,
      targetWorkspacePurpose?: import("@knorvia/shared").WorkspacePurpose,
  ...
  const shouldRenderMainViewHeader = isSingleChat;
              ...
              >
                <WorkflowRunOpenProvider onOpenRun={handleOpenSidebarWorkflowRun}>
                  <WorkspaceSidebar
                    ...
                    onOpenWorkflows={() => onWorkspaceMainViewChange("workflows")}
                    workflowsActive={workspaceMainView === "workflows"}
                    onOpenCreation={() => onWorkspaceMainViewChange("creation")}
                    creationActive={workspaceMainView === "creation"}
                    onCreateStudioTask={
                      studioNavigation.route.chatMode === "groups" ||
                      studioNavigation.route.kernelId !== "knorvia"
                        ? handleCreateStudioTask
                        : undefined
                    }
                    ...
                    studioTaskList={
                      studioNavigation.route.chatMode === "groups" ? (
                        <StudioGroupList
                          selectedGroupId={studioNavigation.route.groupId}
                          onSelectGroup={studioNavigation.selectGroup}
                          onCreateGroup={studioNavigation.createGroup}
                        />
                      ) : studioNavigation.route.kernelId !== "knorvia" ? (
                        <StudioExternalDraftList
              ...
              >
                ...
                >
                  ...
                  >
                    {shouldRenderWorkspaceHeader ? (
                      ...
                      >
                        <WorkspaceHeader
                          ...
                          variant={isExternalChat || activeTaskId === null ? "draft" : "task"}
                          draftDropTargetController={
                            !isExternalChat && activeTaskId === null
                          ...
                          activeTaskTitle={isExternalChat ? "" : activeTaskTitle}
                          activeTaskChangeSummary={isExternalChat ? null : activeTaskChangeSummary}
                          hasUpdateReady={false}
                          activeTaskId={isExternalChat ? null : activeTaskId}
                          activeTraceId={isExternalChat ? null : activeTraceId}
                          activeSessionId={isExternalChat ? null : activeSessionId}
                          activeTaskProvider={isExternalChat ? null : activeTaskProvider}
                          resolvedActiveTaskMeta={isExternalChat ? null : resolvedActiveTaskMeta}
                          ...
                          workspaceHeaderState={workspaceShellKnorviaState}
                          gitSummary={gitState.summary}
                        ...
                        >
                          ...
                          >
                            <Suspense
                              fallback={
                                <div
                                  className="flex h-full items-center justify-center text-ui-sm text-foreground-subtle"
                                  role="status"
                                >
                            ...
                            >
                              {workspaceMainView === "workflows" ? (
                                ...
                                <StudioGroupsPage
                                  groupId={studioNavigation.route.groupId}
                                  onSelectGroup={studioNavigation.selectGroup}
                                  onOpenAgentSettings={openAgentSettings}
                                  createRequest={studioNavigation.createGroupRequest}
                                  onCreateRequestHandled={studioNavigation.consumeCreateGroup}
                                />
                              )}
                            </Suspense>
                          </ScopedErrorBoundary>
                        ...
                        <main className="relative flex h-full min-h-0 flex-1 flex-col overflow-hidden">
                          ...
                          >
                            <Suspense fallback={null}>
                              <StudioExternalChat
                                key={`${studioNavigation.route.kernelId}:${studioNavigation.route.externalSessionId}`}
                                kernelId={studioNavigation.route.kernelId}
                    ...
                    {isWorkspaceVisible ? <KnorviaCompanion /> : null}
                  </section>
                </ResizablePanel>
                {isSingleChat ? (
