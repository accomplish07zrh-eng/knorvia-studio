...
export function useWorkspaceSessionReload({
  ...
  const handleReloadSession = useCallback(
    async (options?: { resumeTaskId?: string | null; provider?: KnorviaProvider | null }) => {
      ...
      const sessionStore = useKnorviaSessionStore.getState();
      const latestWorkspaceState = sessionStore.getWorkspaceState(
        workspaceAbsPath,
        workspaceIdentity,
      );
      const actionScope = resolveWorkspaceModelConfigSyncScope(latestWorkspaceState);
      const provider: KnorviaProvider = options?.provider ?? actionScope.provider;
      const resumeTaskId =
      ...
      if (shouldPrepareWorkspace) {
        ...
        sessionStore.setDraftError(workspaceAbsPath, null, workspaceIdentity);
        sessionStore.setTaskState(workspaceAbsPath, "idle", null, workspaceIdentity);
      }

      try {
        await services.taskService.restartWorkspaceProcess({
        ...
        if (shouldPrepareWorkspace) {
          ...
          const latestAfterPrepare = sessionStore.getWorkspaceState(
            workspaceAbsPath,
            workspaceIdentity,
          );
          if (latestAfterPrepare.selectedProvider === provider) {
            const latestAfterResolve = sessionStore.getWorkspaceState(
              workspaceAbsPath,
              workspaceIdentity,
            );
            if (latestAfterResolve.selectedProvider === provider) {
              sessionStore.setConfigOptions(
                workspaceAbsPath,
        ...
        sessionStore.setWorkspaceInitAttempts(workspaceAbsPath, 0, workspaceIdentity);
        sessionStore.setWorkspaceInitState(workspaceAbsPath, "ready", null, workspaceIdentity);
        ...
        if (shouldPrepareWorkspace) {
          ...
          sessionStore.setDraftError(workspaceAbsPath, reloadDraftError, workspaceIdentity);
        }
