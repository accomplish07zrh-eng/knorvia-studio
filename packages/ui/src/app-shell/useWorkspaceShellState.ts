...
type WorkspaceShellKnorviaState = Pick<
...
export function useWorkspaceShellState(workspaceAbsPath: string, workspaceIdentity?: string) {
  // App 之前直接订阅整个 workspaceKnorviaState，streaming 每个 chunk 都会改 taskMessagesByTaskId。
  ...
  const reloadSessionDisabled = useKnorviaSessionStore((state) => {
    const workspaceState = selectWorkspaceKnorviaState(state, workspaceAbsPath, workspaceIdentity);
  ...
  return { workspaceShellKnorviaState, reloadSessionDisabled };
}
