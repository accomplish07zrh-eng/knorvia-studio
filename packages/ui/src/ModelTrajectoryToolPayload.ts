import type { KnorviaModelTrajectoryMessage } from "@knorvia/services";

export function trajectoryToolOutputs(message: KnorviaModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: KnorviaModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: KnorviaModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: KnorviaModelTrajectoryMessage): {
  names: string;
  ids: string;
} {
