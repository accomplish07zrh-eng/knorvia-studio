...
import { convertToKnorviaAgentMcpServer } from "@knorvia/shared";
...
import {
  importLegacyCommonServersToKnorviaAgent,
  migrateStoredCommonMcpToKnorviaAgent,
...
interface McpStoreState {
  ...
  statusSnapshots: Record<string, KnorviaMcpServerStatusSnapshot>;
  ...
  beginServerStatusListRefresh: (mode?: KnorviaMcpListMode) => number;
  ...
  mergeServerStatusSnapshots: (
    statuses: Record<string, KnorviaMcpServerStatusSnapshot>,
    requestEpoch: number,
    mode?: "connect" | "status",
  ) => void;
  getServer: (id: string) => KnorviaMcpServer | undefined;
  setCurrentProjectPath: (
    path: string,
    workspaceIdentity?: string,
    directoryService?: McpDirectoryService | null,
  ) => void;
  getEnabledMcpServersForKnorvia: (provider: string) => KnorviaAgentMcpServer[];
...
export const useMcpStore = create<McpStoreState>((set, get) => {
  ...
  const statusListEpochs: Record<KnorviaMcpListMode, number> = {
    connect: 0,
    status: 0,
  };
  ...
  function resolveMcpDirectoryService(
    ...
    if (!effectiveWorkspaceIdentity?.trim()) {
      ...
      // 到用户级 Knorvia Agent MCP 的迁移；目录服务只用于远端 workspace 覆盖路由。
      return null;
    }
  ...
  return {
    ...
    loadConfig: () => {
      ...
      // MCP 启停状态已经迁移到 ~/.knorvia-studio/cli/config.json，不能再读取旧 localStorage，
      // 否则旧的本地开关会覆盖新的 Knorvia Agent 配置来源。
      ...
      loadMcpPromise = (async () => {
        try {
          ...
          if (!activeDirectoryService) {
            servers = await migrateStoredCommonMcpToKnorviaAgent(
              mcpPlatformService,
              servers,
              latestWorkspacePath,
            );
    ...
    addMcpServer: (name, config) => get().addKnorviaAgentMcpServer(name, config),
    updateMcpServer: (name, config) => get().updateKnorviaAgentMcpServer(name, config),
    deleteMcpServer: (name) => get().deleteKnorviaAgentMcpServer(name),
    addScopedMcpServer: async (source, name, config, projectPath) => {
      invalidateStatusListRequests();
      const targetSource: CliMcpSource = source === "mcp" ? "knorviaagentmcp" : source;
    ...
    addKnorviaAgentMcpServer: (name, config, projectPath) =>
      get().addScopedMcpServer("knorviaagentmcp", name, config, projectPath),
    updateKnorviaAgentMcpServer: (name, config, projectPath) =>
      get().updateScopedMcpServer("knorviaagentmcp", name, config, projectPath),
    deleteKnorviaAgentMcpServer: (name, projectPath) =>
      get().deleteScopedMcpServer("knorviaagentmcp", name, projectPath),
    ...
    markServerStatusListRefreshFailed: (error, requestEpoch, mode = "connect") => {
      ...
      set((state) => ({
        ...
        servers: state.servers.map((server) => {
          const enabled = state.enabledStates[server.id] ?? server.enabled;
          if (server.source !== "knorviaagentmcp" || !enabled || server.status !== "connecting") {
            return server;
          }
          return {
            ...server,
    ...
    getEnabledMcpServersForKnorvia: (_provider) => {
      const { servers, enabledStates, currentProjectPath } = get();
      const candidates = new Map<string, KnorviaMcpServer>();

      for (const server of servers) {
        const isEnabled = enabledStates[server.id] ?? server.enabled;
        if (!isEnabled) continue;
        if (server.source !== "knorviaagentmcp") continue;
      ...
      const result: KnorviaAgentMcpServer[] = [];
      for (const server of candidates.values()) {
        const agentServer = convertToKnorviaAgentMcpServer(server.name, server.config);
        if (agentServer) result.push(agentServer);
      }
      return result;
    },
    ...
    mergePreloadedMcpServers: (preloaded) => {
      ...
      const targetSource: CliMcpSource = "knorviaagentmcp";
