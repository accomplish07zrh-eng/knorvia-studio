import type { SessionCreateSource } from "@knorvia/shared";
...
} from "@/store/sessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/sessionStoreSelectors.js";
...
function normalizeThoughtLevelConfigOption(option: KnorviaConfigOption): KnorviaConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: KnorviaConfigOption[]): KnorviaConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly KnorviaConfigOption[]): KnorviaConfigOption[] {
...
function isModeConfigOption(option: KnorviaConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<KnorviaConfigOption["options"]>;
}): KnorviaConfigOption {
...
function replaceModeConfigOption(
  currentOptions: KnorviaConfigOption[] | null,
  nextModeOption: KnorviaConfigOption | null,
): KnorviaConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceKnorviaUIState,
  updater: (options: KnorviaConfigOption[] | null) => KnorviaConfigOption[] | null,
): Pick<WorkspaceKnorviaUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceKnorviaUIState,
  taskId: string,
): { configOptions: KnorviaConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: KnorviaTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as KnorviaTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToKnorviaAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToKnorviaAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToKnorviaAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
