 ...
 */
import type { IBroadcastService } from "@knorvia/services";
import {
  createContext,
  useCallback,
  useContext,
...
import { createKnorviaStore, type KnorviaState, type KnorviaStore } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<KnorviaStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<KnorviaStore | null>(null);
  if (!storeRef.current) {
    storeRef.current = createKnorviaStore(broadcastService);
  }
 ...
 */
export function useKnorviaStore<T>(selector: (state: KnorviaState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useKnorviaStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useKnorviaStore（store 耦合剥离配套）。
 *
 ...
 */
export function useKnorviaStoreWithDefault<T>(
  selector: (state: KnorviaState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
