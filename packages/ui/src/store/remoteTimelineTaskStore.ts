...
import type { KnorviaTaskListSortBy, IKnorviaTaskService } from "@knorvia/services";
...
interface RemoteTimelineTaskState {
  itemsByWorkspaceKey: Record<string, KnorviaTaskMeta[]>;
  ...
  refreshWorkspace: (params: {
    ...
    taskService: Pick<IKnorviaTaskService, "listTaskList">;
    sortBy: KnorviaTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: KnorviaTaskMeta, sortBy?: KnorviaTaskListSortBy) => void;
...
function sortTimelineTasks(
  ...
  sortBy: KnorviaTaskListSortBy = "updated",
): KnorviaTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, taskService, sortBy, limit }) {
...
export async function refreshRemoteTimelineTasksForSession({
  ...
  await useRemoteTimelineTaskStore.getState().refreshWorkspace({
    workspacePath,
    workspaceIdentity,
    taskService: session.services.taskService,
    sortBy,
    limit,
  });
}
