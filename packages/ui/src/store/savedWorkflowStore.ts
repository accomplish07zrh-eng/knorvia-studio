...
import {
  ...
  type KnorviaSavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: KnorviaSavedWorkflowEntry[];
  invalid: KnorviaSavedWorkflowInvalidEntry[];
  runs: KnorviaSavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: KnorviaAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: KnorviaAgentSavedWorkflowTarget,
  agentService: IKnorviaAgentService,
): Promise<{ list: KnorviaWorkflowsListResult; runs: KnorviaSavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as KnorviaSavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: KnorviaAgentSavedWorkflowTarget | null,
