...
import { getDefaultWorkspaceState } from "@/store/sessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly KnorviaTaskMeta[]): KnorviaTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: KnorviaPermissionRequest["options"],
  right: KnorviaPermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: KnorviaPermissionRequest,
  right: KnorviaPermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useKnorviaChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToKnorviaAgent(params.provider),
