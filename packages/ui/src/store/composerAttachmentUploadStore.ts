...
import type { AttachmentRef } from "@knorvia/shared/protocol-v4";
...
declare global {
  interface Window {
    __knorviaComposerAttachmentUploadStoreE2E?: typeof useComposerAttachmentUploadStore;
    __knorviaCurrentComposerAttachmentScopeKeyE2E?: string;
  }
}

if (shouldExposeE2EStoreBridge()) {
  // E2E 只暴露当前唯一附件 owner，供 scope 切换用例准备状态；不再把附件塞回旧 Session Store。
  window.__knorviaComposerAttachmentUploadStoreE2E = useComposerAttachmentUploadStore;
}

export function exposeComposerAttachmentScopeKeyForE2E(scopeKey: string): void {
  if (window.__knorviaComposerAttachmentUploadStoreE2E) {
    window.__knorviaCurrentComposerAttachmentScopeKeyE2E = scopeKey;
  }
}
