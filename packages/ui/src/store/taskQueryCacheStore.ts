...
import type { KnorviaTaskListItem } from "@knorvia/services";
import type { KnorviaTaskMeta } from "@knorvia/shared";
import { matchesTaskListMembershipKind } from "@knorvia/shared/protocol-v4";
import { mergeTaskMetaCandidates } from "@/lib/taskMetaMerge.js";
import { compareKnorviaTaskListItems } from "@/lib/taskListOrdering.js";
...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: KnorviaTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: KnorviaTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: KnorviaTaskMeta;
...
type CachedTaskListItem = KnorviaTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: KnorviaTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, KnorviaTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareKnorviaTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
