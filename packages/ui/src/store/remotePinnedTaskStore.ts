...
interface RemotePinnedTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    taskService: Pick<IKnorviaTaskService, "listPinnedTasks">;
  }) => Promise<void>;
  upsertTask: (task: KnorviaTaskMeta) => void;
...
function sortPinnedTasks(tasks: KnorviaTaskMeta[]): KnorviaTaskMeta[] {
...
export const useRemotePinnedTaskStore = create<RemotePinnedTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, taskService }) {
    ...
    try {
      const items = await taskService.listPinnedTasks({
        workspacePath,
