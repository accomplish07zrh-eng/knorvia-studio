...
import type { Hook, HookConfig } from "@knorvia/shared";
import type { IHooksService } from "@knorvia/services";
...
function buildKnorviaHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildKnorviaHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    if (!source || source.location?.source === "knorvia") throw new Error("Hook is not importable");
    const imported: Hook = {
      ...source,
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildKnorviaHookLocation(workspacePath, source.location?.scope ?? "user"),
    };
