/* oxlint-disable eslint(max-lines) -- Knorvia Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * Knorvia Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 sessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
...
export interface WorkspaceInitState {
  status: KnorviaWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: KnorviaTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 Knorvia Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: KnorviaBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: KnorviaPermissionRequest | null;
  pendingPermissionRequests: KnorviaPermissionRequest[];
  elicitationRequest: KnorviaElicitationRequest | null;
  pendingElicitationRequests: KnorviaElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: KnorviaUiError | null;
}
...
export interface WorkspaceKnorviaUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * Knorvia Agent 的 workspace 初始化状态。
   *
   * 单 Knorvia Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  draftError: KnorviaUiError | null;
  ...
  taskConfigOptionsByTaskId: Record<string, KnorviaConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, KnorviaTaskMeta>;
  ...
  /** 当前 workspace 选中的 Knorvia Agent provider */
  ...
  /** Knorvia Agent configOptions（模型、模式、思考级别等） */
  configOptions: KnorviaConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: KnorviaTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface KnorviaSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceKnorviaUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceKnorviaUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: KnorviaPermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: KnorviaElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  /** 任务列表变更时调用（创建/删除任务），驱动 taskMetaMerge 重新拉取列表 */
...
const FALLBACK_PROVIDER: KnorviaProvider = KNORVIA_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceKnorviaUIState {
  // 单 Knorvia Agent 迁移后默认 provider 必须收敛到 glm。
