...
import { StudioAgentsSection } from "@/studio/agents/StudioAgentsSection.js";
...
export function SettingsPage({
  ...
  useEffect(() => {
    // 挂载后再清理一次性范围意图，兼容 Strict Mode；旧市场来源不再影响管理页导航。
    clearPendingSettingsPluginOrigin();
    clearPendingSettingsPluginScopeKey();
  }, []);
  ...
  const setCodePreviewSettings = useKnorviaStore((state) => state.setCodePreviewSettings);
  const uiFontSizePx = useKnorviaStore((state) => state.uiFontSizePx);
  const setUiFontSizePx = useKnorviaStore((state) => state.setUiFontSizePx);
  const notificationEnabled = useKnorviaStore((state) => state.notificationEnabled);
  const setNotificationEnabled = useKnorviaStore((state) => state.setNotificationEnabled);
  const notificationSoundEnabled = useKnorviaStore((state) => state.notificationSoundEnabled);
  const setNotificationSoundEnabled = useKnorviaStore((state) => state.setNotificationSoundEnabled);
   ...
   * 旧入口只会写入 "app" 意图；等 sources 加载后需要落到真实来源。
   * 剩余额度「更多」等入口会先写入来源偏好（当前 coding plan 类型/团队项目），
   * 解析时优先选中该来源，缺失或已不可用时回退第一份真实来源。
   */
  const setNewUserOnboardingOpen = useKnorviaStore((state) => state.setNewUserOnboardingOpen);
  ...
  const [knorviaInteractionBehavior, setKnorviaInteractionBehavior] =
    useState<KnorviaInteractionBehavior>("queue");
  ...
  useEffect(() => {
    services.settingService
      ...
      .then((settings: AppSettings) => {
        ...
        setKnorviaInteractionBehavior(settings.knorviaInteractionBehavior ?? "queue");
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    setKnorviaInteractionBehavior(sharedSettings.knorviaInteractionBehavior ?? "queue");
  }, [sharedSettings]);
  ...
  const handleKnorviaInteractionBehaviorChange = useCallback(
    async (behavior: KnorviaInteractionBehavior) => {
      await runSettingsActionAsync({
        featureId: "settings.conversation",
        action: "change_interaction_behavior",
        trigger: "select",
        operation: () => updateSharedSettings({ knorviaInteractionBehavior: behavior }),
        completed: { resultSource: "shared_settings", valueAfter: behavior },
      });
      setKnorviaInteractionBehavior(behavior);
    },
    [updateSharedSettings],
  );
                    ...
                    >
                      ...
                      <div className="space-y-8">
                        {activeSection === "general" ? (
                          ...
                          <StudioAgentsSection
                            onOpenModels={() => setPendingSettingsSection("modelProvider")}
                            onConnectSsh={onOpenRemoteWorkspace}
                          />
