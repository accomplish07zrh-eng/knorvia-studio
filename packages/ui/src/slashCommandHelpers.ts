...
import type { AgentSummary, Locale, SkillSummary, KnorviaSlashCommand } from "@knorvia/shared";
...
export interface SlashCommandPluginProps {
  ...
  nativeCommands?: readonly KnorviaSlashCommand[];
  nativeOnly?: boolean;
}
    ...
    return [
      {
        ...
        value,
        label: `/${value}`,
        description: [command.inputHint?.trim(), command.description].filter(Boolean).join(" · "),
