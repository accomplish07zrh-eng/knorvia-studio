...
import type { KnorviaGroupedTaskViewNode } from "@knorvia/services";
...
function getTopLevelNodeKey(node: KnorviaGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: KnorviaGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: KnorviaGroupedTaskViewNode, index: number) => ReactNode;
}) {
