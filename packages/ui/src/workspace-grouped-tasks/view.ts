...
function cloneView(view: KnorviaGroupedTaskView): KnorviaGroupedTaskView {
  return {
...
function cloneNodes(view: KnorviaGroupedTaskView): KnorviaGroupedTaskViewNode[] {
  return [...view.nodes];
}

function cloneGroupNode(
  node: Extract<KnorviaGroupedTaskViewNode, { type: "group" }>,
): Extract<KnorviaGroupedTaskViewNode, { type: "group" }> {
...
function getGroupedTaskGroupIds(view: KnorviaGroupedTaskView): string[] {
...
function getTaskGroupIdInView(view: KnorviaGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function findTopLevelTaskIndex(view: KnorviaGroupedTaskView, targetTaskKey: string): number {
  return view.nodes.findIndex(
    (node) => node.type === "task" && taskKey(node.task) === targetTaskKey,
  );
}

function findGroupIndex(view: KnorviaGroupedTaskView, groupId: string): number {
  return view.nodes.findIndex((node) => node.type === "group" && node.group.id === groupId);
}

function findGroupIdByTaskKey(view: KnorviaGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function getNodeIdAfterGroup(
  view: KnorviaGroupedTaskView,
  groupId: string,
): KnorviaGroupedTaskViewNode | null {
...
function insertTaskBeforeTopLevelNode(
  view: KnorviaGroupedTaskView,
  task: KnorviaTaskMeta,
  beforeNode: KnorviaGroupedTaskViewNode | null,
): KnorviaGroupedTaskView {
  const nodes = cloneNodes(view);
  const insertIndex =
    beforeNode === null
