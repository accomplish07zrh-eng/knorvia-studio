...
import type { CuaAccessibilitySettingsResult, CuaPermissionKind } from "@knorvia/shared";
import { requiredCuaPermissionsForRequestAccessStatus } from "@knorvia/shared/protocol-v4";
import {
  isCuaPermissionStatusAvailable,
  type CuaPermissionRestartOptions,
  type CuaPermissionStatusResult,
  type KnorviaAgentCuaPermissionObservation,
...
function cuaPermissionObservationKey(observation: KnorviaAgentCuaPermissionObservation): string {
...
export function CuaPermissionObservationAttachment() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      typeof services.agentService.onDynamicCuaPermissionObservation !== "function"
    ) {
      return;
    }
    ...
    let disposed = false;
    const subscription = services.agentService.onDynamicCuaPermissionObservation()(
      (observation) => {
        const missing = requiredCuaPermissionsForRequestAccessStatus(observation.permissionStatus);
