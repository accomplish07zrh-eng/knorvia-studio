...
import type { Locale, LocalePreference } from "@knorvia/shared";
import { DEFAULT_LOCALE } from "@knorvia/shared";
import type { BroadcastMessage, IBroadcastService, ISettingService } from "@knorvia/services";
import {
...
const LOCALE_PREFERENCE_KEY = "knorvia-locale-preference";
 ...
 */
export function KnorviaIntlProvider({
  children,
  settingService,
  broadcastService,
  initialLocale,
...
/** 获取 intl 上下文 */
export function useKnorviaIntl(): IntlContextValue {
  const ctx = useContext(IntlContext);
  if (!ctx) {
    throw new Error("useKnorviaIntl 必须在 KnorviaIntlProvider 内使用");
  }
  return ctx;
}
