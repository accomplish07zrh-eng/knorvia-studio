...
import { knorviaTaskMetaSchema } from "../../shared/src/validation.js";
...
test("current task metadata is accepted without upgrading third-party Agent identities", () => {
  ...
  for (const provider of ["claude", "codex", "gemini", "opencode"]) {
    assert.equal(knorviaTaskMetaSchema.safeParse({ ...meta, provider }).success, false, provider);
  }
});
...
test("Knorvia subagent identity wins over retired Codex nicknames", () => {
  const tool = {
    id: "tool-example",
    kind: "Agent",
    title: "Agent",
