...
test("attachment filenames reject NUL and line separators while preserving Unicode", () => {
  const params = { connectionId: "c", uploadId: "u", sessionId: "s", mime: "text/plain", totalBytes: 0, totalChunks: 0, checksum: `sha256:${"0".repeat(64)}` };
  for (const fileName of ["报告.txt", "space name.txt"]) {
    assert.equal(v4AttachmentBeginParamsSchema.safeParse({ ...params, fileName }).success, true, fileName);
  }
