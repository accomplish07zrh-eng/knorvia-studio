import type { KnorviaTaskMode } from "./task-types-core.js";
...
/** 整条 automation 的生命周期。 */
export type KnorviaAutomationLifecycleStatus = "active" | "completed" | "failed" | "paused";

/** 单条 automation 当前一轮的派发信息态（供 UI/诊断）。 */
export type KnorviaAutomationDispatchStatus =
  | "idle"
  | "claimed"
  | "dispatched"
  | "failed_to_dispatch";
...
export type KnorviaAutomationLocationKind = "local" | "remote";

/** 触发来源：定时 or 立即运行。 */
export type KnorviaAutomationTrigger = "schedule" | "manual";

/** 自定义重复规则；cronExpr 保留为兼容展示，调度以本字段为权威。 */
export interface KnorviaAutomationScheduleRule {
 ...
 */
export type KnorviaAutomationIntervalUnit = KnorviaAutomationScheduleRule["unit"];

/** 单次 run 的派发结果（scheduler 权威，驱动重试）。skipped=错过触发窗口。 */
export type KnorviaAutomationRunDispatchStatus =
  | "claimed"
  | "dispatched"
  | "failed_to_dispatch"
  | "skipped";
...
export interface KnorviaAutomation {
  ...
  nextRunAt?: number;
  lastRunAt?: number;
  dispatchStatus: KnorviaAutomationDispatchStatus;
...
export interface KnorviaAutomationRunIdParts {
  automationId: string;
...
/**
 * runId 契约（见 KnorviaAutomationRun.runId）的唯一解析入口；desktop host/scheduler 与
 * CLI telemetry 不各自按字符串猜格式。不符合两种声明格式的输入一律返回 null。
 */
export function parseAutomationRunId(runId: string): KnorviaAutomationRunIdParts | null {
...
export interface KnorviaAutomationRun {
...
export interface KnorviaAutomationCreateParams {
  title: string;
  cronExpr: string;
...
/** 编辑 automation 的可变字段。 */
export interface KnorviaAutomationUpdateParams {
  title?: string;
  cronExpr?: string;
  prompt?: string;
