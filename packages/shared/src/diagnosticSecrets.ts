...
const SECRET_KEY = /(?:api[-_]?key|authorization|cookie|credential|password|passwd|private[-_]?key|secret|token|passphrase)/i;
...
export function redactDiagnosticText(value: string): string {
  return value
    ...
    .replace(/(["']?(?:api[-_]?key|authorization|cookie|credential|password|passwd|private[-_]?key|secret|token|passphrase)["']?\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;}]+)/gi, `$1${REDACTED}`)
...
export function redactDiagnosticValue(value: unknown, seen = new WeakSet<object>(), depth = 0): unknown {
  ...
  if (Array.isArray(value)) return value.map((item) => redactDiagnosticValue(item, seen, depth + 1));
