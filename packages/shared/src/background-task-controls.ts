...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<KnorviaBackgroundTaskControlStatus>([
  "running",
]);
...
function normalizeStatus(value: string | undefined): KnorviaBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // Knorvia Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseKnorviaBackgroundTaskControlItems(
  ...
  const jobsById = new Map<string, KnorviaBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveKnorviaBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
...
export function collectVisibleKnorviaBackgroundTaskControlItems(
  ...
  return jobs
    .filter(isActiveKnorviaBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getKnorviaBackgroundTaskControlItemElapsedMs(job, now),
    }))
