/* Knorvia 官方 Server MCP 鉴权的共享常量与类型。
   放在 shared 是因为头集合有两个消费者且分属不同包：
   - `packages/services` 侧生产身份头；
   - `apps/cli/packages/adapters` 侧（Plugin parser + MCP adapter）拦截保留头。
 ...
 * 命名空间前缀沿用 `com.knorvia-studio/`，与既有的 `com.knorvia-studio/request-context` 一致。
 */
...
/** 服务端 `Bigmodel-Target-Type` 的取值（对齐 knorvia-server 的 CodingPlanTargetType）。 */
...
//   - apps/cli/packages/adapters：请求发出前的本地校验；
//   - packages/services（host）：身份权威边界的二次校验（只依赖 @knorvia/shared，
//     无法 import CLI 侧包）。
...
export interface IsOfficialMcpOriginTrustedInput {
   ...
   */
  pluginId: string;
  /** 当前 Knorvia API origin，由调用方按各自口径解析后传入。 */
  knorviaApiOrigin: string | undefined;
}

/**
 * 校验官方 MCP 的凭据目标：要求 HTTPS origin 与运行时 Knorvia API origin 相等，
...
export interface CreateOfficialMcpTrustedOriginRegistryOptions {
  ...
  devTrustedOriginsRaw?: string | undefined;
  /** 当前 Knorvia API origin 的解析器；两侧必须用等价口径，否则会一侧放行一侧拒绝。 */
