...
// 本文件承载旧 Knorvia Protocol 中仍被存活栈（validation/background-task-notifications/
// v4 投影等）消费的承重类型与 schema：KnorviaSessionInfo / KnorviaMessageWithParts /
// KnorviaPermissionResponse / KnorviaInteractionRequestOrigin 及其依赖闭包。
// 旧协议死亡（protocol/index.ts 删除）后，这是该协议面的唯一幸存面。
...
export const knorviaDeliveryKindSchema = z.enum(["desktop-continuous", "web-remote-replayable"]);
export const knorviaMessageVisibilitySchema = z.enum(["user-visible", "model-only"]);
export const knorviaSyntheticUserMessageSourceSchema = z.enum([
  "background_task",
  "fork",
  "goal_state_change",
  "goal-continuation",
...
export const knorviaWorkspaceRefSchema = z
  .object({
  ...
  .strict();
export const knorviaPermissionDecisionSchema = z.enum(["allow", "deny", "escalate", "modify"]);
export const knorviaPermissionRuleBehaviorSchema = z.enum(["allow", "deny", "ask"]);
/** Backward-compatible wire/storage key interpreted only for trusted official CUA tools. */
export const OFFICIAL_CUA_PERMISSION_RULE_TOOL_NAME = "knorvia:permission-capability:official_cua";
/**
...
export const knorviaPermissionRuleValueSchema = z
  .object({
    toolName: nonEmptyString,
    ruleContent: z.string().optional(),
  })
  .strict();
export const knorviaPermissionUpdateSchema = z
  .object({
    type: z.literal("addRules"),
    behavior: knorviaPermissionRuleBehaviorSchema,
    rules: z.array(knorviaPermissionRuleValueSchema).min(1),
  })
  .strict();
export const knorviaPermissionResponseSchema = z
  .object({
    decision: knorviaPermissionDecisionSchema,
    reason: z.string().optional(),
    modifiedInput: z.unknown().optional(),
    permissionUpdates: z.array(knorviaPermissionUpdateSchema).optional(),
  })
  .strict();
export type KnorviaPermissionResponse = z.infer<typeof knorviaPermissionResponseSchema>;
export const sessionModeSchema = z.enum(["plan", "build", "edit", "yolo", "auto"]);
...
export const sessionGoalSchema = z
  ...
  .strict();
export const sessionGoalVerificationSchema = z
  .object({
  ...
  .strict();
export const sessionGoalVerificationTimelineSchema = z
  .object({
    version: z.literal(1),
    ...
    verification: sessionGoalVerificationSchema.optional(),
...
export const sessionInfoSchema = z
  ...
  .strict();
export type KnorviaSessionInfo = z.infer<typeof sessionInfoSchema>;
export const knorviaInteractionRequestOriginSchema = z
  .object({
  ...
  .strict();
export type KnorviaInteractionRequestOrigin = z.infer<typeof knorviaInteractionRequestOriginSchema>;
const messageTimeSchema = z
  ...
  .strict();
export const knorviaTokenUsageSchema = z
  .object({
  ...
  .strict();
export const knorviaMessageSemanticsSchema = z
  .object({
  ...
  .strict();
export const knorviaUserMessageInfoSchema = z
  .object({
    ...
    source: knorviaSyntheticUserMessageSourceSchema.optional(),
    visibility: knorviaMessageVisibilitySchema.optional(),
    semantics: knorviaMessageSemanticsSchema.optional(),
    metadata: jsonObjectSchema.optional(),
  })
  .strict();
export const knorviaAssistantMessageInfoSchema = z
  .object({
    messageId: nonEmptyString,
    sessionId: nonEmptyString,
    ...
    tokens: knorviaTokenUsageSchema,
  ...
  .strict();
export const knorviaMessageInfoSchema = z.discriminatedUnion("role", [
  knorviaUserMessageInfoSchema,
  knorviaAssistantMessageInfoSchema,
]);
export type KnorviaMessageInfo = z.infer<typeof knorviaMessageInfoSchema>;
const partBaseSchema = z.object({
  partId: nonEmptyString,
  sessionId: nonEmptyString,
  messageId: nonEmptyString,
});
export const knorviaToolStateSchema = z.discriminatedUnion("status", [
  z
    .object({
...
const knorviaTimelineModelSelectionSchema = modelSelectionSchema.extend({
  label: z.string().optional(),
});
const knorviaTimelinePartTimeSchema = z
  .object({
    start: timestampMsSchema.optional(),
    end: timestampMsSchema.optional(),
  })
  .strict();
export const knorviaTimelinePartSchema = partBaseSchema
  .extend({
    ...
    anchorTurnId: nonEmptyString.optional(),
    time: knorviaTimelinePartTimeSchema.optional(),
    ...
    fromModel: knorviaTimelineModelSelectionSchema.optional(),
    toModel: knorviaTimelineModelSelectionSchema
      .extend({
  ...
  .strict();
export const knorviaMessagePartSchema = z.discriminatedUnion("type", [
  partBaseSchema
    .extend({
      type: z.literal("text"),
      text: z.string(),
...
export type KnorviaMessagePart = z.infer<typeof knorviaMessagePartSchema>;
export const knorviaMessageWithPartsSchema = z
  .object({
    info: knorviaMessageInfoSchema,
    parts: z.array(knorviaMessagePartSchema),
  })
  .strict();
export type KnorviaMessageWithParts = z.infer<typeof knorviaMessageWithPartsSchema>;
export const sessionApiRetryStatusSchema = z
  .object({
  ...
  .strict();
export const sessionContextCacheUsageSchema = z
  .object({
  ...
  .strict();
export const knorviaContextUsageBreakdownSourceSchema = z.enum([
  "system_prompt",
  "meta_user_context",
  "skills",
  "tool_prompt",
...
export const knorviaContextUsageBreakdownItemSchema = z
  .object({
    source: knorviaContextUsageBreakdownSourceSchema,
...
export const sessionContextUsageSchema = z
  .object({
    ...
    cost: z
      ...
      .nullable()
      .optional(),
    cache: sessionContextCacheUsageSchema.optional(),
    breakdown: knorviaContextUsageBreakdownSchema.optional(),
  })
  .strict();
export const sessionRuntimeStateSchema = z
  .object({
    ...
    apiRetry: sessionApiRetryStatusSchema.nullable().optional(),
    contextUsage: sessionContextUsageSchema.optional(),
    goalVerifications: z.array(sessionGoalVerificationSchema).optional(),
    goalVerificationTimeline: z.array(sessionGoalVerificationTimelineSchema).optional(),
  })
  .strict();
