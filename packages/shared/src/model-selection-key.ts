...
export function buildNativeSupplierKey(knorviaProvider: KnorviaProvider): string {
  return `${NATIVE_SUPPLIER_KEY_PREFIX}${knorviaProvider}`;
}
...
export function buildGhostSupplierKey(
  knorviaProvider: KnorviaProvider,
...
export function resolveSupplierKeyFromModelDisplayValue(
  ...
  return buildNativeSupplierKey(knorviaProvider);
}
