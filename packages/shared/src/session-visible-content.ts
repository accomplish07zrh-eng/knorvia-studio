...
// 消费者：taskServiceAdapter/sessionProjection/taskIndexSyncer、
// CLI bootstrap session-mapper。textFromKnorviaMessageParts 已迁
// protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
...
export function getKnorviaGoalIterationByAssistantMessageId(
  ...
  const sortedMessages = [...messages].sort(compareKnorviaMessagesByCreatedTime);
...
function compareKnorviaMessagesByCreatedTime(
  left: KnorviaMessageWithParts,
  right: KnorviaMessageWithParts,
) {
...
export function resolveKnorviaVisibleSessionTitle(input: {
  ...
  if (
    normalizedTitle &&
    !isKnorviaGoalContinuationReminderText(normalizedTitle) &&
    !isKnorviaGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUserText = textFromKnorviaMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
