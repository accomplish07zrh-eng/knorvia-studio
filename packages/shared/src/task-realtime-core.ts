...
import { z } from "zod";
import type { KnorviaTaskMigrationSource, KnorviaTaskMode } from "./task-types-core.js";
import { agentProviderSchema } from "./agent-policy.js";
import { knorviaPermissionResponseSchema } from "./protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const knorviaTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly KnorviaTaskMode[];
const knorviaTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly KnorviaTaskMigrationSource[];
const knorviaTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 KnorviaTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(knorviaTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: agentProviderSchema.optional(),
  migrationSource: z.enum(knorviaTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: knorviaTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const knorviaPromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const knorviaTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const knorviaTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  knorviaTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: knorviaTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: knorviaTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
