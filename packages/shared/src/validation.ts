...
import { knorviaProviderSchema } from "./providers.js";
import { persistedAgentProviderSchema } from "./agent-policy.js";
...
export { knorviaTaskModeSchema } from "./task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("knorvia-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  knorviaBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 KnorviaTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { knorviaProviderSchema };

export const knorviaTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessSpawnedResponseSchema = z.object({
  ...
  provider: knorviaProviderSchema,
  workspacePath: nonEmptyStringSchema,
  command: z.string(),
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: knorviaProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = knorviaProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `knorviaProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: knorviaMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: knorviaToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: knorviaMcpTelemetryEventSchema,
  })
  .strict();
...
export const knorviaTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const knorviaPromptImageAttachmentSchema = z.object({
...
export const knorviaPromptVideoAttachmentSchema = z.object({
...
export const knorviaPromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const knorviaPromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const knorviaPromptAttachmentSchema = z.discriminatedUnion("kind", [
  knorviaPromptImageAttachmentSchema,
  knorviaPromptVideoAttachmentSchema,
  knorviaPromptPdfAttachmentSchema,
  knorviaPromptFileAttachmentSchema,
]);

export const knorviaPersistedToolCallSchema = z.object({
...
const knorviaPersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const knorviaPersistedMessageSchema = z.object({
  ...
  attachments: z.array(knorviaPromptAttachmentSchema).optional(),
  tools: z.array(knorviaPersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(knorviaPersistedMessagePartSchema).optional(),
...
export const knorviaTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const knorviaTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: knorviaTaskGoalStatusSchema,
...
export const knorviaTaskGoalChangedPatchSchema = z.object({
  action: knorviaTaskTargetChangedActionSchema,
  source: knorviaTaskTargetChangedSourceSchema,
  target: knorviaTaskGoalSchema.nullable(),
  previousTarget: knorviaTaskGoalSchema.nullable().optional(),
});

export const knorviaTaskTargetStatusSchema = knorviaTaskGoalStatusSchema;
export const knorviaTaskTargetSchema = knorviaTaskGoalSchema;
export const knorviaTaskTargetChangedPatchSchema = knorviaTaskGoalChangedPatchSchema;

export const knorviaTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: knorviaTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: persistedAgentProviderSchema.optional(),
  migrationSource: knorviaTaskMigrationSourceSchema.optional(),
  ...
  status: knorviaTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: knorviaTaskGoalSchema.nullable().optional(),
});

export const knorviaTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const knorviaPinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(knorviaTaskIndexEntrySchema),
});

const knorviaPersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const knorviaPersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(knorviaPersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const knorviaPersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
...
export const sessionFileSchema = z.object({
  meta: knorviaTaskMetaSchema,
  messages: z.array(knorviaPersistedMessageSchema),
  fileChanges: z.array(knorviaPersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(knorviaPersistedTurnCheckpointSchema).optional(),
});
