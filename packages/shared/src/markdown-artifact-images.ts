import { isArtifactUri } from "./artifact-uri.js";

const ARTIFACT_IMAGE_RENDER_PREFIX = "/__knorvia_artifact_image__/";
const LEGACY_ARTIFACT_IMAGE_RENDER_PREFIX = "/__knorvia_artifact_image__/";
const FENCE_PATTERN = /^( {0,3})(`{3,}|~{3,})(.*)$/u;
const ARTIFACT_IMAGE_PATTERN =
  /!\[[^\]\n]*\]\(\s*(?:<)?((?:knorvia|knorvia)-artifact:\/\/[^\s)>]+)(?:>)?(?:\s+(?:"[^"]*"|'[^']*'|\([^)]*\)))?\s*\)/gu;
...
export function decodeMarkdownArtifactImageSource(source: string): string | null {
  const prefix = [ARTIFACT_IMAGE_RENDER_PREFIX, LEGACY_ARTIFACT_IMAGE_RENDER_PREFIX].find(
    (candidate) => source.startsWith(candidate),
  );
  if (!prefix) return null;
  try {
    const ref = decodeURIComponent(source.slice(prefix.length));
    return isArtifactUri(ref) ? ref : null;
  } catch {
    return null;
  }
}
