...
/* eslint-disable max-lines -- host process 统一处理 main↔host 生命周期、日志、Knorvia Agent，拆分前先保持跨进程消息收口。 */
...
export interface HostInitMessage {
  ...
  /** Main 解析后的 Knorvia Built-in Provider Config 路径；Host/Services 不感知 Electron 安装布局。 */
...
export function spawnHostProcess(
  ...
  const execArgv = [
    ...(RUNTIME_KNORVIA_DEBUG ? [`--inspect-brk=${RUNTIME_KNORVIA_DEBUG}`] : []),
    "--no-warnings",
  ];
  const child = electronUtilityProcess.fork(hostModulePath, [], {
    serviceName: formatKnorviaHostProcessName(label),
    execArgv,
    env: {
      ...buildHostProcessEnv(dependencies.hostProcessLocalEnv),
      ...buildHostE2ECoverageEnv(),
      KNORVIA_PROCESS_LABEL: label,
      // macOS-only: the Computer Use Helper launcher runs inside this forked host utilityProcess, whose
      // code-signing identity is a nested Electron helper (NOT dev.knorvia-studio.app). Publish THIS (main
      // Electron) process's pid — which IS dev.knorvia-studio.app — so helperLauncher passes it as
      ...
      ...(process.platform === "darwin" ? { KNORVIA_CUA_LAUNCHER_PID: String(process.pid) } : {}),
      ...(dependencies.desktopContextPromptEnabled
        ? {
            [KNORVIA_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]: dependencies.desktopContextPromptEnabled()
              ? "1"
              : "0",
          }
        : {}),
...
export function disposeHostProcess(
  ...
  // 如果 main 仍按 150/300ms 强杀 host，host 会先退出，cli/app-server 子进程就可能被 init 接管成孤儿。
  const effectiveForceKillDelayMs = Math.max(forceKillDelayMs, 30_000);
...
export function disposeHostProcessAndWait(
  ...
  const waitTimeoutMs = Math.max(
    ...
    (options.forceKillDelayMs ?? 30_000) + 2_000,
    32_000,
  );
