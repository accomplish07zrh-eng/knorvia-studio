...
import { PlatformChannels, type RendererActionTraceConfigV1 } from "@knorvia/shared";
...
function resolveRuntimeConfig(
  ...
  if (isTruthy(env.KNORVIA_LOCAL_TTFT_ENABLED)) config = { ...config, localTtftEnabled: true };
  if (!isTruthy(env.KNORVIA_RENDERER_ACTION_TRACE_ENABLED)) return config;
  return {
    ...config,
    enabled: true,
    sampleRatio: 1,
