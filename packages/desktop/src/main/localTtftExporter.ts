...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@knorvia/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "knorvia-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "knorvia.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "knorvia.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@knorvia/local-ttft", "1");
  const duration = meter.createHistogram("knorvia.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("knorvia.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("knorvia.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  ...
  const attemptDuration = meter.createHistogram("knorvia.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("knorvia.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("knorvia.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("knorvia.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("knorvia.local_ttft.records");
  const drops = meter.createCounter("knorvia.local_ttft.dropped");
