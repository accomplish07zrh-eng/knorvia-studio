...
import { listSSHConfigAliasesFromLocalConfig } from "@knorvia/services/node";
...
export const desktopRuntimeEnv: KnorviaRuntimeEnv = isLocalDevelopmentRuntime
  ? "development"
  : "production";
// 身份看编译期 flavor 而不是 KNORVIA_ENV：KNORVIA_PREVIEW_IDENTITY=1 的生产后端构建同样是 Preview，
// 需要独立的应用名、Electron 数据目录和 Helper 安装子目录才能与正式版并排运行。
const isPreviewPackagedRuntime = !isLocalDevelopmentRuntime && KNORVIA_PRODUCT_FLAVOR === "preview";
...
export function loadHostProcessEnvFromLocalFiles(): Record<string, string> {
  if (isElectronAppPackaged()) {
    ...
    return { KNORVIA_TELEMETRY_RUNTIME_DISTRIBUTION: "packaged" };
  }
  ...
  return applySelectedKnorviaEnvLinks(merged);
}
...
function resolveAvailableDevelopmentMockCdnDir(): string | undefined {
  ...
  const releaseDir = join(mockCdnDir, "releases", KNORVIA_VERSION);
...
function shouldUseRemoteCdnInDevelopment(localEnv: LocalRuntimeEnv = {}): boolean {
  return isTruthyEnvFlag(resolveEnvValue("KNORVIA_DEV_REMOTE_ASSET_USE_CDN", localEnv));
}
...
function resolveRemoteCdnBaseUrls(
  ...
  const raw = resolveEnvValue("KNORVIA_REMOTE_ASSET_CDN_BASE_URL", localEnv);
...
function applySelectedKnorviaEnvLinks(env: Record<string, string>): Record<string, string> {
  // 上游产品地址、身份和 OTLP 凭据均不进入 Knorvia Host；模型用户配置另有原有服务路径。
  return Object.fromEntries(
    Object.entries(env).filter(
      ([key]) =>
        !/^KNORVIA_|^ZAI_OAUTH_|^BIGMODEL_OAUTH_|^ZAI_BUSINESS_|^OTEL_|^KNORVIA_(TELEMETRY|ARMS|BASE_URL|ENDPOINT_ORIGIN)/i.test(
          key,
        ),
    ),
  );
...
function resolveHostProcessNodeEnv(): KnorviaRuntimeEnv {
  return desktopRuntimeEnv;
}

function resolveRemoteAssetCacheDir(localEnv: LocalRuntimeEnv = {}): string {
  const overrideCacheDir = resolveEnvValue("KNORVIA_REMOTE_ASSET_CACHE_DIR", localEnv);
  if (overrideCacheDir) {
...
function resolveBundledKnorviaAgentBinaryPath(): string | undefined {
...
export function resolveBundledAgentBinaryPath(): string | undefined {
  return resolveBundledKnorviaAgentBinaryPath();
}
...
function resolveHostProcessBinaryEnv(
  ...
  // Knorvia Agent 与 app 协议适配强绑定版本，生产包必须优先使用随包携带的固定 runtime。
  // 用户机器或本地 .env 里残留的 KNORVIA_AGENT_BINARY 即使存在，也可能版本不兼容。
...
function resolveDynamicWorkflowModeHostEnv(options: {
  ...
  if (!options.isPackaged) {
    ...
    return mode ? { [KNORVIA_DYNAMIC_WORKFLOW_MODE_ENV]: mode } : {};
  }
  if (options.isPreview) {
    return { [KNORVIA_DYNAMIC_WORKFLOW_MODE_ENV]: "alwaysOn" };
  }
  return {};
}
...
export function buildHostProcessEnv(hostProcessLocalEnv: Record<string, string>) {
    ...
    process.platform !== "darwin"
      ...
      : packagedDesktop
        ...
        : // Truthy grammar must match the producer's isUnsignedHelperLocalDevRequested
          ...
          ["1", "true", "on"].includes(
              rawInheritedEnv.KNORVIA_CUA_HELPER_ALLOW_UNSIGNED_LOCAL?.trim().toLowerCase() ?? "",
            )
          ? rawInheritedEnv.KNORVIA_CUA_BUNDLED_HELPER_APP_PATH?.trim() ||
            join(
              rawInheritedEnv.KNORVIA_HOME?.trim() || join(homedir(), ".knorvia-studio"),
              "computer-use",
              "dev",
              DEV_HELPER_APP_NAME,
            )
  ...
  const inheritedEnv = applySelectedKnorviaEnvLinks({
    ...sanitizeKnorviaRuntimeEnv(rawInheritedEnv),
    ...buildKnorviaToolEnvPassthroughEnv(rawInheritedEnv),
  });
  ...
  if (packagedDesktop) {
    delete inheritedEnv.KNORVIA_CUA_HELPER_ALLOW_UNSIGNED_LOCAL;
  }
  const dynamicWorkflowModeHostEnv = resolveDynamicWorkflowModeHostEnv({
    inheritedValue: rawInheritedEnv[KNORVIA_DYNAMIC_WORKFLOW_MODE_ENV],
  ...
  delete inheritedEnv[KNORVIA_DYNAMIC_WORKFLOW_MODE_ENV];

  return {
    ...inheritedEnv,
    // OTLP 凭据只定向传到 host；host 初始化 services 时会立即捕获并从 process.env 清除，
    // 后续只在启动 Agent 时短暂注入，不会进入 Bash/MCP/tool env。
    // Knorvia 运行时不再使用 NODE_ENV；它会被用户 shell、包管理器和测试框架复用。
    // 这里显式下发 KNORVIA_RUNTIME_ENV，并在继承环境里清掉 NODE_ENV，避免 host/agent/Bash 被污染。
    [KNORVIA_RUNTIME_ENV_KEY]: resolveHostProcessNodeEnv(),
    // 显式注入编译期产品身份，保证主进程与 host 的身份语义一致；地址独立解析。
    // inheritedEnv 从 .env 通用变量补齐 Knorvia/ZAI 链接，未覆盖时统一使用线上默认值。
    KNORVIA_ENV: KNORVIA_ENV,
    // Preview 与生产版共享任务、配置和凭据，但不同版本的 Helper 不能互相覆盖或触发降级保护。
    // 只隔离 computer-use 下的运行组件，不改写 KNORVIA_HOME / KNORVIA_DATA_BASE_DIR 业务数据根。
    ...(isPreviewPackagedRuntime ? { KNORVIA_CUA_HELPER_INSTALL_VARIANT: "preview" } : {}),
    ...
    ...buildDesktopProfileEnvironment(dataBaseDir),
    ...(windowsAppInstallDir
      ? { [KNORVIA_WINDOWS_APP_INSTALL_DIR_ENV]: windowsAppInstallDir }
      : {}),
    ...(bundledCuaHelperAppPath
      ? { [KNORVIA_CUA_BUNDLED_HELPER_APP_PATH_ENV]: bundledCuaHelperAppPath }
      : {}),
    ...(resolvedAgentBinaryPath ? { KNORVIA_AGENT_BINARY: resolvedAgentBinaryPath } : {}),
    ...(resolvedLarkCliBinaryPath ? { KNORVIA_LARK_CLI_BINARY: resolvedLarkCliBinaryPath } : {}),
  };
}
