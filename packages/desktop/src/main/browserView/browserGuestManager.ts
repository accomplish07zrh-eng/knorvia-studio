...
export class BrowserGuestManager {
  ...
  attachGuest(
    ...
    if (guest.getType() !== "webview") {
      ...
      // CDP/Runtime 输入会直接操作 Knorvia composer。IAB 只允许真实 <webview> guest fail closed。
      this.log?.(
  ...
  private async installRecordingCursorOverlay(guest: GuestWebContents): Promise<void> {
    await guest.executeJavaScript(`(() => {
      const id = "__knorvia_browser_recording_cursor";
      ...
      window["__knorviaBrowserRecordingCursorCleanup"]?.();
      ...
      window["__knorviaBrowserRecordingCursorCleanup"] = () => {
        ...
        cursor.remove();
        delete window["__knorviaBrowserRecordingCursorCleanup"];
      };
    })()`);
  }
  ...
  private async removeRecordingCursorOverlay(guest: GuestWebContents): Promise<void> {
    ...
    await guest
      .executeJavaScript('window["__knorviaBrowserRecordingCursorCleanup"]?.()')
      .catch(() => undefined);
  }
  ...
  private finalizeTabs(
    ...
    // 产品语义：IAB tab 在当前 Knorvia 进程内默认持久。keep 是状态标记集合，不是清理白名单；
  ...
  private async summary(tab: ManagedTab): Promise<BrowserTabSummary> {
    ...
    return {
      ...
      // closed 是 owner 内部状态，不能写入公开的活动标签页枚举。
  ...
  private async restoreReboundGuest(
    ...
    // await 期间生命周期会被其它事件修改，每次读取 owner 的最新状态。
