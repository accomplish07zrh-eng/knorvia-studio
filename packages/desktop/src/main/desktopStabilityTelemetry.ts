...
interface StabilityGlobalContext {
  ...
  appVersion: string;
  armsEnv: ReturnType<typeof mapKnorviaEnvToArmsRumEnv>;
}
...
function redactRemainingAbsolutePaths(value: string): string {
  ...
  const protectedValue = value.replace(AGENT_CRASH_KNOWN_PATH_PLACEHOLDER_PATTERN, (match) => {
    ...
    return `KNORVIA_REDACTED_PATH_${index}_TOKEN`;
  });
  ...
  return protectedPaths.reduce(
    (result, protectedPath, index) =>
      result.replace(`KNORVIA_REDACTED_PATH_${index}_TOKEN`, protectedPath),
    redacted,
  );
}
...
function reportStabilityCustom(
  ...
  _windowScene?: StabilityWindowScene,
  _lifecycleSceneOverride?: StabilityLifecycleScene,
): void {}
...
export function reportAgentProcessExceptionToArms(
  ...
  const _name = sanitizeAgentCrashDetail(diagnostic.name, event.workspacePath);
  const _message = sanitizeAgentCrashDetail(diagnostic.message, event.workspacePath);
  const _stack =
...
function mapChildProcessGoneToProcessRoleWithName(
  ...
  if (processName?.startsWith("knorvia-host")) {
    return "host";
  }
  if (processName?.startsWith("agent")) {
    return "agent";
  }
  switch (type) {
    case "GPU":
...
function shouldReportChildProcessGoneAsCrash(input: ChildProcessGoneInput): boolean {
  ...
  // 子进程会被系统自动重建，不会让 Knorvia 主窗口或会话不可用；它们只能作为可恢复退出记录，
