import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@knorvia/shared";

import type { KnorviaDataSizeScanResult } from "./dataSizeScanner.js";
import {
  readKnorviaDataSizeTelemetryState,
  writeKnorviaDataSizeTelemetryState,
  type KnorviaDataSizeTelemetryState,
} from "./dataSizeTelemetryState.js";
import { scanKnorviaDataDirectoryInWorker } from "./dataSizeWorkerClient.js";

export type { KnorviaDataSizeTelemetryState } from "./dataSizeTelemetryState.js";

const KNORVIA_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const KNORVIA_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const KNORVIA_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const KNORVIA_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const KNORVIA_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const KNORVIA_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const KNORVIA_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const KNORVIA_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const KNORVIA_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const KNORVIA_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const KNORVIA_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const KNORVIA_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface KnorviaDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: KnorviaDataSizeTelemetryTiming = {
  abortedRetryMs: KNORVIA_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: KNORVIA_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: KNORVIA_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: KNORVIA_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: KNORVIA_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: KNORVIA_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: KNORVIA_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: KNORVIA_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: KNORVIA_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: KNORVIA_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface KnorviaDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface KnorviaDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isKnorviaBusy: () => boolean;
  logger: KnorviaDataSizeTelemetryLogger;
  readState: () => Promise<KnorviaDataSizeTelemetryState | null>;
  report: (result: KnorviaDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<KnorviaDataSizeScanResult>;
  timing?: Partial<KnorviaDataSizeTelemetryTiming>;
  writeState: (state: KnorviaDataSizeTelemetryState) => Promise<void>;
}
...
function buildKnorviaDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: KnorviaDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_knorvia_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      metric_kind: "knorvia_data_bytes",
      ...
      schema_version: 1,
      knorvia_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / KNORVIA_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createKnorviaDataSizeTelemetryScheduler(
  dependencies: KnorviaDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: KnorviaDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isKnorviaBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[knorvia-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<KnorviaDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: KnorviaDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[knorvia-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("Knorvia Studio data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("Knorvia Studio data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[knorvia-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[knorvia-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: KnorviaDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[knorvia-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createKnorviaDataSizeTelemetryScheduler> | null = null;

export function registerDesktopKnorviaDataSizeTelemetry(options: {
  context: Parameters<typeof buildKnorviaDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isKnorviaBusy: () => boolean;
  logger: KnorviaDataSizeTelemetryLogger;
  ...
  desktopScheduler = createKnorviaDataSizeTelemetryScheduler({
    ...
    isKnorviaBusy: options.isKnorviaBusy,
    logger: options.logger,
    readState: () => readKnorviaDataSizeTelemetryState(options.stateFile),
    report: () => {},
    scan: ({ signal }) =>
      scanKnorviaDataDirectoryInWorker(
        {
          maxDurationMs: KNORVIA_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: KNORVIA_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeKnorviaDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[knorvia-data-size] scheduler start failed", error);
  });
}

export function stopDesktopKnorviaDataSizeTelemetry(): void {
