...
export interface KnorviaDataSizeTelemetryState {
  lastReportedAt?: number;
  reportReservedAt?: number;
}

export async function readKnorviaDataSizeTelemetryState(
  stateFile: string,
): Promise<KnorviaDataSizeTelemetryState | null> {
  try {
    const parsed: unknown = JSON.parse(await readFile(stateFile, "utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      throw new TypeError("Invalid Knorvia Studio data size telemetry state");
    }
    const record = parsed as Record<string, unknown>;
    const state: KnorviaDataSizeTelemetryState = {};
    if (record.lastReportedAt !== undefined) {
      if (typeof record.lastReportedAt !== "number" || !Number.isFinite(record.lastReportedAt)) {
        throw new TypeError("Invalid lastReportedAt in Knorvia Studio data size telemetry state");
      }
    ...
    if (record.reportReservedAt !== undefined) {
      if (
        ...
        throw new TypeError("Invalid reportReservedAt in Knorvia Studio data size telemetry state");
      }
...
export async function writeKnorviaDataSizeTelemetryState(
  stateFile: string,
  state: KnorviaDataSizeTelemetryState,
): Promise<void> {
