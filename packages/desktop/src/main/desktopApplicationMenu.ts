...
import {
  HELP_TOGGLE_KNORVIA_STDIO_TAP_MENU_ID,
...
export function getDesktopMenuLabel(
  ...
  return getDesktopMenuMessage(locale, id).replaceAll("Knorvia Studio", "Knorvia Studio");
}
...
export function updateKnorviaStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_KNORVIA_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readKnorviaStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  const stdioTapState = readKnorviaStdioTapDevState();
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_KNORVIA_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleKnorviaStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(DesktopCommandIds.ToggleKnorviaStdioTapDevProxy),
              },
              { type: "separator" as const },
            ]
          : []),
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        endpointSelection: options.endpointSelection,
