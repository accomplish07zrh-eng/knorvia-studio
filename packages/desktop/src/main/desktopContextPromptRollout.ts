import { KNORVIA_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV } from "@knorvia/shared";
import {
...
export { KNORVIA_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV };
...
export function createElectronDesktopContextPromptConfigFetcher(options: {
  ...
  // 模型提示词只来自本地源码和用户设置，不能由上游产品接口远程改写。
  void options;
  return async () => ({ code: 0, data: { configs: {} } });
}
