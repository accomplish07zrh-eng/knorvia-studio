...
import type { SaveFileRequest, SaveFileResult } from "@knorvia/shared";
...
async function resolvePublicRemoteUrl(url: URL): Promise<LookupAddress[]> {
  ...
  // all:true 返回地址数组，不能用重载函数最后一个签名的 ReturnType。
...
async function downloadRemoteFile(sourceUrl: URL, destinationPath: string): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "knorvia-save-file-"));
...
export function registerDesktopSaveFileIpcHandler(logger: { warn: (...args: unknown[]) => void }) {
  ipcMain.handle(
    ...
    async (event, payload: SaveFileRequest): Promise<SaveFileResult> => {
      ...
      // 保存已校验的局部值，异步对话框之后仍使用同一份输入。
      const data = payload.data instanceof ArrayBuffer ? payload.data : undefined;
      if (!suggestedName || (sourceUrl === null && !data)) {
        return { success: false, error: "invalid_file_payload" };
      }
      if (data && data.byteLength === 0) {
        return { success: false, error: "invalid_file_payload" };
      }
      if (data && data.byteLength > MAX_SAVE_FILE_BYTES) {
