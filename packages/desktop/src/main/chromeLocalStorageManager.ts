...
import type { ChromeBrowserDataImportError } from "@knorvia/shared";
...
export async function readChromeCookiesWithHelper(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "knorvia-chrome-cookie-helper-"));
...
export async function importChromeLocalStorage(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "knorvia-chrome-local-storage-"));
  const userDataPath = join(tempRoot, "User Data");
  const targetProfilePath = join(userDataPath, "Default");
  let originCount = 0;
  try {
