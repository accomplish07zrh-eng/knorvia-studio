...
const ABOUT_APPLICATION_NAME = "Knorvia Studio";
...
> = {
  "zh-CN": {
    aboutTitle: "关于 Knorvia Studio",
    versionLabel: "版本",
    okButtonLabel: "确定",
    optimizedForAppleSilicon: "已针对 Apple Silicon 优化。",
    copyright: (year) => `版权所有 © ${year} Knorvia Studio。`,
  },
  "en-US": {
    aboutTitle: "About Knorvia Studio",
    versionLabel: "version",
    okButtonLabel: "OK",
    optimizedForAppleSilicon: "Optimized for Apple Silicon.",
    copyright: (year) => `Copyright © ${year} Knorvia Studio.`,
  },
};
...
export function createAboutSnapshot(options: AboutSnapshotOptions = {}): AboutSnapshot {
  ...
  return {
    appVersion: normalizeValue(options.appVersion ?? buildMetadata?.appVersion ?? KNORVIA_VERSION),
    buildCommitId: normalizeValue(buildMetadata?.buildCommitId ?? KNORVIA_COMMIT),
    buildTime: normalizeValue(buildMetadata?.buildTime ?? KNORVIA_BUILD_TIME),
    environment: normalizeValue(options.environment ?? KNORVIA_ENV),
  ...
  void aboutWindow.loadURL(
    `data:text/html;charset=utf-8,${encodeURIComponent(
      createCustomAboutDialogHtml({
        ...
        iconDataUrl: existsSync(iconPath)
