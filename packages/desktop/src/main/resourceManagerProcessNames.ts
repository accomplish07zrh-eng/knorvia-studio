...
import { formatKnorviaRendererProcessName } from "@knorvia/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatKnorviaRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatKnorviaRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatKnorviaRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
