...
function buildLegacyCommonMcpStorageCandidates(request?: MigrateLegacyCommonMcpRequest): string[] {
  ...
  candidates.push(join(appData, "ai.z.knorvia-studio", "store.json"));

  candidates.push(
    join(localAppData, "ai.z.work", "EBWebView", "Default", "Local Storage", "leveldb"),
    join(appData, "Knorvia Studio", "Local Storage", "leveldb"),
    join(
      appData,
      "Knorvia Studio",
      "Partitions",
      "knorvia-embedded-browser",
      "Local Storage",
      "leveldb",
    ),
    join(appData, "Knorvia Studio Dev", "Local Storage", "leveldb"),
    join(
      appData,
      "Knorvia Studio Dev",
      "Partitions",
