...
type PatchedIpcRenderer = IpcRenderer & { __knorviaArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __knorviaArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__knorviaArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__knorviaArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__knorviaArmsIpcPatched) {
    return;
  }
  ...
  patched.__knorviaArmsIpcPatched = true;
}
