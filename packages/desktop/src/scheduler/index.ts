...
function resolveScheduledAt(automation: KnorviaAutomation, now: number): number {
...
async function handleClaimed(automation: KnorviaAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: KnorviaAutomationRun["modelSelection"],
): void {
...
async function settleDispatchResult(
  ...
  const trigger: KnorviaAutomationTrigger =
