...
export function createRemoteWorkspaceServiceCollection(params: {
  ...
  const reportingRemoteKnorviaTaskService = params.createReportingRemoteKnorviaTaskService(
    params.connectionServices.taskService,
  );
  ...
  const remoteKnorviaTaskService = params.createRemotePromptAttachmentTaskService(
    reportingRemoteKnorviaTaskService,
  );
  const remoteKnorviaSessionService = params.createRemotePromptAttachmentSessionService(
    params.connectionServices.sessionService,
  );
  ...
  // 因此这里为 remote workspace host 补齐本地全局 channel；文件、终端、Knorvia Agent 仍来自远端，
  // 设置、通用凭据、模型供应商和 settings-sync 继续读写本机配置。
  const services = new ServiceCollection()
    .register(IStudioRuntimeService, params.connectionServices.studioRuntimeService!)
    ...
    .register(IKnorviaTaskService, remoteKnorviaTaskService)
    .register(IKnorviaAgentService, params.connectionServices.agentService)
    .register(IKnorviaSessionService, remoteKnorviaSessionService)
    ...
    .register(
      IUsageStatsService,
      createUsageStatsService({ agentService: params.connectionServices.agentService }),
    )
