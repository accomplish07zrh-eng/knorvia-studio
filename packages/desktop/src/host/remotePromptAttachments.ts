...
const REMOTE_PROMPT_ATTACHMENT_ROOT = "~/.knorvia-studio/tmp/prompt-attachments";
const REMOTE_PROMPT_ATTACHMENT_RELATIVE_ROOT = ".knorvia-studio/tmp/prompt-attachments";
...
function getAttachmentLocalPath(attachment: KnorviaPromptAttachment): string | undefined {
      ...
      return async (...args: unknown[]) => {
        ...
        const prepared = await options.materializePromptAttachments({
          ...
          attachments: Array.isArray((params as { attachments?: unknown }).attachments)
            ? (params as { attachments?: KnorviaPromptAttachment[] }).attachments
            : undefined,
        });
