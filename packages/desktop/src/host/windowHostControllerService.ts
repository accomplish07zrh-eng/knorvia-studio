...
import { matchesTaskListMembershipKind } from "@knorvia/shared/protocol-v4";
import type {
...
function taskKey(task: Pick<KnorviaTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: KnorviaTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@knorvia/shared/protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: KnorviaTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  ...
  const sourceTaskServices = new Map<string, IKnorviaTaskService>();
  const sourceSnapshotTaskServices = new Map<string, IKnorviaTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: IKnorviaTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: IKnorviaAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  function resolveQuerySources(
    scopes: KnorviaTaskListWorkspaceScope[],
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@knorvia/shared").WindowHostAttachmentScope;
