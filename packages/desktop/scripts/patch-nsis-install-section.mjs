...
export function patchNsisInstallSectionSource(source) {
  ...
  if (!patched.includes(detailsAnchor)) {
    // 锚点不在只有两种可能，都必须显式失败而不是静默跳过：
    //   1) 上游模板结构变了，补丁需要重新对齐；
    //   2) node_modules 里残留了上一次打包写入的补丁（进程被中断，退出还原没跑到）。
    // 静默跳过会把未经定制的模板打进安装包。
    throw new Error(
      "electron-builder installSection.nsh 缺少预期锚点 SetDetailsPrint none：" +
        "上游模板结构可能已变化，或 node_modules 残留了上次打包的补丁",
    );
  }
