// Agent bundle 的暂存动作：把 apps/cli/packages/cli/dist/knorvia.cjs 放进
// bundled-agents/<平台>/knorvia，并写 meta。
//
// dev 与打包**必须**用同一份暂存实现。
// 只有打包链（prepare-agent-node-bundle.mjs）会暂存是不够的，dev 链
// （scripts/build-desktop-agent-cli.mjs）不会；而 dev 未打包时的 agent 二进制由
// desktopRuntimeEnv.ts 的 resolveBundledKnorviaAgentBinaryPath() 解析，候选**只有**
...
export function resolveAgentBundlePaths({ repoRoot, platformKey }) {
  ...
  return {
    ...
    agentDir,
    stagedBundlePath: resolve(agentDir, "knorvia.cjs"),
    stagedMetaPath: resolve(agentDir, ".node-bundle-meta.json"),
  };
}

/**
 * 干净重建 knorvia 目录再拷贝。清空是刻意的：electron-builder 整目录拷贝
 * bundled-agents/<平台>/knorvia → resources/knorvia，本地工作树里上一次构建残留的原生二进制
 * （agent / knorvia-acp 等）和旧 meta 会被一并打进安装包（CI 干净检出不会有，本地会）。
 */
