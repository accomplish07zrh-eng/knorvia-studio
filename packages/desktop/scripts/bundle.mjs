...
export function createElectronRuntimeMirrorEnv(mirror) {
  return {
    KNORVIA_ELECTRON_RUNTIME_MIRROR: mirror,
...
function resolveDefaultElectronBuilderBinariesMirror(env = process.env) {
  return env.KNORVIA_DEPS_BASE_URL?.trim() || env.INTRANET_MACHINE_HOST?.trim()
...
环境变量:
  KNORVIA_TARGET_OS              与 --os 等价
  KNORVIA_TARGET_ARCH            与 --arch 等价
`);
}
...
function parseArgs(argv) {
  const options = {
    os: process.env.KNORVIA_TARGET_OS ?? null,
    arch: process.env.KNORVIA_TARGET_ARCH ?? null,
    skipPrepare: process.env.KNORVIA_SKIP_PREPARE === "1",
    skipBuild: process.env.KNORVIA_SKIP_BUILD === "1",
    dryRun: false,
    positionals: [],
  };
...
async function main() {
  ...
  const buildEnv = {
    KNORVIA_TARGET_OS: os,
    KNORVIA_TARGET_ARCH: arch,
