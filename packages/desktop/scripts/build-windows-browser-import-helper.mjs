...
const targetArch = (process.env.KNORVIA_TARGET_ARCH ?? arch()).toLowerCase();
const targetKey = `win32-${targetArch}`;
const outputDir = resolve(desktopRoot, `bundled-tools/${targetKey}/browser-import`);
const outputPath = resolve(outputDir, "knorvia-browser-import-helper.exe");
...
if (targetArch === "arm64" && !process.env.KNORVIA_CSC_PATH) {
  throw new Error(
    "Windows arm64 browser import helper requires KNORVIA_CSC_PATH pointing to an arm64-capable Roslyn csc.exe",
  );
}
if (!cscPath) {
  throw new Error("Windows C# compiler not found; set KNORVIA_CSC_PATH to a trusted csc.exe");
}
