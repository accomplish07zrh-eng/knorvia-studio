/**
 * 构建期开关：为真时安装包使用 Preview 身份，而后端环境仍由 `KNORVIA_ENV` 单独决定。
 * 典型用法是 `KNORVIA_ENV=production KNORVIA_PREVIEW_IDENTITY=1`，得到一个连接生产后端、
 * 可与正式版并排安装的 `Knorvia Studio Preview`。
 */
export const KNORVIA_PREVIEW_IDENTITY_ENV = "KNORVIA_PREVIEW_IDENTITY";

const PRODUCTION_IDENTITY = Object.freeze({
  flavor: "production",
  appId: "dev.knorvia.studio",
  productName: "Knorvia Studio",
  linuxExecutableName: "knorvia-studio",
  linuxPackageName: "knorvia-studio",
  cuaHelperInstallVariant: null,
});

const PREVIEW_IDENTITY = Object.freeze({
  flavor: "preview",
  appId: "dev.knorvia.studio.preview",
  productName: "Knorvia Studio Preview",
  linuxExecutableName: "knorvia-studio-preview",
  linuxPackageName: "knorvia-studio-preview",
...
export function isPreviewIdentityRequested(env = process.env) {
  ...
  throw new Error(
    `invalid ${KNORVIA_PREVIEW_IDENTITY_ENV}=${env[KNORVIA_PREVIEW_IDENTITY_ENV]}; expected 1 or 0`,
  );
}

/**
 * 产品身份（flavor）与后端环境（`KNORVIA_ENV`）是两个轴：
 * - `KNORVIA_ENV=test` 一律是 Preview，测试后端不能顶着正式 `Knorvia` 身份覆盖用户的正式安装；
 * - `KNORVIA_ENV=production` 默认是正式身份，显式 `KNORVIA_PREVIEW_IDENTITY=1` 时改用 Preview 身份。
 * 未知 `KNORVIA_ENV` 继续按 test 处理，和共享层 normalizeKnorviaEnv 的 fail-safe 默认值一致。
 */
export function resolveDesktopProductFlavor(env = process.env) {
  if (isPreviewIdentityRequested(env)) {
    return "preview";
  }
  return normalizeDesktopKnorviaEnv(env) === "production" ? "production" : "preview";
}
 ...
 * 生产后端的 Preview 包靠 productName（`Knorvia Studio Preview-<version>-...`）与正式包区分。
 */
export function resolveDesktopArtifactSuffix(env = process.env) {
  return normalizeDesktopKnorviaEnv(env) === "test" ? "_TEST" : "";
}
