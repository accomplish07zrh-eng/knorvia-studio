...
import { refreshWindowsAsarIntegrity } from "./scripts/windows-asar-integrity.mjs";
import {
...
const desktopProductIdentity = resolveDesktopProductIdentity({
  ...process.env,
  KNORVIA_ENV: builtinProviderConfig.environment,
});
...
const shouldEnableMacSigning =
  process.env.KNORVIA_ENABLE_MAC_SIGN === "1" && Boolean(macSigningIdentity);
...
const desktopDistDir = process.env.KNORVIA_DESKTOP_DIST_DIR || "dist";
...
const REQUIRED_ASAR_RUNTIME_MODULES = [
  ...
  "pngjs",
  // @knorvia/services 的代理连通性探测会动态 require("undici") 取 ProxyAgent。
...
const WINDOWS_INSTALL_MANIFEST_NAME = ".knorvia-studio-install-manifest";
...
function resolveElectronDownloadMirror(env = process.env) {
  const existingMirror =
    env.KNORVIA_ELECTRON_RUNTIME_MIRROR ||
...
if (
  ...
  process.env.KNORVIA_ENABLE_MAC_SIGN === "1" &&
  !macSigningIdentity
) {
  throw new Error(
    "Knorvia Studio Preview macOS packaging requires APPLE_SIGNING_IDENTITY or CSC_NAME when KNORVIA_ENABLE_MAC_SIGN=1",
  );
}
...
function resolveAppAsarPath(context) {
  if (context.electronPlatformName === "darwin") {
    const appName = `${context.packager?.appInfo?.productFilename ?? "Knorvia Studio"}.app`;
...
async function injectHoistedRuntimeModulesIntoAsar(context) {
  ...
  const stagingDir = mkdtempSync(resolve(tmpdir(), "knorvia-app-asar-"));
  try {
...
export default {
  ...
  afterPack: async (context) => {
    ...
    if (actualWindowsTarget) {
      await refreshWindowsAsarIntegrity(
        ...
        resolve(context.appOutDir, "resources", "app.asar"),
      );
  ...
  extraResources: [
      ...
      ? [
          {
            ...
            // 只是不吸附，所以这里不做存在性断言。
            from: "resources/macos-window-bounds/knorvia-window-bounds",
            to: "macos-window-bounds/knorvia-window-bounds",
          },
        ]
      : []),
    {
    ...
    {
      // Provider Registry 的 Knorvia Built-in Config 是静态 Provider/Model 事实的唯一内置来源。
      // 显式随包发布，避免正式 Host 回退到旧 Catalog/Preset hardcode。
      from: builtinProviderConfig.sourcePath,
      to: "config/provider/builtin.json",
    },
    {
    ...
    ...(targetPlatform.os === "win32"
      ...
      : []),
    {
      // agent 运行时资产，打包到 resources/knorvia。
      // 桌面端内置的是 agent 的 JS bundle（knorvia/knorvia.cjs，由 prepare:agent-bundle 生成），
      // Host 进程用 app 自带的 Electron Node runtime（ELECTRON_RUN_AS_NODE）执行 `knorvia.cjs app-server --stdio`，
      // 不再随包内置独立 Node 二进制。远端 SSH/WSL 仍走原生二进制（无 Electron）。
      from: `bundled-agents/${targetPlatform.key}/knorvia`,
      to: "knorvia",
      filter: ["**/*", "!**/*.map"],
    },
    {
  ...
  protocols: [
    {
      ...
      // 展示名跟随安装包身份；scheme 仍保持 knorvia，因此两个应用中最后注册者会成为默认 handler。
      name: desktopProductIdentity.productName,
      schemes: ["knorvia-studio"],
    },
  ],
  mac: {
    target: ["dmg", "zip"],
  ...
  linux: {
    ...
    // 与 /usr/share/icons/hicolor/*/apps/knorvia.png 保持一致。
