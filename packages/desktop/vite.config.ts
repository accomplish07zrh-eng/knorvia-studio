...
function createE2EUIRendererCoveragePlugin(repoRoot: string): Plugin {
  ...
  return {
    name: "knorvia:e2e-ui-source-coverage",
    enforce: "pre",
...
export default defineConfig(({ mode }) => {
  // `.env*` 只提供链接常量；当前产品环境由启动脚本或 CI 注入 KNORVIA_ENV。
  const env = { ...loadEnv(mode, "../..", ""), ...process.env };
  const repoRoot = resolve(__dirname, "../..");
  const knorviaEnv = resolveKnorviaEnv(env.KNORVIA_ENV);
  ...
  const e2eCoverageEnabled =
    env.KNORVIA_E2E_COVERAGE === "1" || process.env.KNORVIA_E2E_COVERAGE === "1";
  ...
  return {
    ...
    define: {
      ...
      __KNORVIA_LOCAL_DEVELOPMENT_RUNTIME__: JSON.stringify(mode !== "production"),
      // E2E store bridge 只能由 WDIO 专用变量打开，避免把 KNORVIA_ENV=test 产品环境误当成测试运行态。
      "import.meta.env.VITE_KNORVIA_E2E_STORE_BRIDGE": JSON.stringify(
