...
test("save keeps the validated bytes across an asynchronous dialog", async (t) => {
  ...
  const dialogWait = new Promise<{ canceled: boolean; filePath: string }>((resolve) => {
  ...
  assert.deepEqual(
    await handler(
      { sender: { id: 1 } },
      { suggestedName: "blocked.bin", sourceUrl: "http://127.0.0.1/secret" },
    ),
    { success: false, error: "remote_address_not_allowed" },
  );
});
...
test("PDF export returns exact bytes from a pooled Buffer view", async () => {
  ...
  const result = await handler({
    sender: { id: 7, printToPDF: async () => pool.subarray(32, 37) },
  });
  assert.ok(
    result &&
