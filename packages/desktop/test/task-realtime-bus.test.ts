...
test("realtime bus forwards validated wire events to owner and observer with continuous sequence", async (t) => {
  ...
  bus.registerHost({
    hostId: "owner",
    windowId: 1,
    child: owner as unknown as UtilityProcess,
    workspaceKeys: [target.workspaceKey],
  });
  bus.registerHost({
    hostId: "observer",
    windowId: 2,
    child: observer as unknown as UtilityProcess,
    workspaceKeys: [target.workspaceKey],
  });
  owner.publish({
    type: HostResponseTypes.TaskRunLeaseAcquire,
    request: { ...target, leaseRequestId: "lease-1" },
  });
  ...
  const observerBatch = observer
    .delivered()
    .find((event) => event.type === "task_stream_mirror_batch");
  ...
  assert.deepEqual(
    ownerBatch.ops.map((op) => op.seq),
    [1, 2, 3],
  );
  assert.deepEqual(observerBatch.ops, ownerBatch.ops);
  assert.equal(
    ownerBatch.ops[0]?.kind === "stream_event" && ownerBatch.ops[0].event.content,
    "hello world",
  );
  assert.equal(
    ownerBatch.ops[1]?.kind === "stream_event" && ownerBatch.ops[1].event.extra,
    "preserved",
  );
  ...
  assert.ok(
    owner.messages.some(
      (message) =>
        ...
        message !== null &&
        "type" in message &&
        message.type === HostMessageTypes.TaskRunLeaseResult,
    ),
  );
