...
function loadEnvFiles(): Record<string, string> {
  ...
  // 真实环境变量优先级最高
  if (process.env.KNORVIA_ENV) vars.KNORVIA_ENV = process.env.KNORVIA_ENV;
  if (process.env.KNORVIA_BASE_URL) vars.KNORVIA_BASE_URL = process.env.KNORVIA_BASE_URL;
  return {
    ...vars,
    ...Object.fromEntries(
...
export function resolveDesktopTsupBundleSecurityOptions(
  ...
  const isE2ECoverageBuild = runtimeEnv.KNORVIA_E2E_COVERAGE === "1";
  return {
...
function createSharedDefines() {
  return {
    __KNORVIA_VERSION__: JSON.stringify(buildMetadata.appVersion),
    __KNORVIA_COMMIT__: JSON.stringify(buildMetadata.buildCommitId),
    __KNORVIA_BUILD_TIME__: JSON.stringify(buildMetadata.buildTime),
    __KNORVIA_ENV__: JSON.stringify(knorviaEnv),
    __KNORVIA_ENDPOINT_ENV__: JSON.stringify({}),
    __KNORVIA_PRODUCT_FLAVOR__: JSON.stringify(knorviaProductFlavor),
    // Computer Use Helper build identity — helperInstaller 读它决定下载哪个 Helper bundle。
    // 缺失时 installer 抛 "Packaged Knorvia is missing its embedded Computer Use Helper build identity"。
    // CI 构建时通过 KNORVIA_CUA_HELPER_BUILD_ID env 注入；dev 为空串走兜底（dev helper 不走下载）。
    __KNORVIA_CUA_HELPER_BUILD_ID__: JSON.stringify(
      process.env.KNORVIA_CUA_HELPER_BUILD_ID?.trim() ?? "",
    ),
    // 客户端只有一个 CDN 配置，与发布端 OSS 目标列表分离。
    __KNORVIA_CDN_BASE_URL__: JSON.stringify(env.KNORVIA_CDN_BASE_URL?.trim() || ""),
  };
}
...
export default defineConfig([
  {
    name: "main",
    entry: {
      "main/index": "src/main/launcher.ts",
      "main/browserWebmRecorder": "src/main/browserView/electronBrowserWebmRecorder.ts",
      "main/dataSizeWorker": "src/main/dataSizeWorker.ts",
    ...
    noExternal: [
      ...
      // electron-builder 又会排除 node_modules/@knorvia，导致安装包启动即 ERR_MODULE_NOT_FOUND。
  ...
  },
  {
    ...
    platform: "node",
    target: "node22",
    // 与 host 同构：常驻 cron scheduler 进程复用 @knorvia/services（tasks-index + cron），
