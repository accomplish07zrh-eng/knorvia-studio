...
/**
 * window.knorvia 类型定义 —— 仅包含需要 main 进程参与的平台操作
 *
...
declare global {
  interface Window {
    knorvia: Required<
      Pick<
        import("@knorvia/shared").IPlatformService,
    ...
    > & {
      ...
      /** 通过系统原生另存为对话框保存文件 */
      saveFile?(
        payload: import("@knorvia/shared").SaveFileRequest,
      ): Promise<import("@knorvia/shared").SaveFileResult>;
      /** 将当前页面的 print 媒体版面导出为 PDF（Chromium 打印引擎，矢量文本） */
      printPageToPdf?(): Promise<import("@knorvia/shared").PrintPageToPdfResult>;
      ...
      /** 打开 Knorvia Computer Use 完整权限引导 */
      ...
      importChromeBrowserData?(
        options?: import("@knorvia/shared").ChromeBrowserDataImportOptions,
