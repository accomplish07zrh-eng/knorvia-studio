...
const DEV_AGENT_BUNDLE_RELATIVE_PATH = "apps/cli/packages/cli/dist/knorvia.cjs";
const DEV_AGENT_BUNDLE_ENV = "KNORVIA_REMOTE_DEV_AGENT_BUNDLE";
const REMOTE_DEV_AGENT_BUNDLE_NAME = REMOTE_AGENT_BUNDLE_NAME;
const REMOTE_DEV_AGENT_VERSION_FILE_NAME = ".dev-version";

interface DeployDevelopmentKnorviaAgentRuntimeParams {
...
async function computeDevelopmentAgentAssetsSha256(params: {
  ...
  for (const packageName of REMOTE_AGENT_OFFICIAL_PLUGIN_PACKAGE_NAMES) {
    const packageRoot = join(params.repoRoot, "apps", "cli", "packages", packageName);
...
async function shouldSkipDevelopmentKnorviaAgentDeploy(params: {
  backend: IRemoteBackend;
  remoteVersionFile: string;
...
async function stageDevelopmentOfficialPluginPackages(params: {
  ...
  for (const packageName of REMOTE_AGENT_OFFICIAL_PLUGIN_PACKAGE_NAMES) {
    const sourceRoot = join(params.repoRoot, "apps", "cli", "packages", packageName);
...
async function uploadDevelopmentOfficialPluginPackages(params: {
  ...
  try {
    ...
    params.loggers.log(`[agent-deploy] ${KNORVIA_AGENT_PROVIDER}: 开发态上传官方插件资源`);
    try {
...
export async function deployDevelopmentKnorviaAgentRuntime(
  backend: IRemoteBackend,
  params: DeployDevelopmentKnorviaAgentRuntimeParams,
  loggers: DeployLoggers,
): Promise<boolean> {
  ...
  if (
    await shouldSkipDevelopmentKnorviaAgentDeploy({
      backend,
  ...
  // 开发态 SSH 远端过去只会部署本地 knorvia.cjs，不会携带 packages/*-plugin。
  // builtin plugin seed 依赖 agent 包旁边的官方插件源资源，所以 dev 部署需要同步 bundle 与插件资源。
  // 本地修改 apps/cli 后，远端测试仍运行滞后的发布包。这里改为上传 dev 启动时刚构建的
  // dist/knorvia.cjs，并用远端已部署的 node 包一层 wrapper 启动，保证 agent 仍运行在目标机器内。
  loggers.log(
