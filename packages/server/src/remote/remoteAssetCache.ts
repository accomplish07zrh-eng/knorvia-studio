...
const REMOTE_COMPONENT_MOUNT_RULES: Record<string, ComponentMountRule> = {
  ...
  knorvia: {
    platformScoped: true,
    resolveExpectedMount: (platformArch) => `knorvia/${platformArch}`,
  },
  bfs: {
    platformScoped: true,
...
export function createRemoteAssetManifestRequestSignal(
  ...
  // manifest 是 server-bundle/Knorvia Agent SHA 跳过判断的前置输入；网络半开时若不主动取消，
...
async function ensureRemoteComponentDirFromCdnInternal(
  ...
  if (forceRefresh) {
    // App 版本变化代表一次新的资源发布边界；即使 Knorvia Agent SHA cache 命中，
  ...
  if (!forceRefresh && initialMissingPaths) {
    ...
    // knorvia.cjs 的 knorvia cache 后，再补传 packages 会一直复用残缺 cache。
...
export async function parseRemoteAssetManifestFromResponse(
  ...
  for (const [index, rawComponent] of rawComponents.entries()) {
    ...
    if (!mountRule) {
      ...
      // 当前客户端只认识 Knorvia Agent 与基础运行时，未知组件应跳过，不能阻断当前组件下载。
      continue;
    }
...
function resolveContentAddressedReleaseSegments(
  ...
  for (const componentId of Object.keys(CONTENT_ADDRESSED_COMPONENT_RELEASE_DIRS)) {
    // 兼容历史全量 release：未指定组件时只沿用原有 Knorvia Agent 内容目录；server
...
async function writeResponseBodyToFile(
  ...
  // 此处运行于 Node；显式使用其 Web Stream 声明，避免 DOM 与 Node 的 BYOB 重载冲突。
  await pipeline(Readable.fromWeb(response.body as ReadableStream<Uint8Array>), byteMeter, output);
...
async function isValidReleaseDir(releaseDir: string, platformArch: string): Promise<boolean> {
  ...
  if (!(await fileExists(releaseDir, "server", "knorvia-server.cjs"))) {
    return false;
  }
...
export function resolveFallbackRemoteAssetCacheDir(): string {
  return join(tmpdir(), "knorvia-remote-assets-cache");
}
