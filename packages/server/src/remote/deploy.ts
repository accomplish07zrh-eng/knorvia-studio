...
/**
 * Deploy the knorvia server to the remote machine.
...
export async function deployServer(
  ...
  const getLocalManifestRef = (): Promise<RemoteAssetManifestRef | null> => {
    ...
    // 后续 Knorvia Agent 身份判断与 release materialize 仍复用同一份快照。
  ...
  const deployWithDecision = async (
    ...
    if (shouldForceRefreshContentAddressedAssets) {
      // server 会先于 Knorvia Agent 更新；若后续步骤失败，下次连接时 server 版本
      // 已经匹配。必须持久化升级强刷状态，让重试继续绕过同 SHA cache，直到 Knorvia Agent 成功覆盖 marker。
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/knorvia-server.cjs 可启动，不代表随包工具仍存在。
      // knorvia 内容跟随 app/server 版本刷新；但 wrapper/bundle 被清理或开发态 bundle 变化时仍要按实体检查修复。
    ...
    await installer.installFile({
      ...
      // 判断 server-bundle 可复用；与 Knorvia Agent 一致，必须重新下载并校验当前 manifest 制品。
    ...
    // 部署 Knorvia Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployKnorviaAgentRuntime(
      backend,
      env,
      {
        ...
        platformArch,
        installer,
        // 旧版 App 会覆盖 agents/knorvia，却不会同步新版引入的 Knorvia Agent SHA marker。
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    log("remote version:", JSON.stringify(version), "local:", KNORVIA_VERSION);
    if (version !== KNORVIA_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${KNORVIA_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", KNORVIA_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/knorvia-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "knorvia":
        requiredPaths.add(`knorvia/${platformArch}/knorvia.cjs`);
        for (const relativePath of REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS) {
          requiredPaths.add(`knorvia/${platformArch}/packages/${relativePath}`);
        }
        break;
      case "bfs":
