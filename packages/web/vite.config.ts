...
import {
  resolveRuntimeKnorviaEndpointOrigin,
  pickProductEndpointEnv,
} from "@knorvia/shared/endpoint";
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      __KNORVIA_ENDPOINT_ENV__: JSON.stringify(pickProductEndpointEnv(env)),
      __KNORVIA_VERSION__: JSON.stringify(version),
      __KNORVIA_COMMIT__: JSON.stringify(env.KNORVIA_COMMIT || "unknown"),
      __KNORVIA_ENV__: JSON.stringify(knorviaEnv),
      "import.meta.env.VITE_KNORVIA_BASE_URL": JSON.stringify(knorviaEndpointOrigin),
      // 兼容旧 Web runtime 读取名；新代码统一读 VITE_KNORVIA_BASE_URL。
      "import.meta.env.VITE_KNORVIA_ENDPOINT_ORIGIN": JSON.stringify(knorviaEndpointOrigin),
    },
    build: {
