...
import type { IPlatformService, RemoteTarget, ServerRemoteInfo } from "@knorvia/shared";
import {
  AppErrorBoundary,
  Root,
  KnorviaIntlProvider,
...
function resolveWebThemePreference(defaultTheme: Theme = WEB_DEFAULT_THEME): Theme {
  const saved = localStorage.getItem("knorvia-theme");
...
// system 模式下需要查询系统偏好；非 system 模式直接用存储值
{
  ...
  const saved = resolveWebThemePreference(undefined);
  const resolved =
    saved === "system"
      ? window.matchMedia("(prefers-color-scheme: dark)").matches
        ? "dark"
        : "light"
      : saved === "dark" || saved === "knorvia-dark"
        ? "dark"
        : "light";
  const appliedTheme =
    saved === "system"
