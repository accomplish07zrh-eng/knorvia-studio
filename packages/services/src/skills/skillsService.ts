...
const getSkillSettingsDir = () => join(getKnorviaDataRootDir(), "v2");
const getSkillCliSettingsDir = () => join(getKnorviaDataRootDir(), "cli");
const getSkillCliConfigFile = () => join(getSkillCliSettingsDir(), "config.json");
const GIT_MARKER = ".git";
const HOME_PREFIX = "~/";
const KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE = "knorvia-plugins-bundled";
const KNORVIA_INLINE_PLUGIN_MARKETPLACE = "inline";
const KNORVIA_PLUGIN_MANIFEST_PATH = join(".knorvia-plugin", "plugin.json");
...
/** 对齐 apps/cli/packages/adapters/src/skills/index.ts:19 */
...
async function isUserAgentsSkillCoveredByKnorvia(params: {
  skillPath: string;
  rootPath: string;
  userKnorviaSkillNameKeys: Set<string>;
}): Promise<boolean> {
  const { skillPath, rootPath, userKnorviaSkillNameKeys } = params;
 ...
 * `.knorvia-studio/skills` 与 `.agents/skills` 都收集起来。
 * 对齐 apps/cli/packages/adapters/src/skills/roots.ts:60-72。
...
async function resolveAncestorWorkspaceRoots(workspacePath: string): Promise<string[]> {
  ...
  for (const dir of baseDirectories) {
    ...
    // 当成 `.knorvia-studio` 的 fallback，导致同层 `.knorvia-studio` 只要有一个技能，`/`、`$` 和设置页
    // 就会整根漏掉 `.agents` 技能，形成“模型可执行但 UI 无法引用”的发现语义分裂。
    roots.push(getWorkspaceKnorviaSkillRoot(dir));
...
async function appendSkillsAuditLog(params: {
  ...
  await mkdir(getSkillSettingsDir(), { recursive: true });
  await appendFile(
    join(getSkillSettingsDir(), "skills-audit.log"),
    `${JSON.stringify({
      createdAt: Date.now(),
...
function readStorageDirFromConfig(config: Record<string, unknown>): string {
  // 便携运行的私有插件数据始终跟随所选资料目录。
  if (process.env.KNORVIA_PORTABLE_DIR?.trim()) return getKnorviaDataRootDir();
...
async function scanOfficialPluginCacheRoots(pluginStorageRoot: string): Promise<string[]> {
  const cacheRoot = join(pluginStorageRoot, "cache", KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE);
...
async function findPluginManifestPath(rootPath: string): Promise<string | null> {
  for (const manifestPath of [
    join(rootPath, KNORVIA_PLUGIN_MANIFEST_PATH),
...
async function resolvePluginSkillRootDescriptors(): Promise<SkillRootDescriptor[]> {
  ...
  for (const candidate of candidates) {
    ...
    // 仍会从 cache 贡献技能。
    if (
      candidate.marketplace === KNORVIA_OFFICIAL_PLUGIN_MARKETPLACE &&
...
async function discoverSkills(params: {
  ...
  if (params.includeUserSkills) {
    // 用户级技能是全局资源，`.knorvia-studio/skills` 里只要存在一个技能就截断
    // `.agents/skills` 会导致外部 Agent 的全局技能在导入后从设置页消失。
    roots.push({
      scope: "user" as const,
      rootPath: getUserKnorviaSkillRoot(),
    });
    roots.push({
      scope: "user" as const,
  ...
  const userKnorviaSkillNameKeys = params.includeUserSkills
    ? await collectSkillNameKeysInRoot(getUserKnorviaSkillRoot())
    : new Set<string>();
  ...
  for (const rootPath of scanRootPaths) {
    ...
    for (const skillPath of skillPaths) {
      if (
        await isUserAgentsSkillCoveredByKnorvia({
          skillPath,
          rootPath: root.rootPath,
          userKnorviaSkillNameKeys,
        })
      ) {
        continue;
      }
      ...
      // 这些字段不影响 Knorvia 读取 name/description，继续报 warning 只会制造无操作价值的噪音。
...
function resolveCapabilities(options?: SkillsServiceOptions): SkillsCapability {
  const isDesktopRuntime = options?.isDesktopRuntime ?? Boolean(process.env.KNORVIA_PROCESS_LABEL);
  if (isDesktopRuntime) {
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async copyToCommon(params: {
      ...
      const commonRoot =
        skill.scope === "workspace"
          ? getWorkspaceKnorviaSkillRoot(params.workspacePath)
    ...
    async removeFromCommon(params: {
      ...
      const userCommonRoot = getUserKnorviaSkillRoot().replaceAll("\\", "/").toLowerCase();
      const workspaceCommonRoot = getWorkspaceKnorviaSkillRoot(params.workspacePath)
        .replaceAll("\\", "/")
        .toLowerCase();
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.knorvia-studio/skills/<name>` 下的目录项本身。
      ...
      // 收集工作区各层级（沿 worktree 向上）的 .knorvia-studio/skills 与 .agents/skills，外加用户级两根。
      const allowedRootCandidates = await resolveAncestorWorkspaceRoots(params.workspacePath);
      allowedRootCandidates.push(getUserKnorviaSkillRoot());
