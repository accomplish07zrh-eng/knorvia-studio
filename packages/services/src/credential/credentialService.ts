...
async function readAll(credentialsFile = getCredentialsFile()): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read Knorvia Studio credentials: ${credentialsFile}`, {
      cause: error,
    });
  }
  ...
  try {
    ...
    // 把损坏 JSON/schema 当成空 store 后继续 save 会清空其他第三方 OAuth 与 API 凭据。
    ...
    throw new Error(`Knorvia Studio credentials are corrupt: ${credentialsFile}`, { cause: error });
  }
}
