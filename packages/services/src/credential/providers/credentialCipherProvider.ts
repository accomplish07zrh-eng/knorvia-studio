...
import { CREDENTIAL_DECRYPT_ERROR_CODE, CREDENTIAL_DECRYPT_ERROR_PREFIX } from "@knorvia/shared";
...
const CREDENTIAL_SECRET_ENV_KEY = "KNORVIA_CREDENTIAL_SECRET";
...
function defaultCredentialSecret(env: NodeJS.ProcessEnv): string {
  ...
  return `knorvia-studio-credential-fallback:${platform()}:${homedir()}:${username}`;
}
