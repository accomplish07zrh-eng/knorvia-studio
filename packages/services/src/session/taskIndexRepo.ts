...
function appendKnorviaAgentIndexedProviderFilter(
  where: string[],
...
const WORKSPACE_BOOTSTRAP_TASK_GROUP_COLORS = [
  ...
  "blue",
  "purple",
] satisfies KnorviaTaskGroupColor[];
...
function isTerminalTaskStatus(status: KnorviaTaskMeta["status"]): boolean {
  return status === "completed" || status === "error";
}

function shouldPreserveNewerTerminalStatus(
  existingMeta: KnorviaTaskMeta | null,
  incomingMeta: KnorviaTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): KnorviaTaskMeta {
  ...
  try {
    ...
    if (parsed.success) {
      return {
        ...(parsed.data as KnorviaTaskMeta),
  ...
  return {
    ...
    migrationSource: (row.migration_source as KnorviaTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as KnorviaTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: KnorviaTaskMeta): string {
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): KnorviaTaskListItem {
...
function isTaskGroupColor(value: string): value is KnorviaTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): KnorviaTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): KnorviaTaskGroupColor {
...
function groupedTopNodeOrderRef(node: KnorviaGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareGroupedNodes(
  left: KnorviaGroupedTaskViewNode,
  right: KnorviaGroupedTaskViewNode,
): number {
...
function compareCronGroupTasks(left: KnorviaTaskListItem, right: KnorviaTaskListItem): number {
...
function compareGroupTasks(
  left: KnorviaTaskListItem,
  right: KnorviaTaskListItem,
...
export class TaskIndexRepo {
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendKnorviaAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): KnorviaTaskMeta {
      ...
      try {
        ...
        const meta: KnorviaTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 Knorvia session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 Knorvia 任务。
   ...
   */
  private ensureCronGroupMembership(meta: KnorviaTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<KnorviaTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: KnorviaTaskMeta): Promise<KnorviaTaskMeta> {
    await this.ensureReady();
  ...
  async clearTaskUnreadIfMatches(params: {
    ...
    return this.enqueueWrite(params, () => {
      ...
      try {
        ...
        const nextMeta: KnorviaTaskMeta = {
          ...current,
          unreadAt: undefined,
        };
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<KnorviaTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<KnorviaTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: KnorviaGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: KnorviaGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 Knorvia Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendKnorviaAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
    ...
    const nodes: KnorviaGroupedTaskViewNode[] = groups.map((group) => {
  ...
  async queryGroupedTaskViewStructure(params: {
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: KnorviaGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
  ...
  async applyGroupedTaskViewOrder(
    ...
    const validateTaskRef = (task: KnorviaGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 Knorvia Agent 视图只接受当前 Knorvia task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: KnorviaGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: KnorviaGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: KnorviaGroupedTaskRef[] = [];
