...
import {
  ...
  knorviaTaskModeSchema,
  type KnorviaAutomation,
  type KnorviaAutomationCreateParams,
  type KnorviaAutomationDispatchStatus,
  type KnorviaAutomationLifecycleStatus,
  type ModelSelection,
  type KnorviaAutomationRun,
  type KnorviaAutomationRunDispatchStatus,
  type KnorviaAutomationRunOutcome,
  type KnorviaAutomationTrigger,
  type KnorviaAutomationUpdateParams,
} from "@knorvia/shared";
...
function rowToAutomation(row: AutomationRow): KnorviaAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as KnorviaAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as KnorviaAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as KnorviaAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): KnorviaAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: KnorviaAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): KnorviaAutomation["mode"] | undefined {
  const parsed = knorviaTaskModeSchema.safeParse(mode);
...
function rowToRun(row: AutomationRunRow): KnorviaAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as KnorviaAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as KnorviaAutomationRunDispatchStatus,
    outcome: (row.outcome as KnorviaAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  // 互相覆盖全局值，导致 repo 与裸 SQL 操作在并发窗口内写进真实库 ~/.knorvia-studio/v2（历史脏数据
  ...
  async create(
    ...
    options: { nextRunAt: number | null; lifecycleStatus?: KnorviaAutomationLifecycleStatus },
  ...
  async claimDue(now: number): Promise<KnorviaAutomation[]> {
    ...
    try {
      ...
      const claimed: KnorviaAutomation[] = [];
