/* eslint-disable max-lines -- Knorvia task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@knorvia/rpc";
import { ServiceChannels } from "@knorvia/shared";
import type { CommandPayloadMap } from "@knorvia/shared/protocol-v4";
...
import type {
  ...
  KnorviaTaskCreateResult,
  KnorviaTaskMeta,
  KnorviaStreamEvent,
  KnorviaTaskMode,
  KnorviaConfigOption,
  KnorviaError,
  KnorviaAssistantMessageFeedback,
  ...
  KnorviaTaskSnapshot,
  KnorviaTaskSnapshotBody,
  KnorviaTaskSnapshotToolCallsSlice,
  KnorviaTaskSnapshotRefContent,
  KnorviaEnqueueTaskCommandResult,
  KnorviaCancelTaskCommandResult,
  KnorviaTaskClientMode,
  KnorviaTaskTokenUsageResult,
...
export interface KnorviaTaskSnapshotWithEtagResult {
  snapshot: KnorviaTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface KnorviaArchivedTaskDeletionResult {
...
export type KnorviaModelTrajectoryContentPart =
...
export interface KnorviaModelTrajectoryMessage {
  role: string;
  parts: KnorviaModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface KnorviaModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type KnorviaModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface KnorviaModelTrajectoryCallSource {
  kind: KnorviaModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface KnorviaModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: KnorviaModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: KnorviaModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: KnorviaModelTrajectoryContentPart[];
    usage?: KnorviaModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface KnorviaModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 Knorvia Agent 落盘 model-io）。 */
  available: boolean;
  records: KnorviaModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface KnorviaTaskTerminalOutcome {
  taskId: string;
...
export interface KnorviaTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IKnorviaTaskService — Knorvia task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * Knorvia Agent server 维护，新功能应优先走 IKnorviaSessionService。
 */
export interface IKnorviaTaskService {
  // ---- 生命周期 ----
  ...
  /** 创建 Knorvia session 并同步 task 索引。 */
  createTask(params: {
    workspacePath: string;
    workspaceIdentity?: string;
    provider?: KnorviaProvider;
  ...
  }): Promise<KnorviaTaskCreateResult>;
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<KnorviaEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: KnorviaTaskClientMode;
  }): Promise<KnorviaCancelTaskCommandResult>;
  ...
  }): Promise<KnorviaSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<KnorviaSessionGoalResult>;
   ...
   */
  listTaskList(params: KnorviaTaskListQuery): Promise<KnorviaTaskListResult>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<KnorviaTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<KnorviaTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<KnorviaTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<KnorviaTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<KnorviaTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskMeta(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<KnorviaTaskMeta | null>;

  /** 读取当前 active task 内存态的配置选项，不做 workspace 预热兜底 */
  getTaskConfigOptions(params: { taskId: string }): Promise<KnorviaConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: KnorviaAssistantMessageFeedback | null;
  }): Promise<KnorviaSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: KnorviaTaskMode }): Promise<void>;
  ...
  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.knorvia-studio/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 Knorvia Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<KnorviaTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 Knorvia Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  deleteArchivedTasks(params: {
    ...
    workspaceIdentity?: string;
    taskIds: string[];
  }): Promise<KnorviaArchivedTaskDeletionResult>;
  ...
  onDynamicWorkspaceEvent(
    workspace: string | KnorviaWorkspaceEventSubscriptionParams,
  ): Event<KnorviaWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<KnorviaError>;
}

export const IKnorviaTaskService = createServiceDescriptor<IKnorviaTaskService>(
  ServiceChannels.KnorviaTask,
);
