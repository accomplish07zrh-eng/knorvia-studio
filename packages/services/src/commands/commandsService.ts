...
const DEFAULT_COMMAND_AGENT_SOURCE: CommandAgentSource = KNORVIA_COMMAND_AGENT_SOURCE;
const COMMAND_AGENT_SOURCE_ORDER: readonly CommandAgentSource[] = KNORVIA_COMMAND_AGENT_SOURCES;
...
const KNORVIA_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  agentSource: "agent",
  directorySource: "knorvia",
  userDirectorySegments: [".knorvia-studio", "commands"],
  workspaceDirectorySegments: [".knorvia-studio", "commands"],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
function getUserCommandsRoot(agentSource?: CommandAgentSource): string {
  ...
  return getUserCommandsRootForDescriptor(descriptor);
}
...
function getUserCommandsRootForDescriptor(descriptor: CommandAgentSourceDescriptor): string {
  return descriptor.directorySource === "knorvia"
    ? join(getKnorviaDataRootDir(), "commands")
    : join(resolveUserHomeDir(), ...descriptor.userDirectorySegments);
}
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // Knorvia Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让 user .knorvia-studio 抢在 workspace .agents 前面。
    for (const agentSource of agentSources) {
      const descriptors =
        agentSource === KNORVIA_COMMAND_AGENT_SOURCE
    ...
    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as KnorviaCommand[],
...
async function discoverPluginCommandsRecursive(
  ...
  for (const entry of entries) {
    ...
    if (!entry.toLowerCase().endsWith(KNORVIA_COMMAND_DESCRIPTOR.fileExtension)) {
      continue;
    }
    ...
    try {
      ...
      const parsed = CommandFileParser.parseCommandFile(
        content,
        fullPath,
        KNORVIA_COMMAND_DESCRIPTOR.format,
      );
      if (!parsed) {
        continue;
      }
      const name = getCommandName(rootDir, fullPath, KNORVIA_COMMAND_DESCRIPTOR);
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // `.knorvia-studio` 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
    if (descriptor.directorySource === "knorvia" && discoveredCount > 0) {
      break;
    }
  }
}
