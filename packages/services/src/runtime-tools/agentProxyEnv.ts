import {
  ...
  KNORVIA_WORKSPACE_IDENTITY_ENV,
} from "@knorvia/shared";
...
// HTTP_PROXY → … 先匹配大写）；额外设 KNORVIA_HTTP_PROXY 让 Bash 工具子进程经 subprocess-env 拿到最高优先级。
//
// No Proxy：只接受设置页显式填写的绕过规则。额外设 KNORVIA_NO_PROXY 让 provider/http adapter
// 能在不读取用户 shell NO_PROXY 的前提下复用同一套规则。
//
// 自定义证书：只接受设置页显式填写的 PEM 路径。注入 NODE_EXTRA_CA_CERTS 让 agent（含模型 provider 请求）
// 在 Node 启动时信任它，同时用 KNORVIA_AGENT_CA_CERT 给 adapter 和工具子进程补齐跨运行时 CA 变量。
...
const NO_PROXY_ENV_KEYS = ["NO_PROXY", "no_proxy", KNORVIA_NO_PROXY_ENV_KEY] as const;
...
function buildAgentCaCertEnv(caCertPath: string | undefined): Record<string, string> {
  ...
  return {
    // NODE_EXTRA_CA_CERTS 必须在 Node 进程启动前存在；KNORVIA_AGENT_CA_CERT 则给运行时配置、
    // provider fetch 和工具子进程使用，避免清洗标准 CA 环境后丢失 app 显式注入的证书路径。
    NODE_EXTRA_CA_CERTS: trimmed,
    [KNORVIA_AGENT_CA_CERT_ENV_KEY]: trimmed,
  };
}
...
export function buildAgentWorkspaceIdentityEnv(
  ...
  return trimmed ? { [KNORVIA_WORKSPACE_IDENTITY_ENV]: trimmed } : {};
}
