...
const KNORVIA_PLUGIN_MANIFEST_PATH = [".knorvia-plugin", "plugin.json"] as const;
...
function getWorkspaceKnorviaSkillRoot(workspacePath: string): string {
  return join(workspacePath, ".knorvia-studio", "skills");
}

function getUserKnorviaSkillRoot(): string {
  return join(getKnorviaDataRootDir(), "skills");
}

function getWorkspaceKnorviaCommandRoot(workspacePath: string): string {
  return join(workspacePath, ".knorvia-studio", "commands");
}

function getUserKnorviaCommandRoot(): string {
  return join(getKnorviaDataRootDir(), "commands");
}

function getWorkspaceKnorviaPluginRoot(workspacePath: string): string {
  return join(workspacePath, ".knorvia-studio", "plugins");
}

function getUserKnorviaPluginRoot(): string {
  return join(getKnorviaDataRootDir(), "plugins");
}

function getUserKnorviaCliConfigPath(): string {
  return join(getKnorviaDataRootDir(), "cli", "config.json");
}

function getWorkspaceKnorviaConfigPath(workspacePath: string): string {
  return join(workspacePath, ".knorvia-studio", "config.json");
}
...
function getUserKnorviaAgentsFilePath(): string {
  return join(getKnorviaDataRootDir(), "AGENTS.md");
}
...
function resolveTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserKnorviaSkillRoot();
  }
  return workspacePath ? getWorkspaceKnorviaSkillRoot(workspacePath) : null;
}
...
function resolveCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceKnorviaSkillRoot(workspacePath)
      : null
    : getUserKnorviaSkillRoot();
}
...
function resolveCommandTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserKnorviaCommandRoot();
  }
  return workspacePath ? getWorkspaceKnorviaCommandRoot(workspacePath) : null;
}
...
function resolveCommandCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceKnorviaCommandRoot(workspacePath)
      : null
    : getUserKnorviaCommandRoot();
}
...
function resolvePluginTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserKnorviaPluginRoot();
  }
  return workspacePath ? getWorkspaceKnorviaPluginRoot(workspacePath) : null;
}
...
function resolvePluginCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceKnorviaPluginRoot(workspacePath)
      : null
    : getUserKnorviaPluginRoot();
}
...
function resolvePluginConfigPathForScope(
  ...
  if (targetScope === "global") {
    return getUserKnorviaCliConfigPath();
  }
  return workspacePath ? getWorkspaceKnorviaConfigPath(workspacePath) : null;
}
...
async function findPluginManifestPath(pluginPath: string): Promise<string | null> {
  const knorviaManifestPath = join(pluginPath, ...KNORVIA_PLUGIN_MANIFEST_PATH);
  if (await pathExists(knorviaManifestPath)) {
    return knorviaManifestPath;
  }
...
function readKnorviaMcpServers(parsed: Record<string, unknown>): Record<string, McpServerConfig> {
...
async function collectExistingMcpServerNameKeys(
  ...
  if (targetConfigPath) {
    for (const name of Object.keys(
      readKnorviaMcpServers(await readJsonFileOrEmpty(targetConfigPath)),
    )) {
...
async function addMcpServerToKnorviaConfig(
  filePath: string,
  name: string,
  config: McpServerConfig,
): Promise<void> {
  ...
  const servers = readKnorviaMcpServers(parsed);
...
async function importMcpServersForAgent(
  ...
  for (const candidate of candidates) {
    ...
    try {
      await addMcpServerToKnorviaConfig(selectedConfigPath, candidate.name, candidate.config);
      importedCount += 1;
...
async function getClaudeAgentsFileMigrationStatus(): Promise<SettingsSyncClaudeAgentsFileMigrationStatus> {
  ...
  const targetPath = getUserKnorviaAgentsFilePath();
...
async function copyClaudeAgentsFileToKnorviaAgentsFile(params: {
  overwrite?: boolean;
...
export function createSettingsSyncService(
  ...
  return {
    ...
    async copyClaudeAgentsFileToKnorviaAgentsFile(
      request = {},
    ): Promise<SettingsSyncClaudeAgentsFileCopyResult> {
      const result = await copyClaudeAgentsFileToKnorviaAgentsFile({
