...
function resolveDeviceStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".knorvia-studio", "v2", "telemetry-state.json");
  }
...
function resolveDeviceStateLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".knorvia-studio", "v2", "telemetry-state.lock");
  }
 ...
 * 远端 knorvia-server 没有 Desktop main 进程，由 stdio entry 启动时调用本函数补写，
