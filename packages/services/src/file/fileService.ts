...
const SCRATCH_WORKSPACE_ROOT_NAME = "projects";
 ...
 * 校验 rootPath + .knorviaignore 的 mtime/size 指纹；规则编辑后缓存失效，
 * 保持"编辑规则后下次使用生效"的契约。
 */
...
export function createFileService(options: CreateFileServiceOptions = {}): IFileService {
  ...
  // 全量扫描 + 打包（带 60s TTL / .knorviaignore 指纹缓存）。分块 RPC 共享同一份 packed。
  ...
  return {
    ...
    async createDefaultWorkspace(): Promise<{ path: string }> {
      const workspacePath = join(getKnorviaDataRootDir(), "workspace", SCRATCH_WORKSPACE_ROOT_NAME);
