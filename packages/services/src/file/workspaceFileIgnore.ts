 ...
 *
 * `.knorviaignore`（workspace root，gitignore 语法）是搜索索引的唯一规则文件：
...
interface WorkspaceFileSearchIgnoreContent {
  content: string;
  /** file：.knorviaignore 已存在；template：尚未创建，content 是保存后将落盘的初始内容预览。 */
...
/**
 * 构建 `.knorviaignore` 初始内容：.gitignore 规则拷贝 + 双标记分区（默认排除段 / 自定义区）。
...
/**
 * 扫描前加载 `.knorviaignore` 规则；含自动创建与 fail-open 降级链：
