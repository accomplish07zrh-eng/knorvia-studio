...
// to @knorvia/cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @knorvia/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @knorvia/services root
...
// at compile time; Vite never resolves @knorvia/cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
