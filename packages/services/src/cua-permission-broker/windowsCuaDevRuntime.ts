...
const DEV_ROOT_ENV = "KNORVIA_CUA_DEV_ROOT";
const EXPECTED_PACKAGE_NAME = "@knorvia/cua";
...
async function requireExpectedPackage(
  ...
  try {
    ...
    const contract = pkg.knorviaCuaRuntime;
    if (
  ...
  throw new WindowsCuaDevRuntimeResolutionError(
    "invalid-package",
    `Windows CUA development root package.json must name ${EXPECTED_PACKAGE_NAME} and expose a valid knorviaCuaRuntime contract.`,
    PACKAGE_JSON,
  );
}
