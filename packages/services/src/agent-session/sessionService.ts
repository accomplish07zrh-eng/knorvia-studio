/* eslint-disable max-lines -- sessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
...
import type {
  ...
  KnorviaSessionReadWorkspacePresentationParams,
...
import { createKnorviaSessionApiRetryRuntimeTracker } from "#src/agent-session/sessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/agent-session/importedClaudeSessionRepair.js";
import { createKnorviaDeferredDraftRegistry } from "#src/agent-session/sessionDraftRegistry.js";
...
export function createKnorviaSessionService({
  ...
  const { withApiRetryRuntime } = createKnorviaSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createKnorviaDeferredDraftRegistry();

  function notifySyncer(
    target: KnorviaTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: KnorviaSessionStateSnapshot) {
  ...
  // sessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[agent-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: KnorviaSessionStateSnapshot,
    params: KnorviaSessionResumeParams | KnorviaSessionReadParams,
  ...
  >(params: T): Promise<T> {
    const creationServer =
      !params.workspaceIdentity && !params.remoteSessionId
        ? await creationMcpServer?.(params.workspacePath)
        : undefined;
    const supplied = creationServer
      ? [
          ...(params.mcpServers ?? []).filter((server) => server.name !== creationServer.name),
          creationServer,
        ]
      : params.mcpServers;
    const mcpServers = appendWorkspaceToFilesystemMcpServers(supplied, params.workspacePath);
  ...
  return {
    async initializeWorkspace(params: KnorviaSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: KnorviaSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: KnorviaSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: KnorviaSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[agent-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[agent-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 Knorvia task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[agent-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: KnorviaSessionResumeParams) {
      const startedAt = Date.now();
    ...
    listSessions(params: KnorviaSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: KnorviaSessionReadParams) {
      const startedAt = Date.now();
    ...
    readSessionMessages(params: KnorviaSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: KnorviaSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: KnorviaTaskTarget) {
    ...
    closeSession(params: KnorviaTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: KnorviaTaskTarget) {
      try {
    ...
    async setModel(params: KnorviaSessionSetModelParams) {
    ...
    async setThoughtLevel(params: KnorviaSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: KnorviaSessionSetModeParams) {
