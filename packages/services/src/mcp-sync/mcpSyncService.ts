...
const KNORVIA_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  source: "knorvia",
  directorySource: "knorvia",
  userConfigDirSegments: [".knorvia-studio", "cli"],
  workspaceConfigDirSegments: [".knorvia-studio"],
...
export function createMcpSyncService(
  ...
  return {
    ...
    async checkRemoteUserMcpWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(dirname(getUserKnorviaMcpConfigPath()));
    },
...
function buildDirectoryConfigPath(
  ...
  if (scope === "user" && descriptor.directorySource === "knorvia") {
    return join(getKnorviaDataRootDir(), "cli", descriptor.fileName);
  }
...
function getUserKnorviaMcpConfigPath(): string {
  return buildUserConfigPath(KNORVIA_MCP_DESCRIPTOR);
}
...
async function collectEffectiveUserMcpRecords(): Promise<UserMcpRecord[]> {
  const knorviaRecords = await readUserMcpRecordsFromFile(KNORVIA_MCP_DESCRIPTOR);
  if (knorviaRecords.length > 0) {
    return sortMcpRecords(knorviaRecords);
  }
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  if (payload.action === "set-enabled") {
    ...
    const location =
      payload.location ??
      buildDirectoryMcpLocation(KNORVIA_MCP_DESCRIPTOR, scope, payload.projectPath);
  ...
  await writeKnorviaServersToFile(scope, nextServers, payload.projectPath);
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  const knorviaServers = await readDirectoryServersFromFile(
    KNORVIA_MCP_DESCRIPTOR,
    scope,
    workspacePath,
  );
  if (knorviaServers.length > 0) {
    return knorviaServers;
  }
...
async function writeKnorviaServersToFile(
  scope: Exclude<McpScope, "common">,
  servers: Record<string, Record<string, unknown>>,
  workspacePath?: string,
): Promise<void> {
  const filePath = buildDirectoryConfigPath(KNORVIA_MCP_DESCRIPTOR, scope, workspacePath);
  const current = (await readJsonObject(filePath)) ?? {};
  const next = writeServerMapToJson(current, KNORVIA_MCP_DESCRIPTOR.configKeyName, servers);
  await writeTextAtomic(filePath, `${JSON.stringify(next, null, 2)}\n`);
}

async function readUserCliConfig(): Promise<Record<string, unknown>> {
  return (await readJsonObject(getUserKnorviaMcpConfigPath())) ?? {};
}

async function writeUserCliConfig(config: Record<string, unknown>): Promise<void> {
  await writeTextAtomic(getUserKnorviaMcpConfigPath(), `${JSON.stringify(config, null, 2)}\n`);
}
...
async function importMcpServers(params: {
  ...
  const targetPath = getUserKnorviaMcpConfigPath();
  const current = (await readJsonObject(targetPath)) ?? {};
  const targetServers = readServerMapFromJson(current, KNORVIA_MCP_DESCRIPTOR.configKeyName);
  const existingByName = await collectEffectiveUserMcpRecordByName();
  ...
  if (changed) {
    await writeTextAtomic(
      targetPath,
      `${JSON.stringify(writeServerMapToJson(current, KNORVIA_MCP_DESCRIPTOR.configKeyName, targetServers), null, 2)}\n`,
    );
  }
