...
export function createSkillSyncService(options?: { maxArchiveBytes?: number }): ISkillSyncService {
  ...
  return {
    ...
    async listRemoteUserSkillStatuses(params): Promise<SkillSyncRemoteStatusResult> {
      const root = getUserKnorviaSkillRoot();
    ...
    async checkRemoteUserSkillWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(getUserKnorviaSkillRoot());
    },
...
async function collectUserSkillCandidates(): Promise<SkillSyncCandidate[]> {
  ...
  const userKnorviaSkillRoot = getUserKnorviaSkillRoot();
  const rawKnorviaCandidates = await collectUserSkillCandidatesInRoot(userKnorviaSkillRoot);
  const knorviaDirectoryNames = await collectCoveredSkillDirectoryNamesInRoot(userKnorviaSkillRoot);
  const knorviaSkillNameKeys = new Set(
    rawKnorviaCandidates.map((candidate) => normalizeSkillNameKey(candidate.name)),
  );
  const knorviaCandidates = await dedupeCandidatesByCanonicalSkillPath(
    rawKnorviaCandidates,
    seenSkillRealpaths,
  );
  const agentsCandidates = await dedupeCandidatesByCanonicalSkillPath(
    (await collectUserSkillCandidatesInRoot(getUserAgentsSkillRoot())).filter(
      (candidate) =>
        !knorviaDirectoryNames.has(candidate.directoryName) &&
        !knorviaSkillNameKeys.has(normalizeSkillNameKey(candidate.name)),
    ),
    seenSkillRealpaths,
  );

  return [...knorviaCandidates, ...agentsCandidates].sort((left, right) =>
...
async function collectUserSkillDirectoryPathByName(): Promise<Map<string, string>> {
  const result = await collectSkillDirectoryPathByName(getUserKnorviaSkillRoot());
...
async function importArchive(
  ...
  const targetRoot = getUserKnorviaSkillRoot();
  ...
  try {
    ...
    // 远端 SkillsService 会同时读取用户级 .knorvia-studio/skills 和 .agents/skills。
    // 同名 skill 已在兼容目录存在时也必须跳过，避免同步后在 .knorvia-studio 下生成重复来源。
    const existingSkillPathByName = await collectUserSkillDirectoryPathByName();
