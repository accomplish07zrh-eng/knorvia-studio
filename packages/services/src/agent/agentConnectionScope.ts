...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@knorvia/rpc";
import {
...
export type KnorviaAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface KnorviaAgentV4ConnectionContext {
  connectionId: string;
  clientMode: KnorviaAgentV4ClientMode;
  role?: "terminal-client" | "trusted-host-relay";
}

const TRUSTED_CONNECTION_FIELD = "__knorviaTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__knorviaTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: KnorviaAgentV4ConnectionContext;
};

interface TrustedKnorviaAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedKnorviaAgentV4UnsubscribeRoute;
};

export function readTrustedKnorviaAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedKnorviaAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedKnorviaAgentV4Connection(
  value: unknown,
): KnorviaAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: KnorviaAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedKnorviaAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: KnorviaAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: KnorviaAgentWorkspaceTarget): KnorviaAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: KnorviaAgentWorkspaceTarget;
  connection: KnorviaAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: KnorviaAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: KnorviaAgentV4ConnectionContext): HelloMessage {
...
export interface KnorviaAgentConnectionScope {
  readonly service: IKnorviaAgentService;
...
export function createKnorviaAgentConnectionScope(
  base: IKnorviaAgentService,
  context: KnorviaAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): KnorviaAgentV4ConnectionContext => {
    const downstream = readTrustedKnorviaAgentV4Connection(params);
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies KnorviaAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: KnorviaAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedKnorviaAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedKnorviaAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: KnorviaAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: KnorviaAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: KnorviaAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IKnorviaAgentService> = {
