...
class KnorviaV4CommandRejectedError extends Error {
  readonly code = "KNORVIA_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "KnorviaV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new KnorviaV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new KnorviaV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
