...
import { repairSubagentTaskIndex } from "#src/agent/repairSubagentTaskIndex.js";
import {
...
export interface KnorviaTaskIndexSyncer {
   ...
   * desktop-continuous 路径上，sessionService 在 createSession/resumeSession/setModel
...
): KnorviaWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): KnorviaTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
export function createKnorviaTaskIndexSyncer(
  options: CreateKnorviaTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<KnorviaWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<KnorviaTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<KnorviaTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === KNORVIA_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<KnorviaWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<KnorviaWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: KnorviaAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: KnorviaAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 sessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: KnorviaAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 sessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 sessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 sessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 KnorviaConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useConfig 等）不改。
    getWorkspaceEmitter({
...
function hasUserVisibleContent(snapshot: KnorviaSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isKnorviaGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isKnorviaModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: KnorviaSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getKnorviaUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<KnorviaMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromKnorviaGoal(
  goal: NonNullable<KnorviaSessionStateSnapshot["projection"]["target"]>,
): KnorviaTaskGoal {
  return {
