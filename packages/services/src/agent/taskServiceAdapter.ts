/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 Knorvia session 的协议适配。 */
...
import {
  ...
  appendKnorviaStreamingToolInputDelta,
  attachKnorviaBackgroundTaskNotificationToRaw,
  buildKnorviaStreamingToolInputPreview,
  coalesceConsecutiveKnorviaAssistants,
  collectKnorviaBackgroundTaskNotificationsByToolUseId,
  createSessionTraceId,
  createKnorviaToolProjectionMemory,
  decodeCustomModelValue,
  deriveKnorviaTaskStatusFromSessionSnapshot,
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  finalizeKnorviaToolProjectionInput,
  forgetKnorviaToolProjectionMetadata,
  generateTraceId,
  getKnorviaGoalActiveIterationCount,
  getKnorviaGoalIterationByAssistantMessageId,
  getKnorviaUserVisibleMessages,
  isMainAgentToolProjectionSource,
  isKnorviaModelRetryRecoveryProgressPayload,
  markKnorviaStreamingToolInputPreviewMaterialized,
  mergeKnorviaBackgroundTaskControlItems,
  normalizeKnorviaApiRetryStatus,
  parseModelPickerValue as parseSharedModelSelection,
  parseKnorviaBackgroundTaskControlItems,
  parseKnorviaBackgroundTaskNotificationText,
  resolveWorkspaceKey,
  resolveKnorviaToolProjectionMetadata,
  resolveKnorviaVisibleSessionTitle,
  shouldMaterializeKnorviaStreamingToolInputPreview,
  textFromKnorviaMessageParts,
  knorviaApiRetryFromModelNetworkStatusPayload,
  knorviaApiRetryFromStreamRecoveryPayload,
  knorviaBackgroundTaskNotificationToolUpdateStatus,
  knorviaContextUsageBreakdownSchema,
  sessionSettingsStateSchema,
  knorviaTaskNetworkDebugStatusFromPayload,
  type InputId,
  type ModelSelection,
  type TraceId,
  type KnorviaAgentMcpServer,
  type KnorviaApiRetryStatus,
  type KnorviaAssistantMessageFeedback,
  type KnorviaBackgroundTaskControlItem,
  type KnorviaBackgroundTaskNotificationInfo,
  type KnorviaBackgroundTurnAttribution,
  type KnorviaCancelTaskCommandResult,
  type KnorviaConfigOption,
  type KnorviaContextCompactionTimelineMeta,
  type KnorviaDeliveryKind,
  type KnorviaEnqueueTaskCommandResult,
  type KnorviaError,
  type KnorviaGoalVerificationTimelineMeta,
  type KnorviaImportSessionsResult,
  type KnorviaImportableSessionCandidate,
  type KnorviaMessagePart,
  type KnorviaMessageWithParts,
  type KnorviaPermissionOption,
  type KnorviaPermissionRequest,
  type KnorviaPermissionRequestParams,
  type KnorviaPersistedMessage,
  type KnorviaPersistedMessagePart,
  type KnorviaPersistedToolCall,
  type KnorviaPlanStep,
  type KnorviaPromptAttachment,
  type KnorviaSessionEvent,
  type KnorviaSessionFile,
  type KnorviaSessionMode,
  type KnorviaSessionSettingsState,
  type KnorviaSessionStateSnapshot,
  type KnorviaSlashCommand,
  type KnorviaStateUpdatedNotification,
  type KnorviaStreamEvent,
  type KnorviaTaskClientMode,
  type KnorviaTaskCreateResult,
  type KnorviaTaskGoal,
  type KnorviaTaskGoalStats,
  type KnorviaTaskMeta,
  type KnorviaTaskMode,
  type KnorviaTaskRuntimeCommand,
  type KnorviaTaskSnapshot,
  type KnorviaTaskSnapshotBody,
  type KnorviaTaskSnapshotRefContent,
  type KnorviaTaskSnapshotToolCallsSlice,
  type KnorviaTaskTokenUsageResult,
  type KnorviaTimelineMeta,
  type KnorviaTimelineStatus,
  type KnorviaTimelineTrigger,
  type KnorviaTodoGroup,
  type KnorviaToolProjectionMemory,
  type KnorviaTurnSteerCommandKind,
  type KnorviaTurnSteerSource,
  type KnorviaUsage,
  type KnorviaUserInputRequestParams,
  type KnorviaUserInputResponse,
  type KnorviaWorkspaceEvent,
  type KnorviaWorkspaceTaskListChanged,
} from "@knorvia/shared";
import { errorAttributionSchema, type CommandPayloadMap } from "@knorvia/shared/protocol-v4";
...
import type {
  ...
  KnorviaTaskTerminalOutcome,
  KnorviaWorkspaceEventSubscriptionParams,
...
import {
  ...
  getKnorviaAgentAvailableModes,
  normalizeAvailableKnorviaMode,
  settingsToConfigOptions,
} from "./configOptions.js";
import type {
  KnorviaTaskIndexReadyEvent,
  KnorviaTaskIndexSyncer,
  KnorviaTaskIndexTerminalEvent,
} from "./taskIndexSyncer.js";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./v4HostCommand.js";
...
interface CreateKnorviaTaskServiceAdapterOptions {
...
type KnorviaSendPromptRuntimeCommand = Extract<KnorviaTaskRuntimeCommand, { type: "send_prompt" }>;
type KnorviaTerminalStreamEvent =
  | Extract<KnorviaStreamEvent, { type: "task_complete" }>
  | Extract<KnorviaStreamEvent, { type: "task_error" }>;

const EMPTY_SLASH_COMMANDS: KnorviaSlashCommand[] = [];
const logger = createServiceLogger("knorvia-task-service");
...
function formatKnorviaAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveKnorviaAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.KNORVIA_LOG_DIR?.trim();
  const logDir =
    (!process.env.KNORVIA_PORTABLE_DIR?.trim() && configuredLogDir) ||
    join(getKnorviaDataRootDir(), "cli", "log");
  return join(logDir, `knorvia-${formatKnorviaAgentLogDate(now)}.jsonl`);
}

export function createKnorviaTaskServiceAdapter(
  options: CreateKnorviaTaskServiceAdapterOptions,
): IKnorviaTaskService {
  const errorEmitter = new Emitter<KnorviaError>();
  const taskEmitters = new Map<string, Emitter<KnorviaStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<KnorviaStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, KnorviaTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, KnorviaApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, KnorviaBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, KnorviaToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(
        `Knorvia Studio task service adapter does not support IKnorviaTaskService.${name} yet.`,
      ),
      {
        code: "KNORVIA_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: KnorviaBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: KnorviaAgentMcpServer[] | undefined,
  ): Promise<KnorviaAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    // Knorvia task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 Knorvia session 事件回投 Knorvia Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "Knorvia Studio task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.agentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.agentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "Knorvia Studio task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "Knorvia Studio task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: KnorviaTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: KnorviaSendPromptRuntimeCommand,
  ): KnorviaSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: KnorviaSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: KnorviaSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "Knorvia Studio task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: KnorviaTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is KnorviaSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "Knorvia Studio task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "Knorvia Studio task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "Knorvia Studio task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<KnorviaStreamEvent, { type: "tool_call_update" }>["status"],
  ): KnorviaPersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: KnorviaStreamEvent): void {
    ...
    const nextTool: KnorviaPersistedToolCall =
  ...
  function persistedToolId(tool: KnorviaPersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: KnorviaPersistedToolCall | undefined,
    liveTool: KnorviaPersistedToolCall,
  ...
  function latestAssistantMessageIndex(messages: readonly KnorviaPersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly KnorviaPersistedMessagePart[] | undefined,
    toolIndex: number,
  ): KnorviaPersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: KnorviaPersistedMessage,
    toolIndex: number,
    tool: KnorviaPersistedToolCall,
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    ...
    if (changed) {
      logger.debug(undefined, "Knorvia Studio snapshot 合并 live tool projection", {
        event: "knorvia_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`Knorvia Studio session target is not loaded: ${taskId}`), {
        code: "KNORVIA_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): KnorviaToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createKnorviaToolProjectionMemory();
  ...
  function getTaskEmitter(params: TaskTarget): Emitter<KnorviaStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<KnorviaStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<KnorviaStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: KnorviaStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: KnorviaTaskMeta | undefined,
  ...
  async function resumeSnapshot(
    ...
    return options.agentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.agentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 KnorviaSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.agentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.agentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: KnorviaSessionStateSnapshot): KnorviaTaskMeta {
    const target = {
    ...
    return applyOverlayToMeta(
      {
        ...
        mode: fromKnorviaMode(snapshot.session.mode),
        ...
        target: snapshot.projection.target
          ? fromKnorviaGoal(snapshot.projection.target)
  ...
  function rememberIndexedTaskMeta(meta: KnorviaTaskMeta): KnorviaTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 Knorvia Studio session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: KnorviaTaskMeta): Promise<KnorviaTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 Knorvia session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: KnorviaStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: KnorviaStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToKnorvia(
    ...
    const goalIterationByAssistantMessageId = getKnorviaGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectKnorviaBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveKnorviaAssistants(
              getKnorviaUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: KnorviaPermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<KnorviaStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseKnorviaBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: KnorviaAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 Knorvia Studio snapshot 到 task index 失败", error);
      });
      const snapshotEvent: KnorviaStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: KnorviaTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: KnorviaTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeKnorviaAgentServiceAndWait(): Promise<void> {
    const agentService = options.agentService as IKnorviaAgentService & {
  ...
  const service: IKnorviaTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.agentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.agentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<KnorviaTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: KnorviaSessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.agentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.agentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.agentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        // 迁移属 v4 生命周期收口。
        snapshot = await options.agentService.createSession({
          ...target,
          sessionTraceId: createSessionTraceId(),
          mode: toKnorviaMode(params.mode),
          model: requestedSelection,
          thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<KnorviaEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: KnorviaTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<KnorviaEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<KnorviaCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "Knorvia Studio task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "Knorvia Studio task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "Knorvia Studio task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "Knorvia Studio task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "Knorvia Studio task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.agentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[knorvia-task-service] goalSession start", {
      ...
      const result = await options.agentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[knorvia-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[knorvia-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.agentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<KnorviaTaskMeta> {
    ...
    async listTasks(params): Promise<KnorviaTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<KnorviaTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: KNORVIA_AGENT_PROVIDER,
    ...
    async listTaskList(params: KnorviaTaskListQuery): Promise<KnorviaTaskListResult> {
    ...
    async applyGroupedTaskViewOrder(params) {
      const result = await taskIndexRepo.applyGroupedTaskViewOrder({
        ...params,
        // grouped 保存排序后的回包也必须继承列表查询的 Knorvia provider 边界，
    ...
    async listArchivedTasks(params): Promise<KnorviaTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: KNORVIA_AGENT_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<KnorviaTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<KnorviaTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: KNORVIA_AGENT_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<KnorviaTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const knorviaSnapshot = limitTaskSnapshotMessages(
        snapshotToKnorvia(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(knorviaSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[knorvia-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(knorviaSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...knorviaSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[knorvia-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[knorvia-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[knorvia-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<KnorviaTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<KnorviaTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<KnorviaTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<KnorviaTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<KnorviaConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.agentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<KnorviaSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as KnorviaAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
      ...
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.agentService.createSession({
          ...
          // 导入后的任务必须是真实 Knorvia session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toKnorviaMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<KnorviaConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.agentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toKnorviaMode(params.value as KnorviaTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<KnorviaConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.agentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<KnorviaConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveKnorviaAgentCurrentLogFilePath();
      // 返回 Knorvia Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: KNORVIA_AGENT_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // Knorvia Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[KnorviaTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<KnorviaTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 Knorvia Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.agentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.knorvia-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<KnorviaArchivedTaskDeletionResult> {
      const result: KnorviaArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<KnorviaTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[KnorviaTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[KnorviaTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[KnorviaTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[KnorviaTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[KnorviaTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<KnorviaTaskMeta> {
    ...
    async setTaskUnread(params): Promise<KnorviaTaskMeta> {
    ...
    async archiveTask(params): Promise<KnorviaTaskMeta> {
    ...
    async unarchiveTask(params): Promise<KnorviaTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<KnorviaTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<KnorviaStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<KnorviaTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<KnorviaTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<KnorviaStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 KnorviaStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.agentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toKnorviaDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<KnorviaWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.agentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeKnorviaAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: KnorviaTaskMeta, overlay: TaskOverlay): KnorviaTaskMeta {
  return {
    ...meta,
...
function deriveTitleFromSnapshot(snapshot: KnorviaSessionStateSnapshot): string {
  return resolveKnorviaVisibleSessionTitle({
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<KnorviaTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: KnorviaTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 cli。
    // 旧解析会先按冒号截断成 custom，最终下发 knorvia/custom，触发 Unsupported model。
    return {
...
function toKnorviaMode(mode: KnorviaTaskMode | undefined): KnorviaSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function fromKnorviaMode(mode: KnorviaSessionMode): KnorviaTaskMode {
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `knorvia-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<KnorviaPersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<KnorviaGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `knorvia-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `knorvia-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: KnorviaPersistedMessage[],
  timelineMessages: KnorviaPersistedMessage[],
): KnorviaPersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: KnorviaSessionStateSnapshot): number {
  ...
  return getKnorviaGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toKnorviaDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<KnorviaStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseKnorviaBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = knorviaBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachKnorviaBackgroundTaskNotificationToRaw(
...
function mapMessage(
  ...
  backgroundTaskNotifications?: ReadonlyMap<string, KnorviaBackgroundTaskNotificationInfo>,
): KnorviaPersistedMessage {
  const tools: KnorviaPersistedToolCall[] = [];
  const parts: KnorviaPersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: KnorviaTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly KnorviaMessagePart[],
): KnorviaPromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<KnorviaMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is KnorviaPromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<KnorviaMessagePart, { type: "file" }>,
): KnorviaPromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<KnorviaMessagePart, { type: "text" }>,
): KnorviaTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<KnorviaMessagePart, { type: "compaction" }>,
): KnorviaTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<KnorviaMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, KnorviaBackgroundTaskNotificationInfo>,
): KnorviaPersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // Knorvia Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachKnorviaBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? knorviaBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly KnorviaMessagePart[]): string {
  return textFromKnorviaMessageParts(parts);
}
...
function fromKnorviaGoal(goal: unknown): KnorviaTaskGoal {
  ...
  return {
    ...
    status: isKnorviaTaskGoalStatus(status) ? status : "active",
...
function isKnorviaTaskGoalStatus(status: string | undefined): status is KnorviaTaskGoal["status"] {
  return (
    status === "active" ||
    status === "paused" ||
...
function sessionTodosToPlanSteps(
  todos: KnorviaSessionStateSnapshot["todos"],
...
function sessionGoalStatsToRuntime(
  stats: KnorviaSessionStateSnapshot["goalStats"],
): KnorviaTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: KnorviaSessionStateSnapshot["todoGroups"],
): KnorviaTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = sessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableKnorviaMode(parsedSettings.data.mode.current),
      availableModes: getKnorviaAgentAvailableModes(),
    },
    {
      type: "knorvia_agent_model_state_update",
      taskId: params.taskId,
      traceId,
      version: 1,
      sessionId: params.taskId,
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: KnorviaToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, KnorviaBackgroundTaskControlItem[]>,
  ...
  // Knorvia Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 Knorvia Studio prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: KnorviaStreamEvent = {
  ...
  if (event.type === "turn.completed") {
    const events: KnorviaStreamEvent[] = [];
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "Knorvia Studio compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "Knorvia Studio session failed",
...
): Extract<KnorviaStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isKnorviaModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendKnorviaStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeKnorviaStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildKnorviaStreamingToolInputPreview(state.rawInput);
    markKnorviaStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildKnorviaStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeKnorviaToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "Knorvia Studio streaming tool input projected", {
    ...details,
    event: "knorvia.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<KnorviaStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<KnorviaMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): KnorviaContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<KnorviaStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveKnorviaToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeKnorviaToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: KnorviaStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // Knorvia Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 Knorvia Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetKnorviaToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // Knorvia Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: KnorviaPermissionRequestParams,
): KnorviaPermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: KnorviaSessionStateSnapshot["projection"]["pendingPermissions"][number],
): KnorviaPermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: KnorviaUserInputRequestParams,
): KnorviaStreamEvent {
  const questions =
...
): Extract<KnorviaStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: KnorviaSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<KnorviaStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: KnorviaSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<KnorviaStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, KnorviaBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: KnorviaBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): KnorviaBackgroundTaskControlItem[] | null {
  const parsedJobs = parseKnorviaBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeKnorviaBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): KnorviaPermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as KnorviaPermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<KnorviaStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = knorviaTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromKnorviaGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromKnorviaGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseKnorviaBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<KnorviaStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function recordAgentModelNetworkTelemetry(event: KnorviaSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `knorvia_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<KnorviaStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: KnorviaSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: KnorviaSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // Knorvia Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function usageFromPayload(value: unknown): KnorviaUsage | undefined {
...
function timelineStatusValue(value: unknown): KnorviaTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): KnorviaTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): KnorviaTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): KnorviaTurnSteerCommandKind | undefined {
