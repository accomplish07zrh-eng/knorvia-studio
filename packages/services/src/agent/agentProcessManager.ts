import { resolveKnorviaAgentSpawnCwd } from "#src/agent/agentSpawnCwd.js";
import type { KnorviaAgentStorageStartupSnapshot } from "#src/agent/agent.js";
/* eslint-disable max-lines -- agentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseKnorviaProcessDiagnostic,
  KNORVIA_AGENT_LIFECYCLE_LOG_MARKER,
  KNORVIA_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  KNORVIA_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  KNORVIA_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@knorvia/shared/process-diagnostic";
import {
  KNORVIA_AGENT_RUNTIME,
  KNORVIA_AGENT_PROVIDER,
  KNORVIA_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveKnorviaRuntimeEnv,
  sanitizeKnorviaRuntimeEnv,
} from "@knorvia/shared";
import {
  findKnorviaAgentRuntimeBinary,
  findKnorviaAgentRuntimeNodeBundle,
...
import { KnorviaProtocolClient } from "./protocolClient.js";
import { KnorviaStdioTransport } from "./stdioTransport.js";
import { readKnorviaStdioTapDevState } from "./stdioTapDevConfig.js";
import type { KnorviaAgentPresentationSurface } from "./agentPresentationSurface.js";
...
export interface KnorviaAgentCommand {
...
export interface KnorviaAgentCommandResolverContext {
  presentationSurface?: KnorviaAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type KnorviaAgentCommandResolver = (
  context: KnorviaAgentCommandResolverContext,
) => Promise<KnorviaAgentCommand | null> | KnorviaAgentCommand | null;

export interface KnorviaAgentProcessManagerOptions {
  commandResolver?: KnorviaAgentCommandResolver;
  presentationSurface?: KnorviaAgentPresentationSurface;
...
interface ManagedKnorviaAgentProcess {
  ...
  runtimeIdentity: KnorviaAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.KNORVIA_E2E_ARTIFACT_DIR?.trim();
  if (env.KNORVIA_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "knorvia-e2e-coverage-preload.cjs");
 ...
 */
interface KnorviaAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: KnorviaAgentRuntimeIdentity;
}

export interface KnorviaAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: KnorviaAgentRuntimeIdentity;
...
interface KnorviaAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface KnorviaAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("KNORVIA_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildKnorviaAgentSpawnPreflight(
  command: KnorviaAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<KnorviaAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveKnorviaAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceKnorviaAgentCommand(
  context: KnorviaAgentCommandResolverContext,
): KnorviaAgentCommand | null {
  const distEntrypoint = findUpward("apps/cli/packages/cli/dist/knorvia.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.KNORVIA_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "knorvia.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/cli/packages/cli/src/main.ts");
...
function resolveDeployedKnorviaAgentBinaryCommand(
  context: KnorviaAgentCommandResolverContext,
): KnorviaAgentCommand | null {
  // 旧 resolver 只识别 KNORVIA_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 knorvia-server.cjs 单文件部署到 ~/.knorvia-studio/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 agent 已经部署到 ~/.knorvia-studio/server/agents/knorvia/，
  // resolver 也找不到，第一次 getClient 就抛 "Knorvia agent server command is not configured"。
  // 这里复用 findKnorviaAgentRuntimeBinary 的候选链（含 KNORVIA_AGENT_BINARY env、
  // packagedResourcesPath、~/.knorvia-studio/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findKnorviaAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: KNORVIA_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeKnorviaAgentCommand(
  context: KnorviaAgentCommandResolverContext,
): KnorviaAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/knorvia 的 knorvia.cjs，
  ...
  const bundlePath = findKnorviaAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...KNORVIA_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultKnorviaAgentCommand(
  context: KnorviaAgentCommandResolverContext,
): KnorviaAgentCommand | null {
  const command = process.env.KNORVIA_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.KNORVIA_AGENT_SERVER_ARGS_JSON) ?? [
          "app-server",
          "--stdio",
        ],
        cwd: process.env.KNORVIA_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 knorvia.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceKnorviaAgentCommand(context) ??
    resolveElectronRuntimeKnorviaAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedKnorviaAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: KnorviaAgentCommand | null,
  presentationSurface: KnorviaAgentCommandResolverContext["presentationSurface"],
): KnorviaAgentCommand | null {
...
function wrapKnorviaAgentCommandWithStdioTapDevProxy(
  command: KnorviaAgentCommand,
  workspaceKey: string,
): KnorviaAgentCommand {
  const tapState = readKnorviaStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/stdio-tap.mjs");
  if (!tapScript) {
    debugLog("Knorvia Studio stdio tap proxy enabled but script not found");
    return command;
  }
...
export class KnorviaAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedKnorviaAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedKnorviaAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<KnorviaProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: KnorviaAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): KnorviaAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: KnorviaAgentCommandResolver;
  private readonly presentationSurface: KnorviaAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: KnorviaAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: KnorviaAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<KnorviaAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<KnorviaAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 KnorviaAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: KnorviaAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultKnorviaAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("Knorvia Studio agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedKnorviaAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedKnorviaAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("Knorvia Studio agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private reportRuntimeUnavailable(managed: ManagedKnorviaAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedKnorviaAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("Knorvia Studio agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("Knorvia Studio agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`Knorvia Studio agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<KnorviaProtocolClient> {
    if (this.disposed) {
      throw new Error("Knorvia Studio agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("Knorvia Studio agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("Knorvia Studio agent process start wait completed", {
        workspaceKey,
  ...
  getExistingClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): KnorviaProtocolClient | undefined {
    const managed = this.processesByWorkspaceKey.get(resolveWorkspaceKey(params));
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "Knorvia Studio agent server command is not configured. Set KNORVIA_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw (
        admissionSignal.reason ?? new Error("Knorvia Studio agent process start was cancelled.")
      );
    }
    const effectiveCommand = wrapKnorviaAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("Knorvia Studio agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("Knorvia Studio agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildKnorviaAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 KNORVIA_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 Knorvia 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveKnorviaRuntimeEnv(process.env);
    log("Knorvia Studio agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeKnorviaRuntimeEnv(process.env),
        [KNORVIA_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new KnorviaStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseKnorviaProcessDiagnostic(line);
    ...
    const client = new KnorviaProtocolClient(transport, {
    ...
    const runtimeIdentity: KnorviaAgentRuntimeIdentity = {
    ...
    const managed: ManagedKnorviaAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("Knorvia Studio agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `Knorvia Studio agent process error${this.processLifecycleReporter?.onError ? ` ${KNORVIA_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("Knorvia Studio agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `Knorvia Studio agent process exited unexpectedly${this.processLifecycleReporter ? ` ${KNORVIA_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "Knorvia Studio agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("Knorvia Studio agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("Knorvia Studio agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<KnorviaAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("Knorvia Studio agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "KNORVIA_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("Knorvia Studio agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("Knorvia Studio agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，cli 会残留为孤儿进程。
