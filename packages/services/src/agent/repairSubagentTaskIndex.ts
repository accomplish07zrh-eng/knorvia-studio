import { resolveWorkspaceKey, KNORVIA_AGENT_PROVIDER } from "@knorvia/shared";
import type { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { IKnorviaAgentService, KnorviaAgentWorkspaceTarget } from "#src/agent/agent.js";
...
export async function repairSubagentTaskIndex(params: {
  ...
  agentService: Pick<IKnorviaAgentService, "listSessions">;
  taskIndexRepo: Pick<TaskIndexRepo, "listTaskMetas" | "updateTaskState">;
  isCurrent: () => boolean;
  onRemoved: () => void;
}): Promise<void> {
  const { target, taskIndexRepo, agentService, isCurrent } = params;
  const rows = await taskIndexRepo.listTaskMetas({ ...target, provider: KNORVIA_AGENT_PROVIDER });
  const ids = rows
