...
import { IStudioRuntimeService } from "./studio-runtime/contract.js";
import { createStudioRuntimeService } from "./studio-runtime/node.js";
...
import { createCreationService } from "./creation/creationService.js";
import { createCreationAgentBridge } from "./creation/creationAgentBridge.js";
...
export type { KnorviaStdioTapDevState } from "@knorvia/shared";
...
export {
  resolveDefaultKnorviaAgentCommand,
  KnorviaAgentProcessManager,
} from "./agent/agentProcessManager.js";
export type {
  KnorviaAgentCommand,
  KnorviaAgentCommandResolver,
  KnorviaAgentCommandResolverContext,
  KnorviaAgentProcessManagerOptions,
} from "./agent/agentProcessManager.js";
export { createKnorviaAgentService } from "./agent/agentService.js";
export { KnorviaProtocolClient } from "./agent/protocolClient.js";
export type { KnorviaProtocolTransport } from "./agent/protocolTransport.js";
export {
  getKnorviaStdioTapDevLogDir,
  readKnorviaStdioTapDevState,
  setKnorviaStdioTapDevEnabled,
} from "./agent/stdioTapDevConfig.js";
export { KnorviaStdioTransport } from "./agent/stdioTransport.js";
export { createKnorviaTaskServiceAdapter } from "./agent/taskServiceAdapter.js";
export { createKnorviaSessionService } from "./agent-session/sessionService.js";
...
import {
  ...
  isPotentialKnorviaCuaAgentMcpServer,
...
import {
  ...
  isKnorviaCuaMcpCommand,
  isKnorviaCuaMcpPackageArg,
...
import { buildHelperOpenArgs, isCuaLocalDevelopmentRuntime } from "@knorvia/cua/broker/server";
import { resolveBrokerSocketPath } from "@knorvia/cua/broker/socketPath";
...
import { IKnorviaTaskService } from "./session/taskService.js";
...
import type { CuaOperationStateReporter } from "./agent/cuaOperationTurnTracker.js";
import { IKnorviaAgentService } from "./agent/agent.js";
import { resolveKnorviaAgentPresentationSurface } from "./agent/agentPresentationSurface.js";
import type { KnorviaAgentCommandResolver } from "./agent/agentProcessManager.js";
import { createKnorviaAgentService } from "./agent/agentService.js";
import { createKnorviaTaskIndexSyncer } from "./agent/taskIndexSyncer.js";
import { createKnorviaTaskServiceAdapter } from "./agent/taskServiceAdapter.js";
import { IKnorviaSessionService } from "./agent-session/session.js";
import { createKnorviaSessionService } from "./agent-session/sessionService.js";
...
const managedCreationBridges = new WeakMap<ServiceCollection, CreationAgentBridge>();
...
export function shouldEnableDefaultCuaProductHelper(
  ...
  // CUA 已随正式版默认开启（isKnorviaCuaInternalFeatureEnabled 默认 ON，仅显式 0/false/off 关闭；2026-08 注释更正——旧注释称默认关闭已过期）。显式开启后 macOS 使用既有产品 Helper，Windows 使用安装包内 runtime；
  // 两端都保持按需启动。关闭时不创建 host、不探测资源、不产生子进程或权限提示。
  const env = options.env ?? process.env;
  if (!isKnorviaCuaInternalFeatureEnabled(env)) return false;
  ...
  return {
    async resolveMcpServers(servers, context) {
      ...
      return options.isResolverCurrent?.(resolver, context) === false
        ? servers?.filter((server) => !isPotentialKnorviaCuaAgentMcpServer(server))
        : resolved;
    },
    async restart() {
      ...
      if (!resolver) {
        throw new Error(
          "Knorvia Studio Computer Use is not enabled (plugin off or not product mode).",
        );
      }
...
export const KNORVIA_CUA_BUNDLED_HELPER_APP_PATH_ENV = "KNORVIA_CUA_BUNDLED_HELPER_APP_PATH";

export function resolveBundledCuaHelperAppPath(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const injectedPath = env[KNORVIA_CUA_BUNDLED_HELPER_APP_PATH_ENV]?.trim();
  if (injectedPath) {
    return injectedPath;
  }
  const resourcesPath = (
...
export function hasGlobalCliKnorviaCuaServer(env: NodeJS.ProcessEnv = process.env): boolean {
  const configPath = join(resolveKnorviaDataRoot(env), "cli", "config.json");
  let parsed: unknown;
  try {
  ...
  return Object.entries(parsed.mcp.servers).some(([name, config]) =>
    isGlobalCliKnorviaCuaServer(name, config),
  );
}

function isGlobalCliKnorviaCuaServer(name: string, config: unknown): boolean {
  if (!isRecord(config)) return false;
  if (config.enabled === false) return false;
  if (typeof config.type === "string" && config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 与 desktop/services resolver 和 CLI bootstrap 共用 @knorvia/shared 的单一事实源，避免第三处
  // 判定漂移：git/.git/本地路径形态的 cua 若这里漏判，全局 CLI env 注入不会带 broker
  // socket/token，agent 会回退成 Python/uvx 自己持有 macOS TCC（违反 product broker 边界）。
  if (typeof config.command === "string" && isKnorviaCuaMcpCommand(config.command)) {
    return true;
  }
  return (
    Array.isArray(config.args) &&
    config.args.some((arg) => typeof arg === "string" && isKnorviaCuaMcpPackageArg(arg))
  );
}
...
export async function buildCuaProductHelperAgentEnv(
  ...
  //
  // fail-closed 理由：CLI 全局配置里存在 cua 时，返回空 env 会让 agent 按原始
  ...
  try {
    ...
    if (host.waitForTransport) {
      ...
      return {
        ...
        [KNORVIA_CUA_PLUGIN_AUTHORITY_ENV_KEY]: transport.pluginAuthority,
      };
    }
    ...
    if (reserved && !hasCuaProductHelperAgentEnvUnavailable(host)) {
      ...
      return {
        ...
        [KNORVIA_CUA_PLUGIN_AUTHORITY_ENV_KEY]: reserved.pluginAuthority,
      };
    }
    ...
    return {
      ...
      [KNORVIA_CUA_PLUGIN_AUTHORITY_ENV_KEY]: handle.pluginAuthority,
    };
  } catch (error) {
    ...
    logger.warn(
      undefined,
      `Computer Use Helper broker_unavailable; disabling workspace cua MCP server for this agent spawn (${cuaHelperStartErrorDetail(error)})`,
    );
    return {
...
export function createLocalServices(options: {
  ...
  /** Desktop Local Host only: live, already authenticated SSH Studio environments. */
  remoteStudioEnvironments?: () => RemoteStudioEnvironment[];
  /** desktop-attached remote server 从 Desktop Host 收到的一次性 Agent 网络配置。 */
  remoteAgentNetwork?: {
    httpProxy?: string;
    noProxy?: string;
  };
  /** 所属 Environment 的 Knorvia Built-in Provider Config 物理路径。 */
  ...
  // 导致 bun 这类只在 shell profile 里追加的命令在 Knorvia Agent/终端里不可见。
  // 这里在所有本地服务启动前统一修正运行时环境，并顺带把内置 rg 注入 PATH，
  // 让 Knorvia Agent、终端、认证 runtime 共用同一套命令解析结果。
  initializeRuntimeProcessEnv(options?.runtimeProcessEnvPatch);

  const desktopContextPromptEnabledRaw =
    process.env[KNORVIA_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]?.trim();
  ...
  // 产品账号已删除；通用凭据只保存用户显式配置的第三方授权。
  const credentialService = createCredentialService();
  const creationService = createCreationService({
    rootDir: join(getKnorviaDataRootDir(), "creation"),
    credentials: credentialService,
  });
  const creationMcpScript = [
    join(
      (process as NodeJS.Process & { resourcesPath?: string }).resourcesPath ?? "",
      "creation-mcp.cjs",
    ),
    join(process.cwd(), "packages", "desktop", "resources", "creation-mcp.cjs"),
    join(process.cwd(), "resources", "creation-mcp.cjs"),
  ].find((path) => path && existsSync(path));
  const creationBridge = creationMcpScript
    ? createCreationAgentBridge(creationService, {
        scriptPath: creationMcpScript,
        executablePath: process.execPath,
        runtimeEnv: process.versions.electron ? { ELECTRON_RUN_AS_NODE: "1" } : {},
      })
    : undefined;
  ...
  const providerConfigRuntime = createProviderConfigRuntime({
    knorviaBuiltinFilePath: options.knorviaBuiltinProviderConfigFilePath,
    onPersonalConfigRecovery: (event) => {
      providerConfigLog.warn(undefined, "Personal Provider Config 加载失败，已保留磁盘状态", {
        error: event.error,
      });
    },
  ...
  let providerConnectivityAgentService:
    | Pick<IKnorviaAgentService, "testModelConnectivity">
    | undefined;
  ...
  // mcpSync/hooks 里引用 agentService 的闭包是惰性调用，声明顺序不影响初始化。
  ...
  const mcpSyncService = createMcpSyncService({
    ...
    listMcpServerStatuses: (params) => agentService.listMcpServerStatuses(params),
  });
  ...
  const hooksService = createHooksService({
    grantWorkspaceHookTrust: (params) => agentService.grantWorkspaceHookTrust(params),
  });
  ...
  // createDefaultCuaProductHelper() 在 agentService 存在之前就要组装 resolver，
  // 但"是否有活跃 turn"这个信号只有 agentService 建好之后才能查询。用前向引用占位——resolver 真正
  ...
  const isCuaEnabledForContext = (context?: CuaProductMcpServerResolverContext): boolean =>
    // 保留 main 原有 gate 行为（避免回归）：dev/internal 特性开启时（KNORVIA_CUA_DEV_MODE=1 或
    // KNORVIA_CUA_PRODUCT_HELPER=1）即视为启用，不依赖 config.json 显式 enable——main 的 bootstrap
    // 用 isKnorviaCuaInternalFeatureEnabled 门控 bundled plugin，与 feat 的 workspace enablement 不同。
    // 生产路径（dev mode off）回落到官方插件 workspace enablement 判定（与 feat 一致）。
    isKnorviaCuaInternalFeatureEnabled(process.env) ||
    isOfficialCuaPluginEnabledForWorkspace({
      env: process.env,
  ...
  const createManagedDefaultCuaProductHelper = (
    ...
    const helper = createDefaultCuaProductHelper({
      // 转发活跃-turn 查询（前向引用，agentService 建好后赋值）。
  ...
  //
  // dev 判定直接用上游的 isCuaLocalDevelopmentRuntime（@knorvia/cua/broker/server，
  // 即本文件已经用来 import buildHelperOpenArgs 的那个 subpath，可正常导入）。
  //
  // 行为等价性（别误读成安全加固）：上游是 `COMPILED_LOCAL_DEVELOPMENT_RUNTIME &&
  // KNORVIA_RUNTIME_ENV!=="production"`，而那个编译期常量只有 scripts/build-cua-helper-app.mjs
  // 会用 define 折叠（Helper bundle）；desktop host bundle 没有该 define，于是回退成
  // `process.env.NODE_ENV !== "production"` —— 正是复制品写的那一项。所以在**当前**打包形态下
  // 两者逐字等价，关门靠的是 KNORVIA_RUNTIME_ENV=production（打包态显式注入且不传 NODE_ENV）。
  //
  // 换成上游的收益是消除漂移面：折叠点、因子个数与 fail-closed 方向都由上游一处决定，
  // 哪天 host bundle 也补上 __KNORVIA_LOCAL_DEVELOPMENT_RUNTIME__ define（Helper 侧已经有），
  // 编译期门自动生效，不需要再回来改这里。
  ...
  const launchStandaloneCuaHelperForStatus = async (): Promise<string | null> => {
    ...
    const baseRoot = join(getKnorviaDataRootDir(), "computer-use");
  ...
  const cuaPipSessionService = createCuaPipSessionService({
    ...
    resolveCredentials: async () => {
      ...
      // PiP 客户端以 role=presentation 声明，资格由 Helper 按对端（Knorvia 主进程）签名 identifier 裁决。
  ...
  const cuaPermissionService: ICuaPermissionService = {
    async getStatus(
      ...
      if (
        ...
        return {
          available: false,
          reason: "Knorvia Studio Computer Use is not enabled (plugin off or not product mode).",
        };
      }
      ...
      if (!host || !host.running) {
        ...
        if (!stable) {
          return {
            available: false,
            reason:
              "Knorvia Studio Computer Use is not running; it will start automatically on first Computer Use use.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
        // standalone Helper 上直接查权限真值（身份模式，无 token）。
        try {
          const { callBrokerMethod } = await import("@knorvia/cua/broker/helperHealth");
          ...
          return {
            available: false,
            reason: "Knorvia Studio Computer Use is starting up; retry in a moment.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
      }
      try {
        ...
        if (!helper || !isDefaultCuaProductHelperCurrent(helper)) {
          return {
            available: false,
            reason: "Knorvia Studio Computer Use lifecycle is disposed.",
          };
        }
    ...
    async restartHelper(
      ...
      try {
        ...
        return {
          ok: false,
          reason: `Failed to restart Knorvia Studio Computer Use: ${
            error instanceof Error ? error.message : String(error)
          }`,
        };
      }
  ...
  const agentService = createKnorviaAgentService({
    ...(creationBridge
      ? { creationMcpServer: (workspacePath: string) => creationBridge.issueNative(workspacePath) }
      : {}),
    ...(modelSelectionReadinessSource ? { modelSelectionReadinessSource } : {}),
    authorizeLocalMediaPreviewPath: options?.authorizeLocalMediaPreviewPath,
    commandResolver: options?.agentCommandResolver,
    presentationSurface: resolveKnorviaAgentPresentationSurface({
    ...
    // KnorviaAgentProcessManager，导致 host 永远不向 main 上报 Agent spawn/exit，进程监控器
    // 因而看不到实际运行的 Agent，也无法验证只读到可写升级是否复用同一进程。
    processLifecycleReporter: options?.processLifecycleReporter,
    spawnFallbackCwd: options?.agentSpawnFallbackCwd,
    ...
    cuaOperationStateReporter: shouldEnableCuaOperationStateReporter({
      ...
      : undefined,
    // Knorvia 只发布 turn/session 事实；面板 terminal policy 由 producer coordinator 决定。
    ...
    resolveSpawnEnv: async (context) => {
      ...
      if (!helper && cuaPluginEnabled && process.platform === "darwin") {
        ...
        cuaProductHelperEnv = {
          ...
          [KNORVIA_CUA_PLUGIN_AUTHORITY_ENV_KEY]: randomBytes(16).toString("hex"),
        };
      ...
      return {
        ...
        // broker 凭据（socket/token）注入 agent spawn env，让内置 cua plugin 的
        // computer-use MCP server 经 __knorvia-plugin-host 恢复 token 后连上 broker。
        ...
        ...createNodeProviderRuntimePathEnv({
          ...
          knorviaBuiltinFilePath: await providerConfigRuntime.resolveKnorviaBuiltinActiveFilePath(),
  ...
  providerConnectivityAgentService = agentService;
  ...
  hasActiveTurnRef = () => agentService.hasActiveCuaOperationTurn();
  // desktop-continuous UI 直接订阅 sessionService，绕开 Knorvia task adapter 的
  ...
  const taskIndexSyncer = createKnorviaTaskIndexSyncer({
    agentService,
    taskIndexRepo,
  });
  ...
  const sessionService = createKnorviaSessionService({
    ...(creationBridge
      ? { creationMcpServer: (workspacePath: string) => creationBridge.issueNative(workspacePath) }
      : {}),
    agentService,
  ...
  const gitCommitMessageGenerator = new GitCommitMessageGenerator({
    ...
    textGenerator: {
      async generateText(params) {
        return await agentService.generateWorkspaceText({
  ...
  // task wrapper 由 Knorvia task service adapter 提供；核心 session 状态由 Knorvia agent server 维护。
  const taskService = createKnorviaTaskServiceAdapter({
    agentService,
    taskIndexRepo,
    taskIndexSyncer,
    settingService,
  ...
  const pluginManagementService = createPluginManagementService({ agentService });
  ...
  const services = new ServiceCollection()
    .register(ICreationService, creationService)
    .register(
      ...
      createStudioRuntimeService({
        ...
        creationService,
        creationBridge,
        remoteEnvironments: options?.remoteStudioEnvironments,
      }),
    )
    ...
    .register(IKnorviaTaskService, taskService)
    .register(IKnorviaAgentService, agentService)
    .register(IKnorviaSessionService, sessionService)
    ...
    .register(IUsageStatsService, createUsageStatsService({ agentService }))
    ...
    .register(IPluginManagementService, pluginManagementService)
  ...
  if (creationBridge) managedCreationBridges.set(services, creationBridge);
  return services;
}
...
export function disposeServiceResources(services: ServiceCollection): void {
  ...
  const disposableServices = [
    services.getOptional(IStudioRuntimeService),
    services.getOptional(ITerminalService),
    services.getOptional(IKnorviaTaskService),
    services.getOptional(IKnorviaAgentService),
    services.getOptional(IKnorviaSessionService),
  ...
  const creationBridge = managedCreationBridges.get(services);
  if (creationBridge) void creationBridge.disposeAllAndWait().catch(() => {});
  managedCreationBridges.delete(services);
...
export async function disposeServiceResourcesAndWait(services: ServiceCollection): Promise<void> {
  ...
  // 旧的同步 dispose 会在 host 退出时丢掉强杀 timer，导致 cli/app-server 变成孤儿进程。
  const disposableServices = [
    services.getOptional(IStudioRuntimeService),
    services.getOptional(ITerminalService),
    services.getOptional(IKnorviaTaskService),
  ...
  sharedSqliteRepos.delete(services);
  const creationBridge = managedCreationBridges.get(services);
  if (creationBridge) await creationBridge.disposeAllAndWait().catch(() => {});
  managedCreationBridges.delete(services);
  providerRuntimes.get(services)?.dispose();
