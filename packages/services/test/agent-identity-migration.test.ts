...
function previousDatabase() {
  ...
  insert.run("old", "glm", JSON.stringify({ provider: "glm", model: "GLM-5.3", extra: "preserve" }));
...
test("identity migration preserves task data, model names, other providers and historical checksums", () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(String(task.meta_json)), { provider: "knorvia", model: "GLM-5.3", extra: "preserve" });
    assert.deepEqual(db.prepare("SELECT * FROM tasks WHERE task_id='retired'").get(), retired);
    assert.equal(db.prepare("SELECT meta_json FROM tasks WHERE task_id='damaged'").get()?.meta_json, "invalid-json");
    ...
    assert.deepEqual(db.prepare("SELECT * FROM tasks_schema_migration WHERE id != '0004_agent_identity' ORDER BY id").all(), ledger);
    ...
    assert.deepEqual(db.prepare("SELECT * FROM tasks_schema_migration ORDER BY id").all(), completedLedger);
  } finally {
    db.close();
  }
});
...
test("a failed identity migration rolls back index updates and remains retryable", () => {
  ...
  try {
    ...
    assert.equal(db.prepare("SELECT provider FROM tasks WHERE task_id='old'").get()?.provider, "glm");
    assert.equal(db.prepare("SELECT 1 FROM tasks_schema_migration WHERE id='0004_agent_identity'").get(), undefined);
    db.exec("DROP TRIGGER reject_identity");
    runTasksDatabaseMigrations(db);
    assert.equal(db.prepare("SELECT provider FROM tasks WHERE task_id='old'").get()?.provider, "knorvia");
  } finally {
    db.close();
  }
});
...
test("saved task metadata is normalized while new provider requests use the current identity", () => {
  const meta = {
    taskId: "task", traceId: "trace", title: "Title", workspacePath: "/example",
    createdAt: 1, updatedAt: 2, mode: "build", provider: "glm",
  };
