...
import { createRemoteKernelAdapter, type RemoteStudioEnvironment } from "../src/studio-runtime/adapters/kernels/remoteKernelBridge.js";
...
function remoteFixture(identity: string, path: string, options: { loseSendAck?: boolean; waitForCancel?: boolean } = {}) {
  ...
  const service = {
    ...
    async timeline() {
      ...
      return {
        ...
        messages: state === "succeeded" ? [{ id: "reply", runId, sender: "codex", kind: "text", text: identity, createdAt: 1, updatedAt: 1 }] : [],
        interactions: [],
    ...
    async agentWorkspaceChanges() { return []; },
  } as unknown as RemoteStudioEnvironment["service"];
  return { environment: { workspaceIdentity: identity, workspacePath: path, label: identity, service }, commands };
}
...
test("protocol discovery only publishes installed, valid remote CLI identities", async () => {
  ...
  assert.deepEqual(statuses.map((item) => item.id), [
    remoteStudioKernelId("fixture-a", "codex"),
    remoteStudioKernelId("fixture-b", "codex"),
  ]);
...
test("remote single chat stays bound to its project and uncertain send needs explicit retry after reconnect", async () => {
  ...
  const kernels = {
    remoteWorkspace: (kernel: string) => kernel === remoteId ? environments[0]?.service : undefined,
    adapter: (kernel: string) => createRemoteKernelAdapter(kernel as typeof remoteId, () => environments),
  ...
  const service = new StudioRuntimeService({
    ...
    kernels,
    workspaces: {
      prepare: async () => { throw new Error("remote path entered local workspace manager"); },
      changes: async () => { throw new Error("remote path entered local change scanner"); },
      apply: async () => { throw new Error("remote path entered local apply"); },
    },
    onDidChange: Event.None,
    notify: () => {},
  });
  try {
    assert.equal((await service.inspectKernels()).find((status) => status.id === remoteId)?.installed, true);
    await service.command({ commandId: randomUUID(), type: "create-conversation", id: "remote-chat", kernel: remoteId, workspacePath: root });
    const sent = await service.command({ commandId: randomUUID(), type: "send", kind: "chat", targetId: "remote-chat", text: "fixture request" });
    await until(() => { service.tick(); return db.read<StoredRun>("run", sent.id)?.state === "interrupted"; });
    assert.equal(db.read<StoredRun>("run", sent.id)?.resultKnown, false);
    assert.equal(lost.commands.find((item) => item.type === "create-conversation")?.workspacePath, "/srv/project");
    assert.equal(lost.commands.filter((item) => item.type === "send").length, 1);
    await assert.rejects(service.command({ commandId: randomUUID(), type: "send", kind: "chat", targetId: "remote-chat", text: "follow-up" }), /不确定/);
    await assert.rejects(service.command({ commandId: randomUUID(), type: "resume", runId: sent.id, retryUncertain: false }), /不确定/);
    service.tick();
    assert.equal(lost.commands.filter((item) => item.type === "send").length, 1);
    ...
    assert.equal((await service.inspectKernels()).find((status) => status.id === remoteId)?.installed, true);
    await service.command({ commandId: randomUUID(), type: "resume", runId: sent.id, retryUncertain: true });
    await until(() => { service.tick(); return db.read<StoredRun>("run", sent.id)?.state === "succeeded"; });
    assert.equal(db.read<StoredRun>("run", sent.id)?.attempt, 2);
    assert.notEqual(lost.commands.find((item) => item.type === "send")?.commandId, recovered.commands.find((item) => item.type === "send")?.commandId);
    assert.equal(recovered.commands.find((item) => item.type === "create-conversation")?.workspacePath, "/srv/project");
  } finally {
...
test("remote stop forwards cancel and waits for a known terminal response", async () => {
  ...
  const resultPromise = createRemoteKernelAdapter(remoteId, () => [fixture.environment]).run(
    { runId: "stop", turnId: "turn", dispatchId: "stop:turn:1", conversationId: "chat", kernel: remoteId, workspacePath: "C:\\local", permission: "ask", text: "stop fixture" },
...
test("task group dispatches local and two same-CLI remote members to distinct project owners", async () => {
  ...
  const definition: StudioGroupDefinition = {
    id: "fixture-group", name: "Fixture", goal: "Review projects", members: [...members], host: "codex",
    sharedSummary: "Text context only", mode: "task", workspaceMode: "isolated", createdAt: 1, updatedAt: 1,
  };
  const plan = JSON.stringify({ tasks: members.map((member, index) => ({ id: `task-${index}`, member, instruction: `Inspect project ${index}`, dependsOn: [] })) });
  const h = harness(async (step) => {
    if (step.id.includes("group:plan")) return success(plan);
    if (step.id.includes("group:review")) return success('{"status":"complete","summary":"All fixture projects reviewed"}');
    ...
    const result = await createRemoteKernelAdapter(step.kernel, () => [fixture.environment]).run(
      { runId: "group", turnId: step.id, dispatchId: step.id, conversationId: `group:${step.kernel}`, kernel: step.kernel, workspacePath: "C:\\local-project", workspaceRunId: "group", workspaceStepId: step.id, workspaceMode: "isolated", permission: "ask", text: step.prompt },
      { emit: async () => {}, ask: async () => ({ decision: "deny" }) },
  ...
  for (const [fixture, path] of [[first, "/srv/a"], [second, "/srv/b"]] as const) {
