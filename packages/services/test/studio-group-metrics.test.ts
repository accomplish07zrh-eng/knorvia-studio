...
const run: StudioRun = {
  ...
  createdAt: 1000, updatedAt: 2000, checkpoint: { steps: {}, values: {}, completedRounds: 0 },
  attempt: 1, taskMode: true,
  definition: {
...
test("group statistics add observed turns by member and mark partial reporting", () => {
  const turns: StudioTurnSnapshot[] = [
    { id: "a", runId: run.id, stepId: "a", state: "succeeded", attempt: 1, memberId: "codex", startedAt: 1000, endedAt: 4000 },
    { id: "b", runId: run.id, stepId: "b", state: "running", attempt: 1, memberId: "claude-code", startedAt: 1500 },
  ];
  ...
  assert.deepEqual(metrics.members.map((item) => [item.member, item.tokens, item.durationMs]), [
    ["codex", 15, 3000], ["claude-code", 2, 2000],
  ]);
...
test("unknown old turns and interrupted execution do not fabricate time or usage", () => {
  const turns: StudioTurnSnapshot[] = [
    ...
    { id: "unknown", runId: run.id, stepId: "unknown", state: "running", attempt: 1, memberId: "codex", startedAt: 1000 },
  ];
  const metrics = projectStudioGroupMetrics(
    { ...run, state: "interrupted", resultKnown: false }, turns, () => undefined, 9000, true,
  )!;
...
test("invalid counters and clocks stay unknown instead of becoming zero", () => {
  const metrics = projectStudioGroupMetrics(run, [
    { id: "bad", runId: run.id, stepId: "bad", state: "succeeded", attempt: 1, memberId: "codex", startedAt: 5000, endedAt: 4000 },
  ], () => ({ inputTokens: -1, outputTokens: Number.NaN }), 6000, false)!;
