...
for (const clientMode of ["desktop-continuous", "web-remote-replayable"] as const) {
  test(`previously imported Claude history becomes a real session for ${clientMode}`, async () => {
    const dir = await mkdtemp(join(tmpdir(), "knorvia-import-recovery-"));
    setDataBaseDir(dir);
    const meta = {
    ...
    type CreateInput = Parameters<Options["agentService"]["createSession"]>[0];
    const created: CreateInput[] = [];
    let session: KnorviaSessionStateSnapshot | undefined;
    ...
    const service = createKnorviaTaskServiceAdapter({
      ...
      agentService: {
        ...
        async createSession(input: CreateInput) {
          created.push(input);
          session = sessionStateSnapshotSchema.parse({
