...
import { createKnorviaTaskServiceAdapter } from "../src/agent/taskServiceAdapter.js";
import {
...
const meta = {
  ...
  updatedAt: 2,
  mode: "build" as const,
  provider: "knorvia" as const,
};

test("current Project Memory catalog and files remain readable", async () => {
  const dir = await mkdtemp(join(tmpdir(), "knorvia-current-memory-"));
  setDataBaseDir(dir);
  try {
...
test("opening the task index leaves retired ACP IDs and user rows untouched", async () => {
  const dir = await mkdtemp(join(tmpdir(), "knorvia-acp-index-"));
...
test("missing sessions report the owner error even when a valid ACP snapshot exists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "knorvia-acp-snapshot-"));
  setDataBaseDir(dir);
  ...
  type Options = Parameters<typeof createKnorviaTaskServiceAdapter>[0];
  const service = createKnorviaTaskServiceAdapter({
    agentService: {
      async resumeSession() {
        throw ownerError;
      },
...
test("current session recovery preserves Desktop and replayable projections", async () => {
  const snapshot = sessionStateSnapshotSchema.parse({
  ...
  try {
    ...
    assert.equal(desktop?.meta.provider, "knorvia");
...
test("current Provider configuration starts without an old config migration callback", async () => {
  const dir = await mkdtemp(join(tmpdir(), "knorvia-provider-current-"));
  const runtime = createProviderConfigRuntime({
    knorviaBuiltinFilePath: fileURLToPath(
  ...
  try {
    ...
    assert.ok(config.knorviaBuiltinRevision);
