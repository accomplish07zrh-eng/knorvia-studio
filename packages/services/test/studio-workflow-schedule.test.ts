...
test("existing task database upgrades Studio schedule without changing historical checksums", () => {
  ...
  try {
    ...
    const historical = db
      .prepare("SELECT id, checksum FROM tasks_schema_migration WHERE id < '0005' ORDER BY id")
      .all();
    ...
    const oldAutomation = db
      .prepare(
        "SELECT studio_workflow_id, prompt FROM automations WHERE automation_id='before-upgrade'",
      )
      .get();
    ...
    assert.ok(
      db
        .prepare("SELECT 1 FROM tasks_schema_migration WHERE id='0005_studio_workflow_schedule'")
        .get(),
    );
  } finally {
    db.close();
...
test("Studio schedule rejects a chat binding, model override and invalid target", async (t) => {
  ...
  await assert.rejects(
    service.create({ ...base, modelSelection: { providerId: "p", modelId: "m" } }),
    /不能绑定聊天会话/,
  );
  await assert.rejects(
    service.create({ ...base, studioWorkflowId: "bad id" }),
    /无效的 Studio 工作流/,
  );
