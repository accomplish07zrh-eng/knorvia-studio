...
/**
 * 让 Environment 的 Knorvia 控制面 Endpoint 同时决定 Active/LKG 与刷新控制路径。
...
export class EndpointScopedKnorviaBuiltinSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  ...
  async #resolveCurrent(): Promise<CurrentEndpointSource> {
    const endpointOrigin = normalizeKnorviaBuiltinEndpointOrigin(
    ...
    const paths = resolveKnorviaBuiltinCachePaths({
      ...
      knorviaEndpointOrigin: endpointOrigin,
    });
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("EndpointScopedKnorviaBuiltinSource 已 dispose");
  }
}
