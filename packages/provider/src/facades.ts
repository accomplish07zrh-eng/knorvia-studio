...
export class ProviderSettingsFacade {
  ...
  getView(): ProviderSettingsView {
    ...
    return createProviderSettingsView({
      ...
      knorviaBuiltinProviders: snapshot.config.knorviaBuiltinProviders,
      knorviaBuiltinProviderTemplates: snapshot.config.knorviaBuiltinProviderTemplates,
  ...
  resolveModelConfig(input: ResolveModelConfigInput): ModelConfigResolution {
    ...
    if (!("personalConfig" in input)) {
      const modelRules = ModelConfigRules.composeEffective(
        snapshot.config.knorviaBuiltinModelRules,
    ...
    const inheritedConfig = snapshot.config.knorviaBuiltinModelRules.resolve({
      providerId: input.providerId,
      templateId: provider.templateId,
      modelId: input.modelId,
      apiType: provider.config.api?.type,
...
function createProviderSettingsView(input: {
  revision: number;
  knorviaBuiltinProviders: ProviderRegistryServiceSnapshot["config"]["knorviaBuiltinProviders"];
  knorviaBuiltinProviderTemplates: ProviderRegistryServiceSnapshot["config"]["knorviaBuiltinProviderTemplates"];
  ...
  return Object.freeze({
    ...
    providerTemplates: Object.freeze(
      (input.knorviaBuiltinProviderTemplates ?? ProviderTemplateMap.empty())
        .entries()
