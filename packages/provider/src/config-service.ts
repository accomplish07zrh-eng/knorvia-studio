...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    ...
    this.#sourceDisposers = [
      this.#knorviaBuiltinSource.onDidChange((reason) => this.#emit(`knorviaBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [knorviaBuiltin, personal] = await Promise.all([
      this.#knorviaBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([knorviaBuiltin.revision, personal.revision]),
  ...
  async savePersonalProviderOverlay(
    ...
    const knorviaBuiltin = await this.#knorviaBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = knorviaBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = knorviaBuiltin.providers.overlay(current.providers);
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(knorviaBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = knorviaBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? knorviaBuiltin.providerTemplates?.get(templateId) : undefined;
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        knorviaBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(knorviaBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(knorviaBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(knorviaBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(knorviaBuiltin, current.providers, normalizedProviderId);
...
function normalizeProviderOrder(
  _knorviaBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  knorviaBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    knorviaBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(knorviaBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? knorviaBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return knorviaBuiltin.providers.overlay(personal);
}
