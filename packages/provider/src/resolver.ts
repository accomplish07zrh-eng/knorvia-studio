...
import type { completeModelConfigDataSchema } from "@knorvia/shared/model-config";
import type {
...
export interface ProviderConfigResolverInput {
  readonly knorviaBuiltinProviders: ProviderConfigMap;
  readonly knorviaBuiltinProviderTemplates?: ProviderTemplateMap;
  readonly personalProviders: ProviderConfigMap;
  readonly knorviaBuiltinModelRules: ModelConfigRules;
  ...
  resolve(input: ProviderConfigResolverInput): ProviderConfigResolution {
    const accountProviders = new ProviderConfigMap(
      input.accountProviders
        .entries()
        .filter(([providerId]) => input.knorviaBuiltinProviders.has(providerId))
        .map(([providerId, config]) => [providerId, config.withoutGroup()] as const),
    );
    const concreteBuiltinProviders = input.knorviaBuiltinProviders.overlay(accountProviders);
    const providerTemplates = input.knorviaBuiltinProviderTemplates;
    ...
    const effectiveModelRules = ModelConfigRules.composeEffective(
      input.knorviaBuiltinModelRules,
    ...
    for (const providerId of resolveProviderOrder(input, effectiveProviders)) {
      ...
      const models = orderedModelIds.map((modelId): ResolvedProviderModel => {
        ...
        const effectiveBuiltinConfig = input.knorviaBuiltinModelRules.resolve({
          providerId,
          templateId,
          modelId,
          apiType: config.api?.type,
...
function resolveProviderOrder(
  ...
  const builtinIds = input.knorviaBuiltinProviders
    .keys()
