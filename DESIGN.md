...
Knorvia Studio is a desktop-first and web-compatible AI workspace. The interface should feel calm, dense, and operational rather than decorative.
...
- System
- Light Theme, backed by the `knorvia-light` class (Knorvia black-and-white values)
- Dark Theme, backed by the `knorvia-dark` class (Knorvia black-and-white values)

The active theme identifiers use the `knorvia-*` prefix; old saved identifiers are normalized at the preference-reading boundary. Default light and dark CSS variables still exist as fallback foundations, but new UI must be validated against these two active themes.

## Knorvia visual language

Spec: `specs/knorvia-visual-language.md`. The language grows from the Knorvia icon and is more than colour:

- **Colour**: pure black and white, zero-hue greys, one tone family separated by lightness (light: white workspace on `#f3f3f4`; dark: `#131313` on `#0a0a0a`). No purple or pink. Primary actions are pure black in light and pure white in dark; tooltips are inverted.
- **Ribbon shapes**: actions are pills or circles (buttons, navigation items, tabs/segments, single-line inputs, select triggers, send). Content containers are soft slabs with generous radii. Joined button groups keep their joined shape; menu items stay compact.
...
- `rounded-full` is reserved for deliberate pill shapes or circles. In the Knorvia language, buttons, navigation items, tabs, single-line inputs and select triggers are deliberate pills (applied centrally in `styles.css`); do not hand-roll other pills.
- Tags and counters do not become pills merely because of their component type.
...
Knorvia Studio should use restrained depth. Layer primarily through background contrast, borders, and radius before relying on heavy shadows.
...
The workspace navigation follows `specs/knorvia-activity-rail.md`: a 56px activity rail, the existing resizable context sidebar, and the main workspace. The rail persists beside settings; tools, including the plugin-management shortcut, and kernel shortcuts occupy separate sections on one flat structural surface. In this layout, a single rounded sheet contains both the context sidebar and the main content, below a shared 40px titlebar. Internal conversation, terminal and Side Pane frames use dividers without additional rounded outlines or paper shadows. Settings uses the same sheet. Keep the current paper selection tokens and accessible names on icon actions. Narrow Web retains the existing compact navigation and its plugin settings entry.
...
- Ensure contrast remains safe in Knorvia light and dark modes.
