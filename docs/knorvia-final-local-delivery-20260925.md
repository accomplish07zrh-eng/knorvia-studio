...
本次 Windows 本地规划已完成。私有仓库 `accomplish07zrh-eng/knorvia-studio` 的 `main` 分支（提交 `e7b5951`）包含最终源码；便携包来自提交 `5aec8d5`，版本 `0.8.0-preview.1`，以 `KNORVIA_ENV=production` 构建，产品身份为 Knorvia Studio。上游仓库未接收推送。
...
- 交付分支以 `main` 为准。原 `knorvia/clean-base` 分支与 `main` 指向同一提交 `e7b5951`，已停用，不再作为交付分支；仓库脚本、CI 与配置均不依赖它。
- 文档中的本机用户目录统一写作 `%USERPROFILE%`，不再记录真实用户名。
- `Studio offline checks`（`.github/workflows/studio-offline.yml`）改为在推送 `main` 和提交 PR 时自动运行，仍可手动触发。截至本次更新，该工作流尚未在云端实际运行过，上文结果仍全部来自本机。
- README、README.en.md、FORK-NOTES.md 与 AGENTS.md 已按当前功能范围更新。
