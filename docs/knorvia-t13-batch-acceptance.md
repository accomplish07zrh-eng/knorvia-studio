...
| 版本           | `0.8.0-preview.2`（**交付记录**；收尾时根 `package.json` 已升到 `0.8.0-preview.3` 并重新构建交付，见 `docs/knorvia-0.8.0-preview.3-release.md`）                                                                                                                                      |
...
**「差异审阅」与「用户接纳」都已跑通**。把主持人规划与复核都按 JSON 契约回给夹具后，群运行进入复核阶段；再打开群聊信息面板（`aria-label=studio.groups.details` 的按钮）、展开运行历史里的 `<details>`、点步骤行上的「查看修改」、最后点审阅卡的「应用此文件」，真实项目即按隔离快照内容被写入。完整输出：
...
踩到的四个坑（都写进了脚本注释，便于复现）：① 不能只看 `hasToolResult` 判断回合——主持人的复核请求历史里同样带着成员的 `tool_call_id`，按它判断会把复核当成成员回合而回纯文本，于是报 `Host returned invalid structured output`，必须按回合状态机（规划 → 成员 → 成员收尾 → 复核）判断；② 成员回合会被重复请求、每次重新弹审批，所以审批要**反复批准**而不是只点一次；③ 群聊审批按钮在 `StudioInteractions.tsx`，不是单聊那套 `PermissionDialog`；④ 运行历史里每个运行是 `<details>`，步骤行上的「查看修改」按钮**必须先展开**才可见，且只在运行结束后出现。

**仍未覆盖**：真实模型、真实 CLI/ACP 内核与付费调用（未获授权）；差异审阅与接纳只覆盖了单文件路径，没有覆盖多选、冲突与「应用后断线」等组合（那些由 `studio-workspace*.test.ts` 等离线用例覆盖）。
...
1. **真实完整路径已在打包应用上跑通（离线夹具，非真实模型）**：两个脚本合起来覆盖了任务书 T13 第 3 步要求的全部环节——`scripts/t13-desktop-acceptance.mjs`（便携模式下也通过）覆盖选择内核/供应商与模型 → 真实发送 → 权限门禁 → 显式批准 → 真实工具执行并落盘 → 停止中止在途请求 → 重开核对；`scripts/t13-isolated-workspace-acceptance.mjs` 覆盖选中 isolated 群聊 → 群聊审批 → **隔离执行**（写入落在隔离快照、真实项目未被直接改写）→ **差异审阅**（复核面板出现）→ **用户接纳**（应用后真实项目按隔离快照内容被写入）。全程只连 `127.0.0.1`。**仍未做的**：真实模型、真实 CLI/ACP 内核与付费服务调用（未获授权，任务书要求「需要付费的部分必须先获得明确授权」）；差异审阅与接纳只覆盖单文件路径，未覆盖多选、冲突与「应用后断线」等组合（这些由离线用例覆盖）。
