...
| 项目           | 值                                                                                                                                                                                                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 本批次功能清单 | T01 发布门禁、T02 测试稳定性、T03 升级/便携数据保护、T04 内核分层状态（后端+界面）、T05 交付摘要与接纳、T06 输出契约、T07 Host 引用与参数、T08 工作流场景、T09 创作来源（服务层+界面）、T10 性能基线与运行历史收敛、T11 技能包与兼容面板、T12 受限 CUA 契约与失败关闭门禁 |
| 构建时提交     | `dc51d26b504ac268d132d92634ec7a989e50d0a2`（构建期间工作区干净）                                                                                                                                                                                                          |
| 记录提交       | `5df3480`（仅更新执行记录与验收文档，不含代码改动）                                                                                                                                                                                                                       |
| 版本           | `0.8.0-preview.2`                                                                                                                                                                                                                                                         |
| 远端           | `origin/main` 已包含全部批次提交（T01–T12 与各次修复）                                                                                                                                                                                                                    |
...
```text
Portable data before: 430 files, 54840957 bytes
Portable data verified: 430 files, 54840957 bytes, SHA-256 identical; robocopy code 3
Program files verified: 118 SHA-256 identical to the build
Delivered executable SHA-256: 65552C6E565A03065B0D99F229A7E33517D5BB87140388BDFE11B3735E48B354
```
...
```json
{
  ...
  "exeSha256": "65552C6E565A03065B0D99F229A7E33517D5BB87140388BDFE11B3735E48B354",
  "asarSha256": "763F3ED8DF3CB152E92750A6A522F88E6CFF802F5DD38A9FB284E8F3EF62DC00",
  "verifiedAt": "2026-09-25T14:54:05.1065597+00:00"
}
```
...
| 启动前 `data` | 430 文件 / 54 840 957 字节                |
| 启动后 `data` | 430 文件 / 54 840 957 字节                |
| 丢失文件      | **0**                                     |
| 新增文件      | **0**                                     |

结论：便携包可以打开；覆盖只更新程序文件；`data` 中已有用户文件一个都没有丢失，本次启动也没有改动 `data` 内容。

> 说明：本轮共交付 5 次便携构建（`48894e8`、`7011f67`、`6ee777d`、`81b2073`、`dc51d26`），每次都用同一交付脚本覆盖程序文件，`data` 逐文件 SHA-256 前后一致。首次交付时的启动验证曾新增 1 个 Chromium 会话文件（`profile\session\DIPS-wal`），此后各次启动前后完全一致。

## 交付物哈希

| 产物                                                | SHA-256                                                                                |
| --------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `Knorvia Studio.exe`（便携目录内）                  | `65552C6E565A03065B0D99F229A7E33517D5BB87140388BDFE11B3735E48B354`                     |
| `resources/app.asar`（便携目录内）                  | `763F3ED8DF3CB152E92750A6A522F88E6CFF802F5DD38A9FB284E8F3EF62DC00`                     |
| 安装包 `Knorvia Studio-0.8.0-preview.2-win-x64.exe` | `CA9E29DECD9B3535CDF823739744A508A68A64BADC358159C1AC1DEED02C8DE1`（150 047 155 字节） |
...
| 性能与长会话                                | T10 滚动/启动基线、运行历史有界化前后对比                                                                                   | 已覆盖（启动未优化，未达 3 秒目标）              |
...
7. **T10 未优化启动**：实测 5.9–6.4 秒冷启动、未达 3 秒目标；耗时主要在被测范围外的 Host/存储层，未改动启动代码。
...
3. 代码回退：按任务回退对应提交（T01 `797a76e`、T02 `657a00b`、T03 `d62c610`、T06 `e4208d3`、T09 `3dc3a62`/`6ee777d`、T04 `18acac7`/`a882ae9`、T05 `b6fb13c`、T07 `7011f67`、T08 `0797b30`、T10 `81b2073`、T11 `48894e8`/`11a5660`、T12 `dc51d26`），代码回滚不自动撤销用户已应用的文件。
