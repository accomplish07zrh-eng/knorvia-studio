...
## 7. 后续（本轮第 2 轮）：并行准备两个数据库

复核确认 2.8–3.0 s 的存储准备是**两段串行**：tasks-index 先被 await，之后才是 CLI 会话库；两者是不同文件、不同 WAL，互不依赖。已改为并行准备（`packages/desktop/src/host/hostDatabaseStartup.ts`），并保持原有可观察行为：阶段通知顺序不变（会话阶段先缓冲后补发）、失败优先级不变（先 tasks-index 再会话库）、tasks-index 失败时不补发缓冲通知。

| 指标                               | 改动前 `dc51d26` | 改动后 `6e1b4f2` |
| ---------------------------------- | ---------------- | ---------------- |
| 首次可交互 median                  | 5799 ms          | 5554 ms          |
| Host fork → 首次可交互（中位样本） | 3782 ms          | 3516 ms          |
| `[database-startup]` durationMs    | 2906 ms          | 2654 ms          |

收益约 250–300 ms，**`<3 秒` 仍未达标**（约 5.5 s）。剩余固定成本是 15.9 MB CLI 包启动与 SQLite 建库/WAL/fsync，以及约 0.8 s Host 启动、约 0.24 s 服务构造、约 0.18 s 端口/IPC/渲染，全部仍在同一条串行链上。要达标必须把「首次可交互」与数据库就绪解耦（需要引导页在服务挂载前可用，涉及 UI/架构与失败路径），这是需要显式决策的产品问题，不作为启动微优化偷偷带入。

已明确不做（复核列为禁止项）：删除第 2/3 个工作目录的会话库准备（相对 `sessionDbPath` 按各自 cwd 解析，会导致某个库静默未准备）、跳过快照/backfill/账本校验、在 `prepare` 完成前释放端口或挂载服务。
