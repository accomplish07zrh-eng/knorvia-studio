...
### 各工具页的阅读底色统一（2026-09-26）

- 工作流点阵画布也继承工作面，不能用画布内部的整面填色再次盖住背景。节点、缩略图与浮动详情保留独立阅读材质。
- 窄栏布局的阅读底色统一归 `StudioWorkspaceFrame` 的工作面所有；内部聊天、群聊、工作流、创作、自动化和设置页面的结构容器不再叠加不透明背景。传统布局继续由 conversation/settings frame 提供阅读底色。
- 创作的页根、历史区和底部表单区域继承工作面的底色；群聊与工作流的页根同样处理。卡片、输入框、浮动详情栏和菜单仍使用各自的阅读材质，不能用全局透明规则抹掉对比度。
- 检查全部工具入口，以及插件、常规、外观、Agent 和模型等设置分区的页面底层。对代码／媒体预览、图标底板、图表与实际内容卡片按语义区分，不能将所有 `bg-background` 或白色统一替换。
- 验收使用深浅主题、玻璃开关、背景图与降低透明度偏好的隔离场景，检查结构背景及阅读面计算值、页面可达性与滚动；不把 CSS 验证声称为原生桌面合成目视通过。
