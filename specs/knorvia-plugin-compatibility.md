...
- 面板的渲染层自动化测试（`packages/ui/test` 无 DOM harness，只覆盖纯投影模块）。
- 面板在真实插件列表里的观感、远端 host 的 sidecar 读取（未实测）。

## Settings 兼容性面板的渲染规则

本节定义 `.knorvia-plugin/compatibility.json` 在设置页插件管理入口内的呈现规则。
面板**不是**新页面、不是新入口：它复用设置页唯一的插件管理入口（`packages/ui/src/settings/PluginsSection.tsx`），
渲染在插件详情的可展开区内，`compatibility.json` 仍是唯一数据来源。

### 状态在屏幕上意味着什么

| 状态          | 屏幕上必须说的事                                                            | 严禁出现                                 |
| ------------- | --------------------------------------------------------------------------- | ---------------------------------------- |
| `verified`    | "已验证"+ 逐条列出 `evidence`（可复核产物）；没有 `evidence` 时不渲染该状态 | 无证据的"已验证"                         |
| `declared`    | "已声明（未验证）"：作者声明了目标形态，但**没有**该内核上的运行证据        | "已支持"、"可用"、"可运行"               |
| `unsupported` | "该内核不支持"+ 原样显示作者的 `reason`                                     | 任何"暂时不可用、稍后可用"之类的软化措辞 |
| `unknown`     | "未验证"+ 原样显示作者的 `reason`；缺失或无法解析 sidecar 时也落到这里      | 把"未验证"渲染成"支持"或空白             |

硬规则：

- **未验证必须显示为未验证。** `declared` 与 `unknown` 两种状态在界面上都要出现"未验证"字样，
  不能只靠颜色区分，也不能渲染成中性或无状态。
- **只有在 `verified` 且 `evidence` 非空时才允许出现"已验证"字样。** 状态徽章、颜色、图标都不得
  比文案更乐观：`declared` 与 `unknown` 使用同一"未验证"语气，`unsupported` 使用明确的否定语气。
- sidecar 缺失、读取失败、JSON 非法、字段类型不符、`compatibilityVersion` 不是 `1`、
  `plugin` 与 manifest 不一致：一律按 `unknown` 渲染，**不得**中断渲染、不得抛出、不得回退成"支持"。
  面板必须同时显示它读的是哪个路径，方便用户自查。
- 每个声明的 `requires[]` 能力都要逐条列出处置结果：可用 / 不可用 / 未验证。
  **宿主没有上报该能力时只能是"未验证"，不得写"可用"。**
- `whenMissing: "refuse"` 的能力处于未验证或不可用状态时，面板必须显著标注"该能力缺失时整个请求会被拒绝"。
- 面板**只读**：不安装、不启用、不执行、不修改 `compatibility.json`；没有任何安装、付费、账号或订阅入口。
- 面板不得用 `compatibility.json` 的存在推断插件已经安装、已启用或已在某内核跑过。

### 与"可安装 ≠ 受支持"的关系

上文的四级判定（结构预检 / 宿主 schema / 已安装 / 已启用）在界面上继续分开呈现：
插件行的启用开关只表达"已启用"，兼容性面板只表达"作者按内核声明了什么、验证到哪一步"。
两者**不得**互相蕴含：打开启用开关不能让 `declared` 变成 `verified`，
`verified` 也不代表该插件在当前工作区已经启用。
